...
!  $Id: GEOS_SurfaceGridComp.F90,v 1.174.2.4.4.14.4.1.4.1 2023/01/12 03:41:08 mmehari Exp $
...
!-----------------------------------
    ...
    if (RoutingFile /= "") then
       ...
       if (DischargeAdjustFile /= "null") then
          ...
          call MAPL_CFIORead(DischargeAdjustFile, Bundle, RC=STATUS)
          VERIFY_(STATUS)
          ...
          call MAPL_GetResource ( MAPL, ratiocap, Label="RATIOCAP:", DEFAULT=2.0, RC=STATUS)
          VERIFY_(STATUS)

          where(PCMEDISTILE /= 0.0 .and. PUMEDISTILE /= 0.0 .and. TileType == MAPL_OCEAN) &
             DISCHARGE_ADJUST = MIN(PUMEDISTILE/PCMEDISTILE,ratiocap)
...
!  snowfall. The following scales model precips by the total file precip.
!
...
!-------------------------------------------------------------
    ...
    if (associated(PRECTOT)) then
       TMPTILE = PCUTILE + PLSTILE + SNOFLTILE
...
! Some cannot be verified, because some children don't produce them
      ...
      call MAPL_GetPointer(GEX(type), dum, 'TSKINICE', ALLOC=associated(TSKINITILE  ))
      ...
      call MAPL_GetPointer(GEX(type), dum, 'SS_FOUND', ALLOC=associated(SS_FOUND_TILE ))
