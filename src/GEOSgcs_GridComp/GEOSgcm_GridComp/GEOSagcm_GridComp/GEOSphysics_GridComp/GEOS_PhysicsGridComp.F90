! $Id: GEOS_PhysicsGridComp.F90,v 1.128.2.3.28.1.8.2.2.2.2.1.4.1 2023/01/12 03:41:08 mmehari Exp $
...
!AMM terminate TH import for chem - will fill it here with value after turb
     if ( SYNCTQ.eq.1.) then
       call MAPL_TerminateImport  ( GC,    &
          SHORT_NAME = (/'TH'/),           &
          CHILD = CHEM,                    &
...
! Local derived type aliases
   ...
   real, pointer, dimension(:,:,:)     :: QV, QLLS, QLCN, QILS, QICN, QW
...
!AMM
   real, pointer, dimension(:,:,:)     :: TGWD, DTDTGWD, THFORMOIST
   real, pointer, dimension(:,:,:)     :: SAFTERMOIST, THAFMOIST
   real, pointer, dimension(:,:,:)     :: THFORCHEM, TFORRAD
   ...
   real(kind=MAPL_R8), allocatable, dimension(:,:) ::  dwdt
...
!  get pointer to moist import TH (moist has an import T, but its never used in there)
...
!-srf-gf-scheme
    call MAPL_GetPointer(GEX(MOIST), DTDT_BL, 'DTDT_BL', alloc = .true. ,RC=STATUS); VERIFY_(STATUS)
    call MAPL_GetPointer(GEX(MOIST), DQDT_BL, 'DQDT_BL', alloc = .true. ,RC=STATUS); VERIFY_(STATUS)
!-srf-gf-scheme
...
!--------------------------
    ...
    if ( SYNCTQ.eq.1. ) then
!  AMM - Now Update T (for RAD) and TH (for CHEM) using S after turb run 2 assume change in S is all change in T
...
!---------------------------------------------------------------
   ...
   !  ----------------------------------------------------------------
       do l=1,lm
              dq(:,:,L) =               QV(:,:,L) ! - DT*DQVDTCHM(:,:,L)  We do not keep dry-mass budget for CHEM constituents
              dq(:,:,L) = dq(:,:,L) + QLLS(:,:,L)
              dq(:,:,L) = dq(:,:,L) + QLCN(:,:,L)
              dq(:,:,L) = dq(:,:,L) + QILS(:,:,L)
              dq(:,:,L) = dq(:,:,L) + QICN(:,:,L)
   ...
   !  ------------------------------------------
       ...
       do l=1,lm
           ...
           !  dwdt(:,:)   = 1.0_8/( 1.0_8  + dq(:,:,L) )
              dwdt(:,:)   = (ple(:,:,L)-ple(:,:,L-1)) / (ple_new(:,:,L)-ple_new(:,:,L-1))
