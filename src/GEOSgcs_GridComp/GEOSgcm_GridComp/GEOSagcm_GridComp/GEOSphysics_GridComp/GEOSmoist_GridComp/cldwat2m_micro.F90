...
#define GEOS5
   ...
   !++jtb
   !
   real(r8), parameter :: rhmini       = 0.80_r8       ! Minimum rh for ice cloud fraction > 0.
   ...
   real(r8) :: trice =  35.00_r8  !!Changed Donifan
   real(r8) :: miu_ice = 0.0 !DONIF       
#endif
   !--jtb
...
contains
   ...
   subroutine ini_micro(Dcs_, QCVAR_)
      ...
      call gestbl(tmn     ,tmx     ,trice   ,ip      ,epsilon  , &
      ...
      miu_ice=0._r8
      cons1=gamma(1._r8+miu_ice+ di)/gamma(1._r8+miu_ice) !DONIF  
      ...
      !cons16=gamma(1._r8+bi+miu_ice)/gamma(1._r8+miu_ice) !DONIF 
      !cons17=gamma(4._r8+bi+miu_ice)/gamma(4._r8+miu_ice)  !DONIF
   ...
   subroutine mmicro_pcond (                   &
         ...
         lamcrad,qsout2,dsout2,qrout2, drout2, &
      ...
      real(r8), intent(in) :: pint(pcols,pver+1)    ! air pressure layer interfaces (pa)
      ...
      real(r8), intent(in) :: omega(pcols,pver)    ! vertical velocity (m/s) DONIF
      ...
      real(r8), intent(out) :: mnuccdo(pcols,pver) ! mixing rat tend due to deposition ice nucleation
      real(r8), intent(out) :: nnuccto(pcols,pver) ! number mixing ratio tend due to contact frezzing
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      ...
      real(r8), dimension(:) ::  ndfaer(nbincontactdust), nslip(nbincontactdust), slip(nbincontactdust)
      ...
      do k=1,pver
         ...
         do i=1,ncol
            ...
            ! hm, add 6/23/11 for droplet activation tendency fix  !DONIF added on 01/12/2012
            ! move calculation of dum2l and ncmax to AFTER lcldm has
            ! been assigned a value. This only impacts dum2l and 
            ! diagnostic output.

            ! note: ncold is used below since nc has already been updated
            ! from activation above

            if (qc(i,k).ge.qsmall) then


               dum2l(i,k)=(ncold(i,k)+npccn(k)*deltat)/lcldm(i,k)          
               dum2l(i,k)=max(dum2l(i,k),cdnl/rho(i,k)) ! sghan minimum in #/cm3                   
               ncmax = max(dum2l(i,k)*lcldm(i,k), 0._r8) !maximum number                            
               dum2l(i,k) = npccn(k)*deltat/lcldm(i,k) !actual nucleated number        
            ...
            if (t(i,k).lt.273.15) then
               ...
               if (qi(i,k).gt.qsmall) then
                  ...
                  if (qiic(i,k).ge.qsmall) then
                     ...
                     lami(k) = (cons1*ci*niic(i,k)/qiic(i,k))**(1._r8/di) !estimate lami(k) with mu= 0.0              
                     !miu_ice=mui_hemp(t(i, k)) !correct mu   
                     miu_ice=mui_hemp_l(lami(k))          
                     aux =  (gamma(1._r8+miu_ice+ di)/gamma(1._r8+miu_ice))**(1._r8/di)                                  
                     lami(k) = aux*lami(k)
                     n0i(k) = niic(i,k)*(lami(k)**(miu_ice +1.0))/gamma(1._r8+miu_ice)   
                     ...
                     !epsi = (miu_ice+1._r8)*2._r8*pi*n0i(k)*rho(i,k)*Dv(i,k) &
                     !/(lami(k)*lami(k)) !DONIF

                     epsi = (miu_ice+2._r8)*(miu_ice+1._r8)*pi*niic(i, k)*rho(i,k)*Dv(i,k) &
                           /lami(k) !DONIF
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                  ...
                  if (bergtsf.lt.1._r8.or.icldm(i,k).gt.lcldm(i,k)) then
                     ...
                     if (qiic(i,k).ge.qsmall) then
                        ...
                        if (qc(i,k).lt.qsmall.or.icldm(i,k).gt.lcldm(i,k)) then   !DONIF 
   ...
   ! make substepping dynamic 
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!   
   iter  =  FLOOR(deltatin/deltat_micro) +1 !DONIF
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
         ...
         do it=1,iter
            ...
            do k=1,pver
               ...
               if (cmei(i,k) < 0._r8 .and. qi(i,k) > qsmall .and. cldm(i,k) > mincld) then
                  ...
                  !if  (nsubi(k) .lt. -ni(i,k)/deltat) nsubi(k) = -ni(i,k)/deltat !DONIF Sublimation cannot decrease Ni below zero         
               ...
               ! print *, '==', k, t(i, k), nsubi(k),  ni(i,k), cmei(i,k), qi(i, k), cldm(i, k), deltat          
...
!                  end if
 ...
 !                 pgam(k)=min(pgam(k),15._r8) !DONIF Changed to avoid numerical instability
               ...
               if (qcic(i,k).ge.1.e-8_r8) then
                  ...
                  prc(k) =cons2/(cons3*cons18)*1350._r8*qcic(i,k)**2.47_r8* &  !remember to remove this!!!***************8   
                        (ncic(i,k)/1.e6_r8*rho(i,k))**(-1.79_r8)  ! KK USing factor to correct scale dependency of autoconversion



#ifdef GEOS5   
                  !option to use different autoconversion schemes DONIF
                  ! 0-default, KK2000 
                  !1- Sundqvist 1989
                  !2- Delgenio 1996
                  !3- Apply a factor based on the relation between KK and SD 
                  !4- Total autoconversion function from Liu et al. 2006, Eq. 8a

                  auto_option = 4

                  if (auto_option .eq. 1) then

                     prc(k) =1.0e-3*qcic(i,k)*(1.0 - exp(-(qcic(i,k)*rho(i, k)/3.0e-4)**2.0))      
                     prc(k) =prc(k) * gamma(1.0+qcvar)/(cons3*qcvar)   !using sub-grid scale Enhancement factor 

                  elseif (auto_option .eq. 2) then

                     prc(k) =1.0e-4*qcic(i,k)*(1.0 - exp(-(qcic(i,k)*rho(i, k)/7.5e-4)**4.0))      
                     prc(k) =prc(k) * gamma(1.0+qcvar)/(cons3*qcvar)   !using sub-grid scale Enhancement factor 

                  elseif (auto_option .eq. 3) then

                     !prc(k) = prc(k)*(2.0e-3*( 1.0 - EXP( - ( qcic(i,k) / 8.0e-4 )**2 ) )) &
                     ! / (2.0*1350.0*(qcic(i,k)**2.47) *(100.0**-1.79)) !increase the autoconversion based on LWC    
                     prc(k) = prc(k)*(1.0e-3*( 1.0 - EXP( - ( qcic(i,k) / 3.0e-4 )**2.0 ) )) &
                           / (1350.0*(qcic(i,k)**2.47) *(50.0**(-1.79))) !increase the autoconversion based on LWC    

                     ! prc(k)= 1.0e-4*qcic(i,k)*(1.0 - exp(-(qcic(i,k)*rho(i, k)/7.5e-4)**4.0))/ &
                     !               (2.0*1350.0*(qcic(i,k)**2.47) *(1.0**-1.79))

                  elseif (auto_option .eq. 4) then

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                     ! Autoconvesion rate from Liu, Yangang, et al. Journal of the atmospheric sciences 63.3 (2006): 1103-1109.   
                     xs=1._r8/(1._r8+ pgam(k)) !returs e^2
                     !print *, 'mfg', sqrt(xs)

                     beta6=(1._r8+3.0*xs)*(1._r8+4.0*xs)*(1._r8+5.0*xs) / (1._r8+ xs)/(1._r8+2.0*xs)
                     ...
                     xs=min(20.0, xs)    
                     prc(k) =  1.1e10*beta6*LW*LW*LW*(1.0 - exp(-(xs**miu_disp))) /NW                         
                     prc(k)=prc(k)*1.0e3/rho(i, k) ! return to SI units

                     !recalculate qcvar 
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                     !xs=min(max(0.5, lcldm(i, k)), 1.0)
                     !xs = 0.23*(10.14**xs)   !From Baker et al. 1994
                     prc(k) =prc(k) * gamma(2.0+qcvar)/(gamma(qcvar)*(qcvar*qcvar))!!

                     ! prc(k) =prc(k) * gamma(1.0+qcvar)/(cons3*qcvar)  ! assume KK2000 dependencies to work out enhancement factor

                     !tune autoconversion. Require since our aerosol size distribution is assumed.
                         
                         
                    ! auxx  =  1.0-max((273.0 - t(1, k))/15.0, 0.0) ! Kludge to maintain Arctic stratus 2019
                         
                     prc(k) =prc(k)*dcrit!*max(auxx, 0.001)
                     

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                     !NUmber autoconversion rate is approximated according to Liu, Yangang, et al. Geophysical Research Letters 34.16 (2007).
                     ! Assumes e =  0.33. Replaces previous formulation by Barahona et al. 2014 GMD

                     xs= 1/xs !Eq. 8c   (used below)
                  ...
                  nprc(k) = prc(k)/cons22   
                  !nprc1(k) = prc(k)/(qcic(i,k)/ncic(i,k)) ! Assumes prroportional fraction (MG08)      
               ...
               if (t(i,k).le.273.15_r8.and.qiic(i,k).ge.qsmall) then
                  ...
                  !  psc=(p(i, K)/100000.0)**0.4
                  !  Bh= min(max(0.244-0.0049*(t(i, k) -273.0), 0.245), 0.58)
                  ! Bh=1/Bh

                  !vaux = 1.0
                  !vaux =max(min(omega(i, k), 20.0), 0.1)
                  !xs= ((psc*vaux/2.0)**Bh)*0.01

                  !xs = min(max( xs, dcs), 1.e-2) !Heymsfield et al. 2007!


                  ! note: assumes autoconversion timescale of 180 sec
                  !vaux = 180.0_r8*10.0_r8
                  vaux= ts_auto_ice
                  if (.false.) then !Assume exponential 

                     ! nprci(k) = n0i(k)/(lami(k)*vaux)*exp(-lami(k)*dcs)  
                     nprci(k) = (niic(i, k)/vaux)*exp(-lami(k)*dcs) !assume monodisperse
                     ...
                     !
                  else !Use the whole gamma

                     miu_ice=mui_hemp_l(lami(k))    
                     
                     !nprci(k) = (niic(i, k)/ts_auto_ice)*(1._r8-gamma_incomp(miu_ice, lami(k)*dcs)) 
                     !prci(k) = ((pi*rhoi*niic(i, k))/(6._r8*ts_auto_ice*lami(k)*lami(k)*lami(k)))*gamma_incomp(miu_ice+3._r8, aux) 
                      !Updated Donifan 2018
                     
                     nprci(k) = (niic(i, k)/ts_auto_ice)*(1._r8 - gammp(DBLE(miu_ice), DBLE(lami(k)*dcs)))                                           
                     prci(k) = (qiic(i, k)/ts_auto_ice)*(1._r8 -gammp(DBLE(miu_ice+3._r8), DBLE(lami(k)*dcs)))                     
               ...
               if (qcic(i,k).ge.qsmall .and. t(i,k).lt.269.15_r8) then
...
#ifdef GEOS5    
                  if (.true.) then  !DONIF New Immersion Parameterization (Barahona et al 2013)

                     !mnuccc(k)=cons9/(cons3*cons19)* &
                     !           pi*pi/36._r8*rhow* &
                     !       cdist1(k)*gamma(7._r8+pgam(k))*nimm(i, k)/ &
                     !       lamc(k)**3/lamc(k)**3

                     !nnuccc(k)=cons10/(cons3*qcvar)* &
                     ! pi/6._r8*cdist1(k)*gamma(pgam(k)+4._r8) & 
                     ! *nimm(i, k)/lamc(k)**3  !DONIF 
                     nnuccc(k) = nimm(i, k)
                  ...
                  do ind_aux  = 1,  nbincontactdust
                     ...
                     ndfaer(ind_aux)=1.381e-23_r8*t(i,k)* nslip(ind_aux)/(6._r8*pi*viscosity*rndst(i,k,ind_aux))
                     !account for freezing efficiency
                     ndfaer(ind_aux)=ndfaer(ind_aux)*(1.0-exp(-nsdust*12.5664*rndst(i,k,ind_aux)*rndst(i,k,ind_aux)))
                  end do

                  nslipsoot=1.0_r8+(mfp/rnsoot(i,k))*(1.257_r8+(0.4_r8*Exp(-(1.1_r8*rnsoot(i,k)/mfp))))
                  ndfaersoot=1.381e-23_r8*t(i,k)*nslipsoot/(6._r8*pi*viscosity*rnsoot(i,k))
                  ndfaersoot=ndfaersoot*(1.0-exp(-nssoot*12.5664*rnsoot(i,k)*rnsoot(i,k)))



                  mnucct(k) = cons10/(cons3*qcvar)*  &
                        (sum(ndfaer(1:nbincontactdust)*nacon(i,k,1:nbincontactdust))+ndfaersoot*nsoot(i,k)) &
                        *pi*pi/3._r8*rhow*cdist1(k)*gamma(pgam(k)+5._r8)/lamc(k)**4 !DONIF 


                  ! mnucct(k) = cons10/(cons3*qcvar)*  &
                  !             (ndfaer1*(nacon(i,k,1))+ndfaer2*(nacon(i,k,2))+ndfaer3*(nacon(i,k,3))+&             
                  !          ndfaer4*(nacon(i,k,4))+ndfaer5*(nsoot(i,k))) &
                  !          *pi*pi/3._r8*rhow*cdist1(k)*gamma(pgam(k)+5._r8)/lamc(k)**4 !DONIF 

                  nnucct(k) = (sum(ndfaer(1:nbincontactdust)*nacon(i,k,1:nbincontactdust))+ndfaersoot*nsoot(i,k))*2._r8*pi*  &
                        cdist1(k)*gamma(pgam(k)+2._r8)/lamc(k)

                  !nnucct(k) = (ndfaer1*(nacon(i,k,1))+ndfaer2*(nacon(i,k,2))+ndfaer3*(nacon(i,k,3)) + & 
                  !             ndfaer4*(nacon(i,k,4)) + ndfaer5*(nsoot(i,k)))*2._r8*pi*  &
                  !             cdist1(k)*gamma(pgam(k)+2._r8)/lamc(k)
                  ...
                  ! contact cannot be greater than available droplet number 
                  if ((nnucct(k) + nnuccc(k))*deltat .gt. ncic(i, k)) then 
                     !all droplets freeze by immersion !DONIF
               ...
               if (qcic(i,k)+qiic(i,k).lt.1.e-6_r8.or.cldmax(i,k).gt.lcldm(i,k)) then
                  ...
                  qclr=(q(i,k)-dum*qsn)/(1._r8-dum) !What if cldfrc=1? BUG DONIFF
               ...
               qce=max(qc(i,k) - berg(i,k)*deltat, 0.0) !DONIF bug corrected*************************
               ...
               if (dum.gt.qie) then
                  if (prci(k)+prai(k) .gt. 0.0) then   !!!!!!!!!!!DONIF BUg REMOVED!!!!!
               ...
               if (dum.gt.nie) then
                  if ( abs(nprci(k)+nprai(k)-nsubi(k)) .gt. 0.0) then    
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
               ...
               evapsnow(i,k) = evapsnow(i,k)+prds(k)*cldmax(i,k) !!!!remove DONIFF
               ...
               mnuccdo(i,k)=mnuccdo(i,k)+mnuccd(k)  !DONIF
               nnuccto(i,k)=nnuccto(i,k)+ nnucct(k)*lcldm(i,k)!DONIF

               nnuccdo(i,k)=nnuccdo(i,k)+ nnuccd(k)!DONIF   
               nnuccco(i,k)=nnuccco(i,k)+ nnuccc(k)*lcldm(i,k)!DONIF
               nsacwio(i,k)=nsacwio(i,k)+ nsacwi(k)*icldm(i,k)!DONIF
               nsubio(i,k)=nsubio(i,k)+ nsubi(k)*icldm(i,k)!DONIF
               nprcio(i,k)=nprcio(i,k)+ nprci(k)*icldm(i,k)!DONIF
               npraio(i,k)=npraio(i,k)+ nprai(k)*icldm(i,k)!DONIF

               npccno(i, k)= npccno(i,k)+ npccn(k)*lcldm(i,k)!DONIF
               npsacwso(i,k)=npsacwso(i,k)+ npsacws(k)*lcldm(i,k)!DONIF
               nsubco(i,k)=nsubco(i,k)+ nsubc(k)*lcldm(i,k)!DONIF
               nprao(i,k)=nprao(i,k)+ npra(k)*lcldm(i,k)!DONIF
               nprc1o(i,k)=nprc1o(i,k)+ nprc1(k)*lcldm(i,k)!DONIF   
               ...
               nitend(i,k) = nitend(i,k)+ nnuccd(k)*mtime+ & 
                     (nnuccc(k)+nnucct(k)+nsacwi(k))*lcldm(i,k)+(nsubi(k)-nprci(k)- &   
                     nprai(k))*icldm(i,k)    !DONIFF included nnuccc since immersion freezing!!!!!
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                  if (t(i,k)+tlat(i,k)/cp*deltat > 275.15_r8) then
                     ...
                     if (qstot > 0._r8) then
                        ...
                        if (t(i,k)+(tlat(i,k)/cp+dum)*deltat .lt. 273.15_r8) then      !DONIFF corrected   

                           !dum = (t(i,k)+tlat(i,k)/cp*deltat-275.15_r8)*cp/xlf          
                           dum = (t(i,k)+tlat(i,k)/cp*deltat-275.15_r8)/deltat  !DONIFF corrected          
                           dum = dum/(xlf/cp*qstot/(dz(i,k)*rho(i,k))) !Bug corrected.  There was a problem with the units here!!!!!!!          
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                  if (t(i,k)+tlat(i,k)/cp*deltat < (tmelt - 5._r8)) then
                     ...
                     if (qrtot > 0._r8) then
                        ...
                        if (t(i,k)+tlat(i,k)/cp*deltat+dum*deltat .gt.(tmelt - 5._r8)) then   !DONIFF corrected
                           !dum = -(t(i,k)+tlat(i,k)/cp*deltat-(tmelt-5._r8))*cp/xlf
                           dum = -(t(i,k)+tlat(i,k)/cp*deltat-(tmelt-5._r8))/deltat  !DONIFF corrected
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
               ...
               if (lamc(k).gt.0._r8) then
                  Actmp = cdist1(k) * pi * gamma(pgam(k)+3._r8)/(4._r8 * lamc(k)**2._r8) !! bug 02/19/2015
               else 
                  Actmp = 0._r8
               endif
...
300      continue  ! continue if no cloud water
      ...
      do i=1,ncol
         ...
         do k=1,pver
            ...
            tlataux(i,k)=tlat1_aux(i,k)/real(iter)
            ...
            mnuccdo(i, k) =mnuccdo(i,k)/real(iter) !DONIF
            mnuccto(i, k) =mnuccto(i,k)/real(iter) !DONIF
         ...
         do k=1,pver   
            ...
            if (t(i,k)+tlat(i,k)/cp*deltat > tmelt) then
               if (dumi(i,k) > 0._r8) then
                  ...
                  dum = -dumi(i,k)*xlf/cp
                  if (t(i,k)+tlat(i,k)/cp*deltat+dum*deltat .lt.tmelt) then 
            ...
            if (t(i,k)+tlat(i,k)/cp*deltat < 233.15_r8) then 
               ...
               if (dumc(i,k) > 0._r8) then
                  ...
                  dum = dumc(i,k)*xlf/cp
                  if (t(i,k)+tlat(i,k)/cp*deltat+dum*deltat .gt.233.15_r8) then 
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
            !if (.false.) then !DONIFF  we do this later, do not create new cloud 
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!1
            ...
            if (dumi(i,k).ge.qsmall) then
               ...
               ! effi(i,k) = 1.5_r8/lami(k)*1.e6_r8 !DONIF


               effi(i,k) = 1.e6_r8*gamma(4._r8+miu_ice)/(2.0*lami(k)*gamma(3._r8+miu_ice))  !DONIF
...
500      continue
      ...
      do i = 1,ncol
         do k=1,pver
            if ((qc(i,k)+qctend(i,k)*deltat.ge.qsmall) .and. & 
                  (cldmax(i, k) .gt. 0.0) .and. (lcldm(i, k) .gt.0.0) .and. & 
                  (nc(i,k)+nctend(i,k)*deltat .gt. 0.0)) then  !DONIF
...
10             continue
         ...
         FUNCTION gamma_incomp(muice, x)
            ...
            !Crude (although likely good enough) approximation to the incomplete gamma based on logistic functions
            ! Developed by Donifan Barahona
