! $Id: GEOS_MoistGridComp.F90,v 1.134.50.3.2.1.6.2.4.3.28.1.2.1.2.1.10.2.4.1.6.7 2020/09/08 18:05:56 bzhao Exp $
...
module GEOS_MoistGridCompMod
  ...
  use CLOUDNEW, only: &
       ...
       ! Outputs
       RAD_CLDFRC_DEV, RAD_QL_DEV, RAD_QI_DEV, RAD_QR_DEV, RAD_QS_DEV, QPLS_DEV, &
...
!-srf-gf-scheme    
    call ESMF_ConfigGetAttribute( CF, CONVPAR_OPTION, Label='CONVPAR_OPTION:',   default="RAS",        RC=STATUS)
    VERIFY_(STATUS)
...
!-srf-gf-scheme    
    ...
    !--------------------------------------------------------------
    call ESMF_ConfigGetAttribute( CF, CLDMICRO, Label="CLDMICRO:",  default="2MOMENT", RC=STATUS)
    VERIFY_(STATUS)
...
!   call MAPL_AddImportSpec ( gc,                                  &
!         SHORT_NAME = 'T_N',                                       &
!         LONG_NAME  = 'air_temperature_at_begin_of_time_step',     &
!         UNITS      = 'K',                                         &
!         DIMS       = MAPL_DimsHorzVert,                           &
!         VLOCATION  = MAPL_VLocationCenter,             RC=STATUS  )
!   VERIFY_(STATUS)
...
!   call MAPL_AddImportSpec ( gc,                                 &
!         SHORT_NAME = 'PLE_N',                                     &
!         LONG_NAME  = 'edge_pressureat_begin_of_time_step',       &
!         UNITS      = 'Pa',                                        &
!        DIMS        = MAPL_DimsHorzVert,                           &
...
!   call MAPL_AddImportSpec ( gc,                                 &
!         SHORT_NAME = 'U_N',                                       &
!         LONG_NAME  = 'eastward_wind_at_begin_of_time_step',       &
!         UNITS      = 'm s-1',                                     &
!         DIMS       = MAPL_DimsHorzVert,                           &
...
!   call MAPL_AddImportSpec ( gc,                                  &
!         SHORT_NAME = 'V_N',                                       &
!         LONG_NAME  = 'northward_wind_at_begin_of_time_step',      &
!         UNITS      = 'm s-1',                                     &
!         DIMS       = MAPL_DimsHorzVert,                           &
!         FIELD_TYPE = MAPL_VectorField,                            &
!         VLOCATION  = MAPL_VLocationCenter,             RC=STATUS  )
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  subroutine Initialize ( GC, IMPORT, EXPORT, CLOCK, RC )
    ...
    logical                             :: LCLDMICRO
    real DCS, QCVAR_
    real(ESMF_KIND_R8)  Dcsr8, qcvarr8
    !=============================================================================
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    subroutine MOIST_DRIVER(IM,JM,LM, RC)
      ...
      real, pointer, dimension(:,:,:) :: DQDT, UI, VI, TI, KH, TKE
      ...
      real, pointer, dimension(:,:  ) :: RASPBLQ, MXDIAM, RH600, Q600, QRATIO, CNV_FRC
...
!!$      real, pointer, dimension(:,:,:) :: CURAINMOVE, CUSNOWMOVE
      ...
      real, pointer, dimension(:,:,:) :: QSNOW, QRAIN, QPLS
...
!-srf-gf-scheme    
      ...
      real, pointer, dimension(:,:,:)       :: DYNF_Q
      real, pointer, dimension(:,:,:)       :: DYNF_T  
      real, pointer, dimension(:,:,:)       :: DYNF_PLE
      real, pointer, dimension(:,:,:)       :: DYNF_UA
      real, pointer, dimension(:,:,:)       :: DYNF_VA
      real, pointer, dimension(:,:,:)       :: Q_N
!     real, pointer, dimension(:,:,:)       :: T_N
!     real, pointer, dimension(:,:,:)       :: PLE_N
!     real, pointer, dimension(:,:,:)       :: U_N
!     real, pointer, dimension(:,:,:)       :: V_N
      real, pointer, dimension(:,:,:)       :: DTDT_BL
      real, pointer, dimension(:,:,:)       :: DQDT_BL
      real, pointer, dimension(:,:,:)       :: DQDT_GF,DTDT_GF,MUPDP,MUPSH,MUPMD
...
!-srf-gf-scheme    
      ...
      real, pointer, dimension(:,:,:) :: SMAXL, WSUB, CCN01, CCN04, CCN1, SMAXI, & !DONIF
           ...
           NRAIN, NSNOW, DQVDT_micro, DQIDT_micro, DQLDT_micro, &
           DTDT_micro, RL_MASK, RI_MASK, KAPPA, RHICE, RHLIQ, SC_ICE, &
           NHET_IMM, NHET_DEP,  &
      ...
      real, parameter :: pmin_trop = 50.0 !mbar minimum pressure to do cloud microphysics
...
!!! mmicro_pcond real*8 scalars and arrays 
      real(ESMF_KIND_R8), dimension(1,LM)  ::                                                     &  
           ter8, ttendr8, qvr8, qtendr8, cwtendr8, qcr8, qir8, &
           ncr8, nir8, plevr8, pdelr8, cldfr8 ,liqcldfr8, icecldfr8, &
           cldor8,  rpdelr8, zmr8, omegr8,                    &
           rhdfdar8, rhu00r8, ficer8 ,tker8, turbtypr8, smawr8, &
           nimmr8, nsootr8, rnsootr8, ndropr8


      real(ESMF_KIND_R8), dimension(1,1:LM)        ::    & 
           rate1ord_cw2pr,                               &
           naair8, npccninr8,           &
           !                          ** outputs **
           tlatr8, qvlatr8,        &
           ...
           lamcradr8,qsoutr8,dsoutr8, qroutr8,droutr8,&
           qcsevapr8,qisevapr8,   &
           qvresr8,cmeioutr8, &
           vtrmcr8,vtrmir8,   &
           ...
           praior8,qiresor8,  &
           mnuccror8,pracsor8, &
           meltsdtr8,frzrdtr8, ncalr8, ncair8, nsoutr8, nroutr8, &
      ...
      real(ESMF_KIND_R8), dimension(1,LM+1)  :: pintr8, kkvhr8, rflx, sflx !precip and snow fluxes
      ...
      integer                                 :: n_modes, kbmin, kcldtop, kcldbot , NAUX, kcldtopcvn, nbincontactdust    
...
!!! MODIFIED : remove when done testing shallow
      ...
      real                            :: CBL_TPERTi, CBL_TPERT, CBL_QPERT, RASAL1, RASAL2, QUANX_0, QUANX_1
      ...
      real,    dimension(IM,JM,  LM)  :: DQS, QSS, PLO, ZLO, TEMP, PK, DM, DP, DQSDT, DBZ3D
      ...
      real,    dimension(IM,JM)       :: TPERT, QPERT, TSFCAIR, DTS, RASAL2_2d, QUANX, QUANXTP, NPRE_FRAC_2d
      ...
      real, dimension(IM,JM) ::  CLDREFFI_TOP_X, CLDREFFL_TOP_X,  NCPL_TOP_X, NCPI_TOP_X, &
                                 NCPL_CLDBASEX
...
!!$      real, dimension(IM,JM,LM) :: CUSNOWMOVE_X 
      ...
      real   , dimension(IM,JM)           :: RHat600, QV600
...
! gf convective param
    call MAPL_GetResource(STATE, CONVPAR_OPTION, Label='CONVPAR_OPTION:',   default="RAS",    RC=STATUS)
    VERIFY_(STATUS)
...
!
      ...
      call MAPL_GetResource(STATE, DEPSCALE,        'DEPSCALE:',      DEFAULT= 1.0,    RC=STATUS) !Scaling factor for ice deposition time scale
      ...
      call MAPL_GetResource(STATE, LTS_UP,         'LTS_UP:',         DEFAULT= 25.0,   RC=STATUS) !Upper LTS for morphology correction
      call MAPL_GetResource(STATE, MIN_EXP,        'MIN_EXP:',        DEFAULT= 0.5,   RC=STATUS) !Exponent of the relation CFA=CFV^n
       call MAPL_GetResource(STATE, MAX_EXP,       'MAX_EXP:',       DEFAULT= 1.1,   RC=STATUS) !Exponent of the relation CFA=CFV^n
      call MAPL_GetResource(STATE, DCRIT_,         'DCRIT_DRIZZLE:',  DEFAULT= 0.75,    RC=STATUS) !scale factor for critical size for drizzle
      call MAPL_GetResource(STATE, USE_AV_V,       'USE_AV_V:',       DEFAULT= 0.0,    RC=STATUS) !Set to > 0 to use an average velocity for activation
      call MAPL_GetResource(STATE, TS_AUTO_ICE,    'TS_AUTO_ICE:',    DEFAULT= 3.0, RC=STATUS) !Ice autoconversion time scale
      ...
      call MAPL_GetResource(STATE, MIN_ALH,        'MIN_ALH:',        DEFAULT= 300.0,  RC=STATUS) !minimum mixing lenght to define vertical vel 	
      call MAPL_GetResource(STATE, SCWST,          'SCWST:',      DEFAULT= 5.0,  RC=STATUS) !scale factor for vertical velocity in sttratocumulus
      call MAPL_GetResource(STATE, MAX_CAPE,       'MAX_CAPE:',        DEFAULT= 1000.0,   RC=STATUS) !Defines convective fraction
      call MAPL_GetResource(STATE, MIN_CAPE,       'MIN_CAPE:',        DEFAULT= 500.0,   RC=STATUS) !Defines convective fraction
      call MAPL_GetResource(STATE, TMAXBASELQ,     'TMAXBASELQ:',      DEFAULT= 265.0,  RC=STATUS) !scale factor for vertical velocity in sttratocumulus
     
      call MAPL_GetResource(STATE, MINCDNC,          'MINCDNC:',      DEFAULT= 25.0,  RC=STATUS) !min nucleated droplet conc. cm-3
      call MAPL_GetResource(STATE, RASPARAMS%CCNFRAC, 'CCNFRAC_RAS:',  DEFAULT= 1.0,   RC=STATUS) !how much CCN make it into the convective core
     
     
      ! USe to disable a specific INP type
      call MAPL_GetResource(STATE, DUST_INFAC,     'DUST_INFAC:',        DEFAULT= 1.0,   RC=STATUS) 
      ...
      call MAPL_GetResource(STATE, SWCIRRUS, 'SWCIRRUS:', DEFAULT= 2.0, RC=STATUS) !Tunes vertical velocity in cirrus
...
!!!!!!!!!!!!!!!!!
     ...
     !----------------------------------------------
      ...
      call MAPL_GetResource(STATE, DOSHLW,             'DOSHLW:'  ,DEFAULT=0,        RC=STATUS)
      call MAPL_GetResource(STATE, SHLWDIAG,           'SHLWDIAG:',DEFAULT=0,      RC=STATUS)

      if (DOSHLW /= 0) then
         call MAPL_GetResource(STATE, RASPARAMS%MIN_DIAMETER, 'MIN_DIAMETER:',  DEFAULT= 800.,    RC=STATUS)
      else
         call MAPL_GetResource(STATE, RASPARAMS%MIN_DIAMETER, 'MIN_DIAMETER:',  DEFAULT= 200.,    RC=STATUS)
      end if
      ...
      call MAPL_GetResource(STATE, SHLWPARAMS%RPEN,    'RPEN:'    ,DEFAULT=3.0, RC=STATUS) !3.0 was 1.0
      call MAPL_GetResource(STATE, SHLWPARAMS%RLE,     'RLE:'     ,DEFAULT=0.1, RC=STATUS)
      call MAPL_GetResource(STATE, SHLWPARAMS%RKM,     'RKM:'     ,DEFAULT=12.0, RC=STATUS) !12 was 14
      call MAPL_GetResource(STATE, SHLWPARAMS%RKFRE,   'RKFRE:'   ,DEFAULT=1.0, RC=STATUS) !1.0 was 0.4
      ...
      call MAPL_GetResource(STATE, SHLWPARAMS%FRC_RASN,'FRC_RASN:',DEFAULT=1.0, RC=STATUS) !0 to 1 
      call MAPL_GetResource(STATE, SHLWPARAMS%KEVP,    'KEVP:'    ,DEFAULT=2.e-6,    RC=STATUS)
      call MAPL_GetResource(STATE, SHLWPARAMS%RDROP,   'SHLW_RDROP:',DEFAULT=11.e-6,    RC=STATUS)
...
!!$      call MAPL_GetPointer(EXPORT, PFICNMOVE, 'PFICNMOVE',  RC=STATUS); VERIFY_(STATUS)
      ...
      call MAPL_GetPointer(EXPORT, NRAIN, 'NRAIN'      , RC=STATUS); VERIFY_(STATUS)
      call MAPL_GetPointer(EXPORT, NSNOW, 'NSNOW'      , RC=STATUS); VERIFY_(STATUS)
      ...
      if(adjustl(CLDMICRO)=="2MOMENT") then
         ...
         ALLOC_NRAIN    = .not.associated(NRAIN   )
         ALLOC_NSNOW    = .not.associated(NSNOW   )
         ...
         if(ALLOC_NRAIN      ) allocate(NRAIN      (IM,JM,LM))
         if(ALLOC_NSNOW      ) allocate(NSNOW      (IM,JM,LM))       
...
!--srf-gf-scheme
      ...
      DO J=1, JM
         DO I=1, IM
            QUANX(I,J)   = SUM(  KH(I,J,0:lm-1)*( ZLE(I,J,0:lm-1) - ZLE(I,J,1:LM) ) )
            QUANX(I,J)   = SQRT( MAX( QUANX(I,J) , 0.0 ) )
            QUANXTP(I,J) = QUANX_0
            QUANX(I,J)   = MIN( QUANX(I,J) , QUANX_1 )
            QUANX(I,J)   = MAX( QUANX(I,J) , QUANX_0 )
         END DO
      END DO
      ...
      DO J=1, JM
         DO I=1, IM
            QUANX(I,J)   = MAX( QUANX(I,J) , QUANXTP(I,J) )
         END DO
      END DO
    ...
    ! call MAPL_GetResource(STATE,CNV_FRACTION_MIN, 'CNV_FRACTION_MIN:', DEFAULT= 0.00600, RC=STATUS)
    ! VERIFY_(STATUS)
    ! call MAPL_GetResource(STATE,CNV_FRACTION_MAX, 'CNV_FRACTION_MAX:', DEFAULT= 0.00700, RC=STATUS)
    ! VERIFY_(STATUS)
    ! CAPE Criteria
      ...
      if( CNV_FRACTION_MAX > CNV_FRACTION_MIN ) then
         if (CNV_FRACTION_MAX < 1.0) then ! QV at 600mb
           DO J=1, JM
             DO I=1, IM
...
! find the minimun level for cloud micro calculations
	 call find_l(KMIN_TROP, PLO, pmin_trop, IM, JM, LM, 10, LM-2) 
      ...
      ! via Deep, slow - RASAL2, and Shallow, fast - RASAL1 timescales (jtb 4/25/07).
...
!-srf - placed here before the cum parameterizations are called.
...
!-srf-gf-scheme
	 ...
	 if(DOSHLW==1) icumulus_gf(shal)=0
         !
         ...
         call MAPL_GetPointer(IMPORT, DYNF_Q   ,'DYNF_Q'  ,RC=STATUS); VERIFY_(STATUS)
         call MAPL_GetPointer(IMPORT, DYNF_T   ,'DYNF_T'  ,RC=STATUS); VERIFY_(STATUS)
         call MAPL_GetPointer(IMPORT, DYNF_PLE ,'DYNF_PLE',RC=STATUS); VERIFY_(STATUS)
         call MAPL_GetPointer(IMPORT, DYNF_UA  ,'DYNF_UA' ,RC=STATUS); VERIFY_(STATUS)
         call MAPL_GetPointer(IMPORT, DYNF_VA  ,'DYNF_VA' ,RC=STATUS); VERIFY_(STATUS)
         call MAPL_GetPointer(IMPORT, Q_N      ,'Q_N'     ,RC=STATUS); VERIFY_(STATUS)
         call MAPL_GetPointer(EXPORT, DTDT_BL  ,'DTDT_BL' ,RC=STATUS); VERIFY_(STATUS)
         call MAPL_GetPointer(EXPORT, DQDT_BL  ,'DQDT_BL' ,RC=STATUS); VERIFY_(STATUS)
       ...
       !    CNV_NDROP=  0.0
       !  !else   
           !STOP "GF scheme works only with single-moment MP" 
        ! endif
...
!      if(associated(TVQX1))  TVQX1     = SUM( (  Q1 +  QLLS + QLCN + QILS + QICN + CNV_PRC3)*DM + CNV_DQLDT*DT_MOIST , 3 )
      ...
      call MAPL_GetResource( STATE, CLDPARAMS%AUTOC_LS,       'AUTOC_LS:',       DEFAULT= 2.0e-3  )
      ...
      call MAPL_GetResource( STATE, CLDPARAMS%CCW_EVAP_EFF,   'CCW_EVAP_EFF:',   DEFAULT= 3.3e-4  )
      call MAPL_GetResource( STATE, CLDPARAMS%NSUB_AUTOCONV,  'NSUB_AUTOCONV:',  DEFAULT= 20.     )
      call MAPL_GetResource( STATE, CLDPARAMS%LS_SUND_INTER,  'LS_SUND_INTER:',  DEFAULT= 4.8     )
      call MAPL_GetResource( STATE, CLDPARAMS%LS_SUND_COLD,   'LS_SUND_COLD:',   DEFAULT= 4.8     )
      ...
      call MAPL_GetResource( STATE, CLDPARAMS%LS_ICEFALL,     'LS_ICEFALL:',     DEFAULT= 0.5     )
      call MAPL_GetResource( STATE, CLDPARAMS%REVAP_OFF_P,    'REVAP_OFF_P:',    DEFAULT= 2000.   )
!     call MAPL_GetResource( STATE, CLDPARAMS%CNV_ENVF,       'CNV_ENVF:',       DEFAULT= 0.8     )
      call MAPL_GetResource( STATE, CLDPARAMS%WRHODEP,        'WRHODEP:',        DEFAULT= 0.5     )
      call MAPL_GetResource( STATE, CLDPARAMS%ICE_RAMP,       'ICE_RAMP:',       DEFAULT= -40.0   )
      ...
      call MAPL_GetResource( STATE, CLDPARAMS%SCLM_SHW,       'SCLM_SHW:',       DEFAULT= 2.0   )
	 ...
	 call MAPL_GetResource( STATE, CLDPARAMS%TURNRHCRIT,     'TURNRHCRIT:',     DEFAULT= 884.0 )
         ...
         call MAPL_GetResource( STATE, CLDPARAMS%MAXRHCRITLAND,  'MAXRHCRITLAND:',  DEFAULT= CLDPARAMS%MINRHCRIT+0.01 )
	 ...
	 call MAPL_GetResource( STATE, CLDPARAMS%TURNRHCRIT,     'TURNRHCRIT:',     DEFAULT= 750.0 )
      ...
      call MAPL_GetResource( STATE, CLDPARAMS%FR_AN_WAT, 'FR_AN_WAT:',  DEFAULT= 0.0    )
      ...
      call MAPL_GetResource( STATE, CLDPARAMS%CFPBL_EXP,      'CFPBL_EXP:',      DEFAULT= 3 )
    ...
    ! call MAPL_GetResource( STATE, CLOUD_CTL%SCLMFDFR,    'SCLMFDFR:',      DEFAULT= 1.00  )
      ...
      if(adjustl(CLDMICRO) /="2MOMENT") then
         ...
         COPY_RSU_SC = ASSOCIATED(RSU_SC) .OR. ASSOCIATED(PREVTOT)    .OR. ASSOCIATED(SUBPZ)

         COPY_REV_CN = ASSOCIATED(REV_CN) .OR. ASSOCIATED(REVSU_CN)   .OR. ASSOCIATED(PREVTOT) .OR. ASSOCIATED(EVPPZ)
         COPY_REV_SC = ASSOCIATED(REV_SC) .OR. ASSOCIATED(PREVTOT)    .OR. ASSOCIATED(EVPPZ)
...
!!$         COPY_CUSNOWMOVE = ASSOCIATED(CUSNOWMOVE)
         ...
         VERIFY_(STATUS)
         ALLOCATE(SC_RMFDTR_DEV(IM*JM,LM), STAT=STATUS)
         VERIFY_(STATUS)
         ALLOCATE(QLWDTR_DEV(IM*JM,LM), STAT=STATUS)
         VERIFY_(STATUS)
         ALLOCATE(SC_QLWDTR_DEV(IM*JM,LM), STAT=STATUS)
         VERIFY_(STATUS)
         ALLOCATE(SC_QIWDTR_DEV(IM*JM,LM), STAT=STATUS)
         VERIFY_(STATUS)
...
!!$         ALLOCATE(CUSNOWMOVE_DEV(IM*JM,LM), STAT=STATUS)
         ...
         STATUS = cudaMemcpy(SC_RMFDTR_DEV,MFD_SC,IM*JM*LM) 
         STATUS = cudaMemcpy(QLWDTR_DEV,CNV_DQLDT,IM*JM*LM) 
         STATUS = cudaMemcpy(SC_QLWDTR_DEV,QLDET_SC,IM*JM*LM) 
         STATUS = cudaMemcpy(SC_QIWDTR_DEV,QIDET_SC,IM*JM*LM) 
         ...
         call PROGNO_CLOUD<<<Grid, Block>>>(IM*JM,LM,DT_MOIST,CLOUD_CTL%SCLMFDFR,DOSHLW,DOING_GF)
         ...
         STATUS = cudaMemcpy(UFRC_SC,SC_UPDFRC_DEV,IM*JM*LM) 
...
!!$         IF (COPY_CUSNOWMOVE)  STATUS = cudaMemcpy(CUSNOWMOVE_X,CUSNOWMOVE_DEV,SIZE(CUSNOWMOVE_X))
         ...
         DEALLOCATE(SC_RMFDTR_DEV)
         DEALLOCATE(QLWDTR_DEV)
         DEALLOCATE(SC_QLWDTR_DEV)
         DEALLOCATE(SC_QIWDTR_DEV)
         DEALLOCATE(U_DEV)
         DEALLOCATE(V_DEV)
         DEALLOCATE(QST3_DEV)
         DEALLOCATE(DZET_DEV)
         ...
         DEALLOCATE(QRN_SC_DEV)
         DEALLOCATE(QSN_SC_DEV)
...
!!$              LIQANMOVE_X, ICEANMOVE_X, DANCLD_X, DLSCLD_X, &
!!$              CURAINMOVE_X, CUSNOWMOVE_X,  &
                                ! End diagnostics
              TEMPOR2D, &
	      DOSHLW,   &
!-srf-gf-scheme	      
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
         ...
         where (CFICE .lt. 0.001)
            CFICE=0.0
         end where
         where (CFLIQ .lt. 0.001)
            CFLIQ=0.0
         end where
...
!	  
   ...
   !                  end if 
                  ...
                  ! end where 
                   ...
                   call   gw_prof (1, LM, 1, tm_gw, pm_gw, pi_gw, &
                    ...
                    kbmin= max(min(KLCL(I, J), LM-1)-2, 10)
 ...
 !               do K = KMIN_TROP(I, J), LM-1 !limit to troposphere and no activation at the surface
               ...
               do K = 30, LM-1 !limit to troposphere and no activation at the surface
		       ...
		       call vertical_vel_variance(omegr8(1, K), lc_turb(1, K), ter8(1, K), plevr8(1, K), rad_cooling(1,K),  uwind_gw(1,K), &
                  ...
                  if ((FRLAND(I, J) .lt. 0.1)  .and. (LTS(I, J) .gt. CLDPARAMS%MIN_LTS-1.0)) then                   
                       if (K .ge. kbmin-2) wparc_ls(1, K)=max(wparc_ls(1,K)+ zws(i, j), 0.00)*SCWST  
                   end if 
 
               
                aux1=  min(ZPBL(I, J)/1000.0, 5.0)*(1.0+0.25*uwind_gw(1,K))*(ter8(1, LM)-max(ter8(1, K), TMAXBASELQ-5.0))
                aux1   = (0.20*sqrt(max(aux1, 0.0)) + 0.26)**2.0
                
!                if ((FRLAND(I, J) .lt. 0.1) .and. (LTS(I, J) .gt. CLDPARAMS%MIN_LTS)) then 
!                  aux1 = aux1*SCWST
 !               end if 
                 
                  aux2  =  max(min((ter8(1, K) - TMAXBASELQ)/5.0, 1.0), 0.0)                
                  wparc_turb(1, K) =  aux1*aux2  + wparc_turb(1, K)*(1.0-aux2)
         ...
         !             end if 
		     ...
		     !!Subroutine aerosol_activate contains the CCN activation and ice nucleation parameterizations. Lives in aer_cloud.F90.
                     ...
                     call   aerosol_activate(tauxr8, plevr8(1, K), swparc(1, K), wparc_ls(1, K),  AeroAux, &
			              ...
			              fdust_drop, fsoot_drop, pfrz_inc_r8(1, K), rh1_r8, frachet_dust, frachet_bc, frachet_org, frachet_ss)

                      CCN01(I, J, K) = max(ccn_diag(1), 0.0)
                      CCN04(I, J, K) = max(ccn_diag(2), 0.0)
                      CCN1 (I, J, K) = max(ccn_diag(3), 0.0)
                      npccninr8(1, K) =max(npccninr8(1, K), MINCDNC*1.e6)
  ...
  call  macro_cloud (                    &
      ...
      if(associated(TVQX1)) TVQX1    = SUM( ( Q1 +  QLLS + QLCN + QILS + QICN )*DM , 3 )  + TPREC*DT_MOIST
	 ...
	 CFLIQ=0.0
         ...
         rhdfdar8   = 1.e-8_r8
         rhu00r8    = 0.95_r8
         ncnstr8 = 1.0 !initialize constant number conc
         ninstr8 = 1.0
         ttendr8=0._r8
         qtendr8=0._r8
         cwtendr8=0._r8
	 ...
	 nprc1or8 =0.0_r8
         ...
         do J=1,JM
            do I=1,IM
               ...
               !kbmin = KMIN_TROP(I, J)    !try to trick MG into doing less
	           kbmin =10 	   
               npccninr8  = 0.0
               naair8     = 0.0
               omegr8     = 0.0
...
!               kbmin = KCBL(I, J)
			...
			!!!!!!!!
               ...
               if ((FRLAND(I, J) .lt. 0.1) .and. (LTS(I, J) .gt. CLDPARAMS%MIN_LTS-1.0)) then 
                    dcrit = 0.25*DCRIT_ !reduce further autoconversion in stratus regions               
               end if 
...
!!!Call to MG microphysics. Lives in cldwat2m_micro.F90
               call mmicro_pcond (                           &
                    ...
                    nsoutr8, nroutr8, ncnstr8,  ninstr8, nimmr8, disp_liu, &
               ...
               TEMP(I,J,1:LM) = MAX(TEMP(I,J,1:LM)   + REAL(tlatr8(1,1:LM)) * DT_R8 / (MAPL_CP), 0.0)  
              ...
              PFL_LS_X(I, J, 1:LM) = rflx(1, 1:LM)*(1.0-FQA(I, J, 1:LM))
              PFL_AN_X(I, J, 1:LM) = rflx(1, 1:LM)*FQA(I, J, 1:LM)
              PFI_LS_X(I, J, 1:LM) = sflx(1, 1:LM)*(1.0-FQA(I, J, 1:LM))
              PFI_AN_X(I, J, 1:LM) = sflx(1, 1:LM)*FQA(I, J, 1:LM)
                                        

               LS_ARFX(I,J)     = maxval( REAL(cldfr8(1,1:LM)) )
               AN_PRC2(I,J)     = 0. !prectr8(1)
               AN_SNR(I,J)      = 0. !precir8(1)
               AN_ARFX(I,J)     = 0. !maxval( cldfr8(1,1:LM) )    
               ...
               CLDREFFR(I,J,1:LM) = REAL(droutr8(1, 1:LM))/2.0        
	           CLDREFFS(I,J,1:LM) = REAL(dsoutr8(1, 1:LM))/2.0/scale_ri

               QRAIN(I,J,1:LM)  = max(REAL(qroutr8(1, 1:LM)), 0.0) ! grid average 
               ...
               SUBLC_X(I,J,1:LM) = REAL(cmeioutr8(1, 1:LM))                         ! Ice subl already grid -ave
               ...
               EVAPC_X(I,J,1:LM) = REAL(qcsevapr8(1,1:LM) )                ! cloud evap
...
do K= 1, LM
 ...
 !if(associated(TVQX2)) TVQX2    = SUM( ( Q1 +  QLLS + QLCN + QILS + QICN )*DM , 3 )  + TPREC*DT_MOIST
...
!----------Other diagnostics
     ...
     if (associated(SCF_ALL)) then 
      ...
      if (associated(DBZ) .OR. associated(DBZ_MAX)) then
        !call CALCDBZ(DBZ3D,PLO,TEMP,Q1,QRAIN+QLCN+QLLS,QSNOW+QICN+QILS,QSNOW*0,IM,JM,LM,1,0,0)
         if(adjustl(CLDMICRO)=="2MOMENT") then
           call CALCDBZ(DBZ3D,100*PLO,TEMP,Q1,QRAIN,QSNOW,QSNOW*0,IM,JM,LM,1,0,0)
         else
           call CALCDBZ(DBZ3D,100*PLO,TEMP,Q1,QRAIN*RAD_CF,QSNOW*RAD_CF,QSNOW*0,IM,JM,LM,1,0,0)
         endif
...
!!$      IF (ASSOCIATED(CUSNOWMOVE)) CUSNOWMOVE = CUSNOWMOVE_X
      ...
      if(adjustl(CLDMICRO)=="2MOMENT") then 
         call FILLQ2ZERO2( Q1, DM, FILLQ ) !Slightly different formulation
      else
...
!      if(associated(TVQX2)) TVQX2    = SUM( ( Q1 +  QLLS + QLCN + QILS + QICN )*DM , 3 )  + TPREC*DT_MOIST
      ...
      if (associated(RHICE    ))   then
         RHICE     =  Q1/GEOS_QsatICE (TH1*PK, PLO*100.0)
         where (TEMP>273.15)
            RHICE=0.0
         end where
      end if
      if (associated(RHLIQ    ))   RHLIQ     =  Q1/GEOS_QsatLQU (TH1*PK, PLO*100.0)  !DONIF

      if(adjustl(CLDMICRO)=="2MOMENT") then 
         if (associated(CCNCOLUMN    ))   CCNCOLUMN      = SUM(CCN1*DM/(100.*PLO*r_air/TEMP) , 3)
         if (associated(NDCOLUMN    ))    NDCOLUMN      =  SUM(NCPL_VOL*DM/(100.*PLO*r_air/TEMP) , 3)
         if (associated(NCCOLUMN    ))    NCCOLUMN      =SUM(NCPI_VOL*DM/(100.*PLO*r_air/TEMP) , 3)
...
! --------------------------------------------------------------------------------------------
    ...
    if(associated(DNCDT   ))   then 
      ...
      if (associated(EVPPZ      ))   EVPPZ     =  SUM( ( REV_CN_X + REV_AN_X + REV_LS_X + REV_SC_X ) * DM , 3 )

      if (associated(SUBCZ      ))   SUBCZ     =  SUM( ( SUBLC_X  + DIFIX_X ) * DM , 3 )
      if (associated(SUBPZ      ))   SUBPZ     =  SUM( ( RSU_CN_X + RSU_AN_X + RSU_LS_X + RSU_SC_X ) * DM , 3 )

      if (associated(FRZCZ      ))   FRZCZ     =  SUM( FRZ_TT_X * DM , 3 )
      if (associated(FRZPZ      ))   FRZPZ     =  SUM( FRZ_PP_X * DM , 3 )
      if (associated(COLLIZ     ))   COLLIZ    =  SUM( ( ACIL_CN_X + ACIL_AN_X + ACIL_LS_X + ACIL_SC_X ) * DM , 3 )

      if (associated(COLLLZ     ))   COLLLZ    =  SUM( ( ACLL_CN_X + ACLL_AN_X + ACLL_LS_X + ACLL_SC_X ) * DM , 3 )
      ...
      if(adjustl(CLDMICRO)=="2MOMENT") then
         ...
         if(ALLOC_NRAIN  )  deallocate(NRAIN  )
         if(ALLOC_NSNOW  )  deallocate(NSNOW  )
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  subroutine FILLQ2ZERO2( Q, DM, FILLQ  )
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
