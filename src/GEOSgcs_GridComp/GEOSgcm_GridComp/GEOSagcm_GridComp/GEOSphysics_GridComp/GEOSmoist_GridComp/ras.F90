...
! $Id: ras.F90,v 1.29.20.1.28.2.2.1.44.2 2019/06/05 20:49:52 dbarahon Exp $
...
!!!!!!!!!!!!!!!!!!!!!
   ...
   ! note for sea salt we just assume that it is equal to the current droplet concentration and take the area from the calculation at cloud base
         ...
         if (TEMP .lt. 265.0)  ninbkg=coolr*42.8*exp(3.88*si)*0.01!tendency in IN from background IN. Derived from Phillips et al. 2007
         ...
         INsea= aseasalt*dnsss*coolr*0.0 
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!
   ...
   subroutine Qremoval(RATE_Q, FICE_, F_NL, F_NI, TE, PRE, QC, NI, &
      ...
      if ((qr .gt. 1.e-9) .and. (NL .gt. 1.0)) then
         ...
         fpr=COAUTO*prc/qr  !COAUTO is a tuning factor that accounts for the increase in rain by accretion 
