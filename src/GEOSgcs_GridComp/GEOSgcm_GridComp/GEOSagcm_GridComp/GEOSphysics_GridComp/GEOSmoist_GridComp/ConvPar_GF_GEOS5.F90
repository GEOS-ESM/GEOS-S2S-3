...
!.. USE GTMP_2_GFCONVPAR, only : GTMP_2_GFCONVPAR_interface
 ...
 PUBLIC  GF_GEOS5_INTERFACE, maxiens, icumulus_gf, closure_choice, deep, shal, mid &
        ,DEBUG_GF,USE_SCALE_DEP,DICYCLE
...
!-- max cloud fraction allowed: deep, shallow, congestus
 ...
 LOGICAL, PARAMETER :: MASSCON        = .TRUE.  !- mass conserv formulation flag
 ...
 LOGICAL, PARAMETER :: MOMENTUM       = .TRUE.  !- turn ON/OFF conv transp of momemtum
...
!---------------------------------------------------------------------------------------------------
  ...
  SUBROUTINE GF_GEOS5_INTERFACE(mxp,myp,mzp,LONS,LATS,dt_moist                    &
                               ...
                               ,DYNF_Q  ,DYNF_T ,DYNF_PLE ,DYNF_UA ,DYNF_VA       &
                               ,Q_N                                               &
    ...
    REAL   ,DIMENSION(mxp,myp,0:mzp) ,INTENT(IN)   :: PLE,ZLE,DYNF_PLE
    REAL   ,DIMENSION(mxp,myp,mzp)   ,INTENT(IN)   :: T ,U ,V ,ZLO ,PLO ,PK ,OMEGA         &
                                                     ,DYNF_Q ,DYNF_T ,DYNF_UA  ,DYNF_VA    &
                                                     ,RADSW  ,RADLW  ,DQDT_BL  ,DTDT_BL    &
                                                     ,Q_N ,DTDTDYN,DQVDTDYN   !,T_N ,PLE_N ,U_N ,V_N        
    ...
    !-for debug purposes
    REAL   ,DIMENSION(mxp,myp,mzp)   ,INTENT(INOUT)  :: DQDT_GF,DTDT_GF,MUPDP,MUPSH,MUPMD  
    ...
    INTEGER, DIMENSION(mxp,myp) :: kpbl
...
!---- for coupling with uphysics
    ...
    whoami=-999
    !call mpi_comm_rank(MPI_COMM_WORLD,whoami,status)
    if( whoami==WANTRANK)write(8,*)"in GF scheme interface" 
    !----
    ...
    !- time counter
    ntimes = ntimes + 1
    if(ntimes==1 .and. whoami == 0) print *,'==> Applying GF convection scheme '
    mynum = -999
    ...
    dx             = sum(dx2d)/max(1,size(dx2d))! meters
    !- special setting for SCM runs
    if(mxp==1 .and. myp==1) dx= 100000.
    ...
    IF(trim(env_setting)=='CURRENT') then 
     ...
     DO j=1,myp
      DO i=1,mxp
       DO k=1,mzp
         gsf_t (k,i,j) = 0. !DYNF_T (i,j,flip(k))
         gsf_q (k,i,j) = 0. !DYNF_Q (i,j,flip(k))
    ...
    ELSEIF(trim(env_setting)=='DEFAULT') then 
     ...
     !- "Q" just after dynamics is saved in the var named "Q_N" in "GEOS_AgcmGridComp.F90".
     T_N   = T   - DYNF_T  *dt_moist
     U_N   = U   - DYNF_UA *dt_moist
     V_N   = V   - DYNF_VA *dt_moist
     PLE_N = PLE - DYNF_PLE*dt_moist
     ...
     DO j=1,myp
      DO i=1,mxp
       DO k=1,mzp
        ...
        rvap (k,i,j) = Q_N   (i,j,flip(k))! check conversion
     ...
     DO j=1,myp
      DO i=1,mxp
       DO k=1,mzp
   !    gsf_t(k,i,j) = DYNF_T (i,j,flip(k))  + RADLW(i,j,flip(k)) + RADSW  (i,j,flip(k))
   !    gsf_q(k,i,j) = DYNF_Q (i,j,flip(k))
      ...
      ENDDO 
     ENDDO  
     !print*,"DTDT=",maxval(DTDTDYN),MINVAL(DTDTDYN),maxval(DQVDTDYN),MINVAL(DQVDTDYN);call flush(6)
    ELSE
     stop 'unknown env_setting at convpar_gf_geos5.F90'
    ENDIF

    !-temporary for debugging purposes only
    IF( whoami==WANTRANK) then
     call writetxt(mzp,T(1,1,:),ple(1,1,:),th1(1,1,:),pk(1,1,:),q1(1,1,:),u1(1,1,:),zle(1,1,:),zlo(1,1,:)&
                  ,dynf_q(1,1,:) ,dynf_t(1,1,:) ,dynf_ple(1,1,:) ,dynf_ua(1,1,:) &
                  !
                  ,temp(:,1,1),press(:,1,1),rvap(:,1,1),up(:,1,1),zm3d(:,1,1),zt3d(:,1,1),vp(:,1,1),omega(:,1,1)&
                  ,sflux_r(1,1),sflux_t(1,1),topt(1,1),xland(1,1),sfc_press(1,1),dx,kpbl(1,1),temp2m(1,1),dt_moist&
                  )
    ENDIF
 
    !- call the driver routine to apply the parameterization
    CALL GF_GEOS5_DRV(mxp,myp,mzp                 &
...
IF(ITEST==0) CNPCPRATE(i,j) =  0.
          !      
          DO k=1,mzp ! in the future, limit the vertical loop to ktop+3 (DO k=mzp,flip(ktop+3),-1)?
...
IF(ITEST==3) then 
              ...
              !print*,"qi=",k,QILS (i,j,k)*1000., 1000*DT_moist * SRC_CI(flip(k),i,j),tem1,T(i,j,k)-273
ENDIF
         ENDDO 
        ENDDO 
      ENDDO 
      IF(MOMENTUM) THEN
        ...
        ENDDO 
      ENDIF       
!XXXX      DM = ( PLE(:,:,1:mzp)-PLE(:,:,0:mzp-1) )*(1./MAPL_GRAV)
      DO IENS=1, maxiens 
            ...
            DO i=1,mxp
             if(ierr4d(i,j,IENS) .ne. 0) cycle
              DO k=mzp,flip(ktop4d(i,j,IENS))-1,-1
                ...
                !- SRC_CI contains contributions from deep, shallow,... . So, don't need to be accumulated over  CNV_DQLDT
                !-*** note the SRC_CI has different array structure (k,i,j) _not_ (i,j,k)
if(ITEST /=3 ) CNV_DQLDT(i,j,k)=  SRC_CI(flip(k),i,j)*DZ * air_dens !units: kg[w]/(kg[air] s) * m * kg[air]/m^3 = kg[w]/(m^2 s)


!XXXX CNV_DQLDT(i,j,k)=  SRC_CI(flip(k),i,j)*DM(i,j,k)    !units: kg[w]/(kg[air] s) * m * kg[air]/m^3 = kg[w]/(m^2 s)
                !
                !-'detraining_mass_flux',UNITS     = 'kg m-2 s-1',  (only updraft)
                CNV_MFD (i,j,k) = CNV_MFD (i,j,k) + xmb4d(i,j,IENS)*( up_massdetr5d(i,j,flip(k),IENS) )!+ &
                !!- not including downdrafts:       edt4d(i,j,IENS)*  dd_massdetr5d(i,j,flip(k),IENS)   )
...
!----------------------------------------------------------------------------------------------------
                ...
                !- Tiedtke formulation
                CLCN(i,j,k) = CLCN(i,j,k) + (1.0-CLCN(i,j,k))*(xmb4d(i,j,IENS)*up_massdetr5d(i,j,flip(k),IENS) &
                                          * DT_moist/(DZ*air_dens)+CNV_UPDF(i,j,k))

                !~ CLCN(i,j,k) = CLCN(i,j,k) + (1.0-CLCN(i,j,k))*min(mxclfr(IENS),&
                             !~ (xmb4d(i,j,IENS)*up_massdetr5d(i,j,flip(k),IENS)*DT_moist/(DZ*air_dens)+CNV_UPDF(i,j,k)))


!                print*,"ies=",iens,k,(xmb4d(i,j,IENS)*up_massdetr5d(i,j,flip(k),IENS)) * DT_moist/(DZ*air_dens), CLCN(i,j,k)
                !~ !
                !!- Chab&Betchold 2002/2005 formulation
...
!            tem1 =  min(1., (max(0.,(T(i,j,k)-T_ice))/(T_0-T_ice))**2)   
             ...
             SRC_NL(k,i,j) =  SRC_CI(k,i,j) * (1.0-tem1)/(1.333 * MAPL_PI *RL*RL*RL*997.0*disp_factor)  
             SRC_NI(k,i,j) =  SRC_CI(k,i,j) * tem1      /(1.333 * MAPL_PI *RI*RI*RI*500.0*disp_factor)   

             !-outputs 
             QLCN (i,j,k) = QLCN (i,j,k)     + DT_moist * SRC_CI(flip(k),i,j) * (1.0-tem1)
             QICN (i,j,k) = QICN (i,j,k)     + DT_moist * SRC_CI(flip(k),i,j) * tem1

             NCPL (i,j,k) = max(NCPL (i,j,k) + DT_moist * SRC_NL(flip(k),i,j)  , 0.0) 
             NCPI (i,j,k) = max(NCPI (i,j,k) + DT_moist * SRC_NI(flip(k),i,j)  , 0.0)
 
             CNV_FICE (i, j, k)   =   tem1
             DZ       = -( ZLE(i,j,k) - ZLE(i,j,k-1) )
             air_dens = 100.*PLO(i,j,k)/(287.04*T_n(i,j,k)*(1.+0.608*Q_n(i,j,k)))
            ...
            DO IENS=1, maxiens 
                  ...
                  ! update  convective_cloud_area_fraction', adimensional . In the future the convective param will manage its own cfraction contribution
                  !- Tiedtke formulation
                  CLCN(i,j,k) = CLCN(i,j,k) + (1.0-CLCN(i,j,k))*(xmb4d(i,j,IENS)*up_massdetr5d(i,j,flip(k),IENS) &
                                            * DT_moist/(DZ*air_dens)+CNV_UPDF(i,j,k))
            enddo                      
...
!  
  IF(DEBUG_GF==1 .and. maxval(icumulus_gf)>0) then
      !~ IF(ntimes==1) then
      ...
      !~ DO j=1,myp
        !~ DO i=1,mxp
          !~ water_bud(i,j)=0.0
          !~ do k=1,mzp
           !~ water_bud(i,j) = water_bud(i,j)+ &
                            !~ DM(i,j,k)*(SRC_Q(flip(k),i,j)+SRC_CI(flip(k),i,j)) + CONPRR(i,j)
          !~ ENDDO
          !~ print*,"water bud =", water_bud(i,j)
      !~ ENDDO; ENDDO
  ENDIF
...
!---------------------------------------------------------------------------------------------------
   ...
   SUBROUTINE GF_GEOS5_DRV(mxp,myp,mzp                &
              ,ims,ime, jms,jme, kms,kme              & 
              ,its,ite, jts,jte, kts,kte              & 
              ,mynum                 &
              ,dt                    &
...
!
    ...
    REAL,   DIMENSION (its:ite,kts:kte) ::                                      &
                               qcheck,zo,temp_old,qv_old,PO,US,VS,rhoi,phil     &
    ...
    REAL,   DIMENSION (its:ite,maxiens) :: cuten

    REAL,   DIMENSION (its:ite)   :: ter11, xlandi,pbl,zws,ccn,psur,umean,vmean &
                                    ,pmean,ztexec,zqexec,h_sfc_flux,le_sfc_flux,tsur&
                                    ,xlons,xlats
...
!
         ...
         qv_old  (i,k)  = max(1.e-08, rvap(kr,i,j))   
     ...
     DO k=  kts+1,ktf-1
      DO i = its,itf
         if((po(i,1)-po(i,k)).gt.150.and.po(i,k).gt.300)then
            dp=-.5*(po(i,k+1)-po(i,k-1))
    ...
    IF(icumulus_gf(shal) == ON )then
        ...
        DO i=its,itf
          DO k=kts,ktf 
           ...
           qv_new_sh  (i,k)= max(1.0e-8,qv_new_sh  (i,k))
        ...
        CALL CUP_gf_sh(itf,ktf,its,ite, kts,kte&                                
                      ,cumulus_type  (shal) &
                      ,closure_choice(shal) &
                      ,cum_entr_rate (shal) &  
                      ,use_excess    (shal) &
     ...
     IF(icumulus_gf(deep) == ON )then
       ...
       CALL CUP_gf(its,ite,kts,kte, itf,ktf          &
                  ,cumulus_type   (deep)             &
                  ,closure_choice (deep)             &
                  ,cum_entr_rate  (deep)             &
                  ,use_excess     (deep)             &  
        ...
        if(.not. use_gate) then ! GATE
         DO i=its,itf
          DO k=kts,ktf 
           ...
           qv_new_md  (i,k)= max(1.0e-8,qv_new_md  (i,k))
...
!!!!              ,ierr4d               (:,j,shal)  &
     ...
     DO I=its,itf
       cuten(i,deep)=0.
       if(cprr4d(i,j,deep).gt.0.)then
          !--   rainfall output
          CONPRR(i,j)=CONPRR(i,j)+cprr4d(i,j,deep)
     ...
     DO I=its,itf
       cuten(i,mid)=0.
       if(cprr4d(i,j,mid).gt.0.)then
          !--   rainfall output
          CONPRR(i,j)=CONPRR(i,j)+cprr4d(i,j,mid)
     ...
     DO i = its,itf     
      DO k = kts,kte
             ...
             !- adding         shallow (s) +        deep + mid
             RTHCUTEN(kr,i,j)= outt (i,k,shal)+(outt (i,k,deep))*cuten(i,deep) & 
                                              +(outt (i,k,mid ))*cuten(i,mid )
             
             RQVCUTEN(kr,i,j)= outq (i,k,shal)+(outq (i,k,deep))*cuten(i,deep) &
                                              +(outq (i,k,mid ))*cuten(i,mid ) 
             
             RQCCUTEN(kr,i,j)= outqc(i,k,shal)+ outqc(i,k,deep) *cuten(i,deep) &
                                              + outqc(i,k,mid ) *cuten(i,mid )
     ENDDO
     ENDDO
     if(MOMENTUM) then
      DO i = its,itf     
       DO k = kts,kte
             kr=k!+1
             RUCUTEN (kr,i,j)= outU(i,k,deep)*cuten(i,deep)+outU(i,k,mid)*cuten(i,mid)+outU(i,k,shal)
             RVCUTEN (kr,i,j)= outV(i,k,deep)*cuten(i,deep)+outV(i,k,mid)*cuten(i,mid)+outV(i,k,shal)
       ENDDO
      ENDDO
     endif
...
!---------------------------------------------------------------------------------------------------
    ...
    SUBROUTINE CUP_gf(its,ite,kts,kte ,itf,ktf   &
                     ,cumulus           &
                     ,ichoice           &
                     ,entr_rate_input   &
                     ,use_excess        &
...
!~ !    REAL, PARAMETER ::    pgcon=-0.55 , lambau=0.! SAS

     CHARACTER*(*),INTENT(IN) :: cumulus
     INTEGER      ,INTENT(IN) :: itf,ktf,its,ite,kts,kte,ichoice,use_excess
  ...
  !
     ...
     REAL,    DIMENSION (its:ite)               ,INTENT (IN   )    ::   &
        ccn,Z1,PSUR,xland,xlons,xlats, h_sfc_flux,le_sfc_flux       
     REAL,    DIMENSION (its:ite)               ,INTENT (INOUT)    ::   &
        zws,ztexec,zqexec
     REAL                                       ,INTENT (IN   )    ::   &
        dx,dtime,entr_rate_input
!
...
!
     ...
     real,    dimension (its:ite,kts:kte) ::  dtempdz,d2tempdz2
...
! 
      if(cumulus == 'mid' ) then 
         !edtmin=0.1;  edtmax=1.0 ---orig
          edtmin=0.2;  edtmax=0.4
          if(c0_mid < 1.e-8) edtmin=0.0          
      endif
      if(cumulus == 'deep') then 
          !edtmin=0.1;  edtmax=0.6 ---orig
          !edtmin=0.2;  edtmax=0.4  
          DO i=its,itf
             if(xland(i) > 0.99 ) then !- over water          
                   edtmin(i)=0.1;  edtmax(i)=0.2 
             else!- over land          
                   edtmin(i)=0.1;  edtmax(i)=0.3 
             endif
          ENDDO          
      endif 
!
...
!       
       DO i=its,itf
        ...
        if(ierr(i) ==0)then
           ...
           call get_cloud_bc(kts,kte,q_cup (i,kts:kte),rlll,k22(i),x_add)
           x_add = float(use_excess)*ztexec(i)
           call get_cloud_bc(kts,kte,t_cup (i,kts:kte),tlll,k22(i),x_add)
           ...
           if(dzlcl >= 0.) then ! LCL found (if dzlcl<0 => not found)
             call get_cloud_bc(kts,kte,z_cup (i,kts:kte),zlll,k22(i))
             do k=kts,ktf
...
!
      do i=its,itf
       IF(ierr(I).eq.0)THEN
             ...
             call get_cloud_bc(kts,kte,he_cup (i,kts:kte),hkb (i),k22(i),x_add)
             call get_cloud_bc(kts,kte,heo_cup(i,kts:kte),hkbo(i),k22(i),x_add)
       endif ! ierr
      enddo
!
           ...
           DO i=its,itf
             ...
             do k=kts,ktf
                     ...
                     if(ENTRNEW) then
                       ...
                       if(cumulus == 'deep') cd(i,k)=1.e-2*entr_rate
...
!
        !~ iloop=1
        !~ jprnt=0
        !~ if(cumulus == 'mid') iloop=5
        !~ IF(cumulus == 'deep' ) & 
        !~ call cup_kbcon(cumulus,ierrc,cap_max_increment,iloop,k22,kbcon,heo_cup,heso_cup, &
                       !~ hkbo,ierr,kbmax,po_cup,cap_max, &
                       !~ ztexec,zqexec,use_excess,       &
                       !~ jprnt,itf,ktf,its,ite, kts,kte, &
                       !~ z_cup,entr_rate_2d,heo,klcl,hcot)
        !~ IF(cumulus == 'mid' ) & 
        CALL cup_cloud_limits(cumulus,ierrc,ierr,cap_max_increment,cap_max,entr_rate  &
                             ,heo_cup,heso_cup,qo_cup,qeso_cup,po_cup,zo_cup,heo,hkbo &
                             ,entr_rate_2d,hcot,k22,kbmax,klcl,kbcon,ktop             &
                             ,use_excess,zqexec,ztexec,itf,ktf,its,ite, kts,kte)
...
!
        ...
        !- scale dependence factor
            ...
            if(frh .gt. 0.7)then 
              frh=0.7
              radius=sqrt(frh*dx*dx/3.14)! dx*sqrt(frh/3.14)
...
!
      IF(cumulus == 'mid') THEN
        ...
        !--- get inversion layers
          call get_inversion_layers(cumulus,ierr,psur,po_cup,tn_cup,zo_cup,qo_cup,qeso_cup,k_inv_layers,&           
                                    dtempdz,itf,ktf,its,ite, kts,kte,d2tempdz2)
          DO i=its,itf
           if(ierr(i) /= 0)cycle
      ...
      ENDIF
!      
!-- last check for ktop (deep and mid)
!
      DO i=its,itf
        if(ierr(i) /= 0)cycle
...
!
      DO i=its,itf
         zuo(i,:)=0.
         if(ierr(i) /= 0) CYcle
         CALL get_zu_zd_pdf(trim(cumulus)//"_up",ierr(i),k22(i),ktop(i),zuo(i,kts:kte),kts,kte,ktf  &
...
!===start_level
      if(cumulus == 'deep') start_level(:)=  KLCL(:)!k22(:)
      if(cumulus == 'mid' ) start_level(:)=  KLCL(:)!kbcon(:)
      ...
      do i=its,itf
       IF(ierr(i).eq.0)THEN
         do k=kts,start_level(i)
            ...
            call get_cloud_bc(kts,kte,u_cup(i,kts:kte),uc(i,k),k22(i))
            call get_cloud_bc(kts,kte,v_cup(i,kts:kte),vc(i,k),k22(i))
         enddo
       ENDIF 
      enddo
...
!
       do i=its,itf
         zd(i,:)=0.
         IF(ierr(i) /= 0) cycle
         call get_zu_zd_pdf("DOWN",ierr(i),kdet(i),jmin(i),zdo(i,:),kts,kte,ktf&
                           ,kpbl(i),k22(i),kbcon(i),klcl(i),po_cup(i,kts:kte),psur(i),xland(i))
       enddo
!      
...
!
      if(cumulus=='deep') then 
            ...
            T_star=40.  ! T_star = temp scale in original paper = 1 K 
      endif
      if(cumulus=='mid') then
           wmeanx=3!.4.
           T_star=40.!1
      ...
      DO i=its,itf
           ...
           !- time-scale cape removal from  Betchold et al. 2008
            ...
            tau_ecmwf(i)=max(tau_ecmwf(i),3600.)!exp RADM
            tau_ecmwf(i)= tau_ecmwf(i) * (1. + 1.66 * (dx/(125*1000.)))! dx must be in meters
      enddo
      !
      DO i=its,itf
            if(ierr(i) /= 0) cycle
      ...
      ELSEIF( DICYCLE==4) then!zhang(2002)
         ...
         call cup_env_clev(tn_x,qeso_x,qo_x,heo_x,heso_x,zo,po,qeso_cup_x,qo_cup_x, &
                         heo_cup_x,heso_cup_x,zo_cup,po_cup,gammao_cup_x,tn_cup_x,psur,  &
                         ierr,z1,itf,ktf,its,ite, kts,kte)
         ...
         DO i=its,itf 
             ...
             DO k=max(kbcon(i),kts+1),ktop(i)
                 ...
                 Zenv=0.5*(zuo(i,k+1)+zuo(i,k) - (zdo(i,k+1)+zdo(i,k))*edto(i))
                 S2= cp*tn_cup_x(i,k+1) + g*zo_cup(i,k+1)
                 S1= cp*tn_cup_x(i,k  ) + g*zo_cup(i,k  )
                 Q2= qo_cup_x(i,k+1)
                 Q1= qo_cup_x(i,k  ) 

                 dqdt(i,k)=         -Zenv*(Q2-Q1)*g/dp
                 dtdt(i,k)= -(1./cp)*Zenv*(S2-S1)*g/dp
...
PHY2:  DO step=fa,fa
        ...
        do i=its,itf
             ...
             call get_cloud_bc(kts,kte,heo_cup_x(i,kts:kte),hkbo_x(i),k22(i),x_add)
        enddo
        do i=its,itf
             hco_x (i,:)=0.
             IF(ierr(i) /= 0)cycle
...
!        call error_fatal ( 'totmas .gt.1.e-6' )
            ...
            dellu(i,k)=((detup+lambau(i)*detup)*.5*(uC(i,K+1)+uC(i,K)) &
                    +detdo*.5*(UCD(i,K+1)+UCD(i,K)) &
                    -(entup+lambau(i)*detup)*us(i,k) &
                    -entdo*us(i,k) &
                    +subin*u_cup(i,k+1) &
                    -subdown*u_cup(i,k) &
            ...
            dellv(i,k)=((detup+lambau(i)*detup)*.5*(vC(i,K+1)+vC(i,K)) &
                    +detdo*.5*(vCD(i,K+1)+vCD(i,K)) &
                    -(entup+lambau(i)*detup)*vs(i,k) &
                    -entdo*vs(i,k) &
                    +subin*v_cup(i,k+1) &
                    -subdown*v_cup(i,k) &
...
!                   +pgcon*.5*(zuo(i,k)+zuo(i,k+1))*(v_cup(i,k+1)-v_cup(i,k)))*g/dp
           ...
           if(i.eq.ipr .and. i.eq.ipr .and. k.eq.ktop(i))then
...
!
      ...
      do i=its,itf
       ...
       if(ierr(i).eq.0)then
         ...
         if(dellaqc(i,k).lt.0)write(0,*)'neg della',i,k,ktop(i),qrco(i,k), &
              qrco(i,k+1),up_massdetro(i,k),zuo(i,ktop(i))
         dellaqc(i,k)=max(0.,dellaqc(i,k))
         if(i.eq.ipr.and.i.eq.ipr)write(0,*)'dellaqc,qrco = ',k,dellaqc(i,k),qrco(i,k)
       endif
      enddo
      enddo
      dellah(its:itf,kts:ktf)=dellah(its:itf,kts:ktf)+dsubh(its:itf,kts:ktf)
      dellaq(its:itf,kts:ktf)=dellaq(its:itf,kts:ktf)+dsubq(its:itf,kts:ktf)
      dsubh(:,:)=0.0
      dsubq(:,:)=0.0
      ...
      do i=its,itf
        ...
        if(ierr(i).eq.0)then
         do k=kts,ktop(i)
            ...
            detup=up_massdetro(i,k)
            if(.not. USE_C1D  .or. cumulus == 'mid') then
...
!        XT(I,K)=((1./cp)*DELLAH(i,k)-(xlv/cp)*(DELLAQ(i,k)+DELLAQC(i,k)))*MBDT(i)+TN(I,K)
      ...
      do i=its,itf
         ...
         !-ztexec and zqexec
         call get_cloud_bc(kts,kte,DELLAH (i,kts:kte),DELLAH_aver,k22(i))
...
!- begin: for GATE soundings------------------------------------------- 
   if(use_gate .or. wrtgrads) then
    if(cumulus == 'deep') then ; cty='1'; nvarbegin =  1 ;endif
    if(cumulus == 'mid' ) then ; cty='3'; nvarbegin = 51 ;endif
    do i=its,itf
     !if(ierr(i).eq.0) then
      !- 2-d section
      do k=kts,max(1,ktop(i))
       ...
       call set_grads_var(jl,k,nvar,edto(i)*pwdo(i,k),"pwd"//cty ,' xx','3d')
...
!        print*,'DD1',k,dh,qrcd(i,k),qcd(i,k),pwd(i,k);call flush(6)
!
        bu(i)=dz*dh
        do ki=jmin(i)-1,1,-1
...
!
         ...
         !print*,'DD2',ki,dh,qcd(i,ki),pwd(i,ki),dqeva;call flush(6)
        enddo
!
!--- end loop over i
...
!        p_cup(i,1)=.5*(p(i,1)+psur(i))        
      ...
      enddo
!------------------------------------------------
     !~ write(4,*) z1(1),z(1,1),z_cup(1,1),0.5*(z1(1)+z(1,1))
     !~ do k=kts,ktf
      !~ do i=its,itf
        !~ if(ierr(i).eq.0)then
          !~ write(4,1000) k,z(i,k),p(i,k),z_cup(i,k),p_cup(i,k),t(i,k),t_cup(i,k),gamma_cup(i,k)
          !~ 1000 format(i3,7F10.3)
        !~ endif
      !~ enddo
      !~ enddo
...
!------------------------------------------------------------------------------------
   SUBROUTINE cup_up_aa0_OLD(aa0,z,zu,dby,GAMMA_CUP,t_cup,       &
              kbcon,ktop,ierr,                               &
              itf,ktf,its,ite, kts,kte,range                 )
  ! aa0 cloud work function
  ! gamma_cup = gamma on model cloud levels
  ! t_cup = temperature (Kelvin) on model cloud levels
  ! dby = buoancy term
  ...
  ! on input
     ...
     integer, dimension (its:ite)         ,intent (in   )  ::  &
        kbcon,ktop
     character(LEN=2),OPTIONAL, intent(in) :: range
  ! input and output  
     integer, dimension (its:ite) ,intent (inout)  :: ierr
     real,    dimension (its:ite) ,intent (out  )  :: aa0
  !
  ...
  !  set domain of integration      
      ...
      do i=its,itf
        if(ierr(i) /= 0)cycle
        do k= kts,kte
        write(4,1000)k,z(i,k),zu(i,k),dby(i,k),GAMMA_CUP(i,k),t_cup(i,k)
        1000 format(1x,i3,5F10.3)
...
enddo;enddo
      ...
      do i=its,itf
        if(ierr(i) /= 0)cycle
        do k= kbeg(i)+1,kend(i)
         
          dz=z(i,k)-z(i,k-1)
          da=zu(i,k)*dz*(g/(cp*((t_cup(i,k)))))*dby(i,k-1)/(1.+gamma_cup(i,k))

          !~ ! if(k.eq.ktop(i).and.da.le.0.) exit
          aa0(i)=aa0(i)+da
          !~ !aa0(i)=max(aa0(i),0.0)
...
!
  ...
  !
     real,    dimension (its:ite,kts:kte)                              &
        ,intent (in   )                   ::                           &
        t_cup,p_cup,rho,q,zu,gamma_cup,qe_cup,hc,                      &
...
!
     ...
     integer                              ::                           &
        iounit,iprop,iall,i,k,k1,k2,n,nsteps,rk
     real                                 ::                           &
        prop_ave,qrcb_h,bdsp,dp,rhoc,dh,qrch,c0,dz,radius,berryc0,q1,berryc
...
!--- no precip for small clouds
	  ...
	  tempc   (i,k)=t_cup(i,k)
        ...
        do i=its,itf
          if(ierr(i).eq.0)then
            call get_cloud_bc(kts,kte,qe_cup (i,kts:kte),qaver,k22(i))
            qaver = qaver + float(use_excess)*zqexec(i)

            do k=kts,start_level(i)!k22(i)      
              qc (i,k)=qaver
              qch(i,k)=qc(i,k)
              !print*,"MOIS1=",k,k22(i),qc(i,k),q(i,k),zqexec(i);call flush(6)!
            enddo
           endif
        enddo
        ...
        DO 100 i=its,itf
          ...
          DO k=start_level(i)+1,ktop(i)      !k22(i)+1,ktop(i)
		...
		ENDIF
	        ...
	        c0=.002
	          ...
	          aux = 1. * exp(0.01* (t_cup(i,k) - 273.16))
                endif
	        !.. cx0 = aux*(c1d(i,k)+c0)*DZ         !exp B1a3x
	         c0=.004
...
!-v2 - conserves total water: 	QRCH+PW (i,k)+QRC(i,k)
		...
		if(qrc(i,k)>min_liq) then 
		   ...
		   cx0 = 0.0005*cbf !use this: c1d(i,k)+c0
...
!------------------------------------------------------------------------------------
   ...
   SUBROUTINE CUP_gf_sh(itf,ktf ,its,ite, kts,kte&
             ,cumulus                            &
             ,ichoice                            &
             ,entr_rate                          &
             ,use_excess                         &
     ...
     implicit none
     logical, parameter :: make_calc_for_xk = .false.
  ...
  !
     real, dimension (its:ite,kts:kte) ,intent (in   ) ::      &
                       rho,t,po,us,vs,tn,dhdt
     real, dimension (its:ite,kts:kte) ,intent (inout) ::      &
                       Q,QO
     real, dimension (its:ite) ,intent (in   )            ::      &
                       zws,ztexec,zqexec,ccn,z1,psur,xland        &
                      ,h_sfc_flux,le_sfc_flux,tsur
  ...
  ! edt     = epsilon
     ...
     integer, dimension (its:ite)      , intent (inout)  :: &
      ...
      character(len=1):: cty      
      logical, parameter :: USE_LCL       =.FALSE.
      logical, parameter :: USE_FORM_OLD  =.FALSE.
      logical, parameter :: USE_INV_LAYERS=.FALSE. !XXXX
...
!--------------------------------------------------------------------------------------
!--------------------------------------------------------------------------------------
     IF(USE_FORM_OLD) THEN
!--------------------------------------------------------------------------------------
!--------------------------------------------------------------------------------------

!
!--- 1st guess for entraiment
!
      DO i=its,itf
         IF(ierr(i).eq.0)THEN
            do k=kts,ktf
...
!--- DETERMINE THE LEVEL OF CONVECTIVE CLOUD BASE  - KBCON
!
      call cup_kbcon(cumulus,ierrc,cap_max_increment,5,k22,kbcon,heo_cup,heso_cup, &
           hkbo,ierr,kbmax,po_cup,cap_max, &
           ztexec,zqexec,use_excess, &
           0,itf,ktf,its,ite, kts,kte, &
           z_cup,entr_rate_2d,heo,klcl,hcot)
                      
!--- update hkb/hkbo in case of k22 is redefined in 'cup_kbcon'
      do i=its,itf
       IF(ierr(i) /= 0) CYCLE
             x_add = float(use_excess)*(xlv*zqexec(i)+cp*ztexec(i))
             call get_cloud_bc(kts,kte,he_cup (i,kts:kte),hkb (i),k22(i),x_add)
      ...
      enddo        
!
!--- 2nd guess for entrainment and detrainment
!
      DO i=its,itf
         IF(ierr(I).eq.0)THEN
            do k=kts,ktf
               frh = min(qo_cup(i,k)/qeso_cup(i,k),1.)
               entr_rate_2d(i,k)=entr_rate*(1.3-frh)*min(1.,(qeso_cup(i,k)/qeso_cup(i,kbcon(i)))**1.25)
               cd(i,k)=0.75*entr_rate_2d(i,k)!+0.5e-3
            enddo
         endif
      enddo
!           
!--- get ktop and beta PDF profiles for normalized mass flux
!
...
!--------------------------------------------------------------------------------------
!--------------------------------------------------------------------------------------
     ELSE ! use new formulation for cloud base and top
!--------------------------------------------------------------------------------------
!--------------------------------------------------------------------------------------
...
!
      IF(MAKE_CALC_FOR_XK) THEN
            DO i=its,itf
              ...
              aa1(i)=0.
            enddo
            call cup_up_aa0(aa0,z_cup,zu,dby,GAMMA_CUP,t_cup &
                 ,k22,klcl,kbcon,ktop,ierr,           &
                 itf,ktf,its,ite, kts,kte)
            call cup_up_aa0(aa1,zo_cup,zuo,dbyo,GAMMAo_CUP,tn_cup &
                 ,k22,klcl,kbcon,ktop,ierr,           &
                 itf,ktf,its,ite, kts,kte)
            do i=its,itf
               if(ierr(i).eq.0)then
         ...
         do k=kts,ktop(i)+1  
           ...
           ! entrainment/detrainment for updraft
            ...
            if(abs(totmas).gt.1.e-6)then
               ...
               write(0,123)detup,entup,detupk,zuo(i,k+1),zuo(i,k)      
...
!-srf - this part of the code implements XK calculation for Grell's closure
!-srf - however it is not really being used and we will avoid extra calculations
    IF(MAKE_CALC_FOR_XK) THEN
      ...
      do i=its,itf
       if(ierr(i).eq.0)then
         !XHKB(I)= DELLAH(I,K22(i))*MBDT+HKBO(I)
        call get_cloud_bc(kts,kte,DELLAH (i,kts:kte),DELLAH_aver,k22(i))
        XHKB(I)= DELLAH_aver*MBDT + HKBO(I)
       endif
      enddo
!
!--- calculate moist static energy, heights, qes
...
!
      ...
      do k=klcl(i)+1,ktop(i) ! kbcon(i)+1,ktop(i)
       xhc(i,k)=(xhc(i,k-1)*xzu(i,k-1)-.5*up_massdetro(i,k-1)*xhc(i,k-1)+ &
                                          up_massentro(i,k-1)*xhe(i,k-1))   /            &
                    (1.e-16+xzu(i,k-1)-.5*up_massdetro(i,k-1)+up_massentro(i,k-1))
...
!--- workfunctions for updraft
!
      call cup_up_aa0(xaa0,xz_cup,xzu,xdby,GAMMA_CUP,xt_cup &
           ,k22,klcl,kbcon,ktop,ierr,           &
           itf,ktf,its,ite, kts,kte)

     ENDIF ! avoid calculation for the Grell's own closure which is not used 
!
...
!----------------------------------------------------------------
  ...
  do i=its,itf
         ...
         do k=kts,ktop(i)
           ...
           tup   (i,k) = tempco(i,k) !temperatura da parcela ! de ar no updraft 
        enddo
      endif
  enddo
!      
  ...
  if(use_gate .or. wrtgrads) then
    ...
    do i=its,itf
      do k=kts,kte!max(1,ktop(i))
       ...
       !~ call set_grads_var(jl,k,nvar,1000.*d2tempdz2(i,k),"secder"//cty ,' K km-2','3d')
       !~ call set_grads_var(jl,k,nvar,clfrac(i,k),"clfrac"//cty ,'shcf #','3d')
      enddo
      if(.not. use_gate) call wrt_bin_ctl(1,kte,po(1,1:kte),cumulus)
    enddo
  endif
  !- end  : for GATE soundings-------------------------------------------
...
!~ !=================
     ...
     IF(version == 0 ) then
        do i=its,itf
!~ !=================
!~ if(i==iprloc .and. cumulus == 'deep')  print*,"2loc=",xlons(i)-360.,xlats(i),ierr(i)
!~ !=================

           if(ierr(i) /= 0 ) cycle
!~ !=================
!~ if(i==iprloc .and. cumulus == 'deep')  print*,"3loc=",xlons(i)-360.,xlats(i),ierr(i)
!~ !=================
           do k=kts,kbcon(i)
...
!~ !=================
!~ if(i==iprloc .and. cumulus == 'deep')  then
...
!------------------------------------------------------------------------------------
  ...
  SUBROUTINE get_zu_zd_pdf(draft,ierr,kb,kt,zu,kts,kte,ktf,kpbli,k22,kbcon,klcl,po_cup,psur,xland)
...
!-------- gama pdf 
  ...
  INTEGER :: k1
  real :: lev_start,g_alpha2,g_a,y1,x1,g_b,a,b,alpha2,csum,zubeg
...
!-===========-===========-===================-============================tmp  
   !integer, intent(inOUT) :: kt,kb,klcl!!!!temp
   !real :: zuout(50,50,kts:kte), zuout1(50,50,kts:kte), zuout2(50,50,kts:kte)
   !INTEGER :: kkk,tune
   !~ open(19,file= 'pdf.gra', form='unformatted',access='direct'&
        !~ ,status='unknown',recl=50*50)
   !~ zuout=0. ;zuout1=0.; zuout2=0.;nrec=0
   !~ !   do tune=1,49;    do kkk=10,50
   !~ do tune=10,41;    do kkk=5,tune-4
!-===========-===========-===================-============================tmp  
  ...
  IF(itest==5 .and. (draft == "deep_up" .or. draft == "mid_up" )) then
      ...
      !tunning= po_cup(kt)+(po_cup(kpbli)-po_cup(kt))*lev_start
      !tunning = min(0.8, (tunning-po_cup(kb_adj))/(po_cup(kt)-po_cup(kb_adj))) !=.6
      !tunning = max(0.1, tunning)
      tunning = 0.30
      ...
      do k=kb_adj,min(kte,kt-1)
         ...
         !print*,"zu=",k,zu(1,1,k),kratio,fzu
      enddo
      !- normalize ZU
      ...
      kb_adj=minloc(abs(po_cup(kts:kt-1)-pmaxzu),1)
      ...
      Do k=kts,max(kts,maxloc(zuh(:),1)-2)
          zu(k)=zuh(k)
      ENDDO
      Do k=max(kts,maxloc(zuh(:),1)-1),min(maxloc(zuh(:),1)+1,kt-1)
          zu(k)=0.5*(zu(k)+zuh(k))
      ENDDO
      ...
      zu(kts)=0.
!
   ELSEIF(itest==4 .and. (draft == "deep_up" .or. draft == "mid_up" )) then
      csum =0.
      zubeg=0.
      lev_start=min(.9,.1+csum*.013)
      kb_adj=max(kb,2)  
      ...
      !where (zu(:) .ge. 1.e-6) zu(:)=zu(:)/maxval(zu(:))
  ...
  ELSEIF(itest==3 .and. draft == "deep_up") then
      ...
      zu(kts)=0.
!===========================================
  ELSEIF(itest==10 .and. draft == "deep_up") then
      !pmaxzu=psur-px*(psur-po_cup(kt))
      pmaxzu=850.
      kb_adj=minloc(abs(po_cup(kts:kt-1)-pmaxzu),1)!;print*,"1=",kb_adj,po_cup(kb_adj)
      kb_adj=max(kb,kb_adj)
      kb_adj=min(kb_adj,kt)
      tunning = 0.25 !-- clo-X and tune0.6 experiment.
      beta    = 2.0/tunning
      alpha   = tunning*beta
      !- this alpha constrains the location of the maximun ZU to be at 
      !- "kb_adj" vertical level
      ...
      do k=klcl-1,min(kte,kt-1) 
          kratio= float(k)/float(kt+1)
          zu(k) = kratio**(alpha-1.0) * (1.0-kratio)**(beta-1.0)
          !print*,"1",k,zu(k),kb_adj,pmaxzu
      enddo    
      zu(kts:min(kte,kt+1))= zu(kts:min(kte,kt+1))/ (1.e-9+maxval(zu(kts:min(kte,kt+1)),1))
      
      !-----------     
      !pmaxzu=psur-px2*(psur-po_cup(kt))
      pmaxzu=400.
      kb_adj=minloc(abs(po_cup(kts:kt-1)-pmaxzu),1)!;print*,"2=",kb_adj,po_cup(kb_adj)
      kb_adj=max(kb,kb_adj)
      kb_adj=min(kb_adj,kt)
      tunning = 1.1 !-- clo-X and tune0.6 experiment.
      beta    = 2.0/tunning
      alpha   = tunning*beta
      !- this alpha constrains the location of the maximun ZU to be at 
      !- "kb_adj" vertical level
      ...
      do k=klcl-1,min(kte,kt-1) 
          kratio= float(k)/float(kt+1)
          zuh(k) = kratio**(alpha-1.0) * (1.0-kratio)**(beta-1.0)
          !print*,"2",k,zuh(k),kb_adj,pmaxzu
      enddo
     zuh(kts:min(kte,kt+1))= zuh(kts:min(kte,kt+1))/ (1.e-9+maxval(zuh(kts:min(kte,kt+1)),1))
...
!===========================================!===========================================
  ...
  ELSEIF(itest==11 .and. draft == "deep_up") then
      ...
      kb_adj=max(kb,kb_adj)
      kb_adj=min(kb_adj,kt)
      tunning = 0.60 !-- clo-X and tune0.6 experiment.
    !-===========-===========-===================-============================tmp  
    !~ kb=kkk
    !~ klcl=kkk+1  
    !~ kt=tune 
    !~ !tunning=float(tune)/50.
    !~ kb_adj=minloc(abs(po_cup(kts:kt-1)-pmaxzu),1);print*,"1=",kb,kb_adj,po_cup(kb_adj),tune
    !~ kb_adj=max(kb,2)  
    !~ kb_adj=max(kb,kb_adj)
    !~ kb_adj=min(kb_adj,kt-1)
    !-===========-===========-===================-============================tmp 
      ...
      enddo    
      zul(kts:min(kte,kt-1))= zul(kts:min(kte,kt-1))/ (1.e-9+maxval(zul(kts:min(kte,kt-1)),1))

     !-===========-===========-===================-============================tmp 
     !zuout1(tune,kkk,kts:kte)=zul(kts:kte)
     !-===========-===========-===================-============================tmp 

      !-----------     
      pmaxzu=po_cup(kt)+150.
      !pmaxzu=200.
      kb_adj=minloc(abs(po_cup(kts:kt-1)-pmaxzu),1)!;print*,"2=",kb_adj,po_cup(kb_adj),tune
      kb_adj=max(kb,kb_adj)
      kb_adj=min(kb_adj,kt)
      tunning = 0.60 
    !-===========-===========-===================-============================tmp 
    ...
    !~ !kb_adj=kkk  + 10
    !~ !tunning=float(tune)/50.
    !~ kb_adj=minloc(abs(po_cup(kts:kt-1)-pmaxzu),1);print*,"2=",kb,kb_adj,po_cup(kb_adj)
    !~ kb_adj=max(kb,2)  
    !~ kb_adj=max(kb,kb_adj)
    !~ kb_adj=min(kb_adj,kt-1)
    !-===========-===========-===================-============================tmp 
...
!      zu(:)=0.8*zul(:)+ 0.2*zuh(:)
      ...
      !-===========-===========-===================-============================tmp 
      !zuout2(tune,kkk,kts:kte)=zuh(kts:kte)
      !-===========-===========-===================-============================tmp 

      !~ !--remove any layer of zero detrainment between maxloc(zul) and maxloc(zuh)
      !~ maxzul=max(kts+1,maxloc(zul(:),1))
      !~ maxzuh=max(kts+1,maxloc(zuh(:),1))     
      !~ minzu =max(kts+1,minloc(zu (maxzul:maxzuh),1))+maxzul-1
      !~ !if(tune==20)print*,"xxx=",maxzul,minzu,maxzuh,minloc(zu (maxzul:maxzuh),1),"xxx",zu(:)
      !~ if(maxzuh  > minzu+1) then
        !~ Do k=maxzul+1,min(minzu,kt-1)
         !~ if(zu(k) < zu(k-1)) zu(k)=zu(k-1)
        !~ ENDDO
      !~ endif
      !~ if(minzu-1 > maxzul ) then
        !~ Do k=maxzuh-1,min(minzu,kt-1),-1
         !~ if(zu(k) < zu(k+1)) zu(k)=zu(k+1)
        !~ ENDDO
      !~ endif
      ...
      zu(kts)=0.
!     
  ELSEIF(draft == "XXXshallow_up" .or. draft == "mid_up" ) then
      ...
      !- location of the maximum Zu:  "level_max_zu" levels above 
      if(xland  > 0.90 ) then !- over water          
            level_max_zu=1
            !pmaxzu=850.
            !ktarget=minloc(abs(po_cup(kts:kt-1)-pmaxzu),1)!;print*,"1=",kb_adj,po_cup(kb_adj)
            !level_max_zu=ktarget-(kpbli_adj+1)
      else!- over land           
            pmaxzu=800.
            ktarget=minloc(abs(po_cup(kts:kt-1)-pmaxzu),1)!;print*,"1=",kb_adj,po_cup(kb_adj)
            level_max_zu=ktarget-(kpbli_adj+1)
      ...
      enddo
      zu(kts)=0.
 ELSEIF(draft == "shallow_up" .or. draft == "UPMIDx" ) then
      !alpha= 3.
      !beta = 2.*alpha
      !kb_adj=kb ! level where mass flux starts
      kb_adj=klcl-1!kbcon-1! level where mass flux starts
      ...
      enddo
      zu(kts)=0.
      !- zu = zero below klcl-1 and above ktop
      !zu(1:max(klcl-1,1)) = 0.0

  ELSEIF(draft == "xxDOWN" .or. draft == "DOWNM") then
  ...
  ELSEIF(draft == "DOWN" ) then
      ...
      beta=2.5          
      !pmaxzu=psur-20.
      !pmaxzu=min(psur-20.,pmaxzu)
      !kb_adj=minloc(abs(po_cup(kts:kt-1)-pmaxzu),1)!;print*,"2=",kb_adj,po_cup(kb_adj),tune
      kb_adj=kb+2
      kb_adj=max(kts+2,kb_adj)
      kb_adj=min(kb_adj,kt-1)
      !print*,"2=",kb_adj,po_cup(kb_adj)
      !~ !- this alpha constrains the location of the maximun ZU to be at "kb_adj" vertical level
      alpha=1. + (beta-1.0)*(float(kb_adj)/float(kt+1))/(1.0-(float(kb_adj)/float(kt+1)))
  ...
  ENDIF
!-===========-===========-===================-============================tmp 
      !~ zuout(tune,kkk,kts:kte)=zu(kts:kte)
    !~ ENDDO;ENDDO
    
      !~ nrec=0
      !~ do k = kts,kte
       !~ nrec=nrec+1
       !~ write(19,rec=nrec) zuout (1:50,1:50,k)
      !~ enddo
      !~ do k = kts,kte
       !~ nrec=nrec+1
       !~ write(19,rec=nrec) zuout1(1:50,1:50,k)
      !~ enddo
      !~ do k = kts,kte
       !~ nrec=nrec+1
       !~ write(19,rec=nrec) zuout2(1:50,1:50,k)
       !~ !nrec=nrec+1
       !~ !write(19,rec=nrec) zuout2(1:50,k)
      !~ enddo
...
!------------------------------------------------------------------------------------
  ...
  SUBROUTINE cup_kbcon(NAME,ierrc,cap_inc,iloop,k22,kbcon,he_cup,hes_cup, &
   ...
   ! only local  dimensions are need as of now in this routine
      ...
      integer ,intent (in   )             ::    &
        use_excess,jprnt,itf,ktf,its,ite, kts,kte
   ! 
   ! ierr error value, maybe modified in this routine
   !
     real,    dimension (its:ite,kts:kte)                              &
     ...
     real, dimension (its:ite,kts:kte),intent (in) :: z_cup,heo,entr_rate_2d
...
!print*,"1",k22(i),kbcon(i),klcl(i),KBMAX(i)+3
       !== including entrainment for hetest
        if(name.eq.'shallow'.or. NAME.eq.'mid' )start_level(i)=klcl(i)!kbcon(i)
        if(name.eq.'deep'                      )start_level(i)=klcl(i)!k22(i)

        hcot(i,1:start_level(i)) = HKB(I)
!print*,"h1",HKB(I),start_level(i),name
...
!print*,"h2",k,hcot(I,k),heo(i,k),entr_rate_2d(i,k-1);call flush(6)
 ...
 31     CONTINUE
        ...
        IF(KBCON(I).GT.KBMAX(i)+2)THEN
           ...
           !print*,"===>could not find reasonable kbcon in cup_kbcon"
           GO TO 27
       ENDIF
 32    CONTINUE
...
!print*,"h8",k,hcot(I,k),heo(i,k),entr_rate_2d(i,k-1);call flush(6)
            ...
            IF(KBCON(I).GT.KBMAX(i)+2)THEN
               ...
               print*,"===>could not find reasonable kbcon in cup_kbcon",ierr(i)
               GO TO 27
            ENDIF
            GO TO 32
       ENDIF
...
!------------------------------------------------------------------------------------
 SUBROUTINE get_cloud_bc(kts,mzp,array,x_aver,k22,add,maxi)
    implicit none       
    integer, intent(in)     :: kts,mzp,k22
    real   , intent(in)     :: array(kts:mzp)
    real   , OPTIONAL , intent(in)  :: add
    INTEGER, OPTIONAL , intent(in)  :: maxi
    ...
    if(present(maxi)) x_aver=maxval(array(kts:local_order_aver))
...
!------------------------------------------------------------------------------------
 ...
 SUBROUTINE get_inversion_layers(NAME,ierr,psur,po_cup,to_cup,zo_cup,qo_cup,qeso_cup,k_inv_layers,&           
                                 dtempdz,itf,ktf,its,ite, kts,kte,d2tempdz2)
        ...
        REAL,    DIMENSION (its:ite,kts:kte), INTENT (IN )  :: po_cup,to_cup,zo_cup,qo_cup,qeso_cup                              
        REAL,    DIMENSION (its:ite,kts:kte), INTENT (OUT)  :: dtempdz                         
        REAL,    DIMENSION (its:ite,kts:kte), INTENT (OUT), OPTIONAL :: d2tempdz2                     
        ...
        !- 2nd method
         ...
         ENDDO
         first_deriv(kts)=first_deriv(kts+1)
         
         DO k = kts+1,ktf-3
           !-  get the 2nd derivative
           sec_deriv(k)= (first_deriv(k+1)-first_deriv(k-1))/(zo_cup(i,k+1)-zo_cup(i,k-1))        
           sec_deriv(k)= abs(sec_deriv(k))        
         ENDDO
         dtempdz(i,kts)=dtempdz(i,kts+1)
         sec_deriv(kts)=sec_deriv(kts+2)
         dtempdz(i,ktf)=dtempdz(i,ktf-1)
         sec_deriv(ktf:kte)=sec_deriv(ktf-2)
         
         if(present(d2tempdz2)) d2tempdz2(i,kts:kte)= sec_deriv(kts:kte)
         
         ilev=kts+2
         ix=1
         k=ilev
         DO WHILE (zo_cup(i,ilev)<15000.)
           DO kk=k,ktf-3             
             if(sec_deriv(kk) < sec_deriv(kk+1) .and. sec_deriv(kk) < sec_deriv(kk-1)) then
                k_inv_layers(i,ix)=kk
                ix  =ix+1
                ilev=kk+1
                !print*,"3=",kk,zo_cup(i,kk),ilev-1
                exit   
             endif
             ilev=kk+1
           ENDDO
           k=ilev
         ENDDO         
          !print*,"layers=",k_inv_layers(i,1:maxloc(k_inv_layers(i,:),1)),maxloc(k_inv_layers(i,:),1)
          !write(13,100) k_inv_layers(i,1:maxloc(k_inv_layers(i,:),1))
          !where(k_inv_layers < 0)k_inv_layers=1
          !write(13,101) zo_cup(i,k_inv_layers(i,1:7))
          !write(13,101) po_cup(i,k_inv_layers(i,1:7))

         !- 2nd criteria
         DO k=1,maxloc(k_inv_layers(i,:),1)
           kk=k_inv_layers(i,k)
           !print*,"x=",kk, dtempdz(i,kk-1),dtempdz(i,kk) , dtempdz(i,kk+1) 
           if( dtempdz(i,kk) < dtempdz(i,kk-1) .and. &
               dtempdz(i,kk) < dtempdz(i,kk+1) ) then ! the layer is not a local maximum
               k_inv_layers(i,k) = 1
               !print*,"y=",kk,k,k_inv_layers(i,k)
           endif
         ENDDO
        ...
        DO i = its,itf        
         ...
         DO k=kts,kte
           distance(k)=abs(po_cup(i,k_inv_layers(i,k))-(800.-delp))
         ENDDO
         k800=minloc(abs(distance),1)        
         
         !- this is for mid/congestus convection k500
         DO k=kts,kte
           distance(k)=abs(po_cup(i,k_inv_layers(i,k))-(550.-delp))
         ENDDO
         k550=minloc(abs(distance),1)
         ...
         if( k800 <= kts) then 
           ...
           k_inv_layers(i,shal)=k_inv_layers(i,k800) +extralayer            
         endif
         ...
         if( k550 <= kts) then 
            ...
            k_inv_layers(i,mid )=k_inv_layers(i,k550) +extralayer
          endif
...
!------------------------------------------------------------------------------------
  ...
  SUBROUTINE cup_cloud_limits(name,ierrc,ierr,cap_inc,cap_max,entr_rate              &
                             ,heo_cup,heso_cup,qo_cup,qeso_cup,po_cup,z_cup,heo,hkbo &
                             ,entr_rate_2d,hcot,k22,kbmax,klcl,kbcon,ktop            &
                             ,use_excess,zqexec,ztexec &
                             ,itf,ktf,its,ite, kts,kte)
     ...
     real,    dimension (its:ite,kts:kte)  ,intent (in   )     ::    &
        heo_cup,heso_cup,po_cup,z_cup,heo,qo_cup,qeso_cup
     real,    dimension (its:ite)          ,intent (in   )     ::    &
        cap_max,cap_inc
     real,    dimension (its:ite)          ,intent (in   )     ::    &
        zqexec,ztexec
...
!------------------------------------------------------------------------------------
     subroutine get_bouyancy(itf,ktf, its,ite, kts,kte,ierr,klcl,kbcon,ktop &
        ...
        do i=its,itf
!~ write(8,*)"---------------------HCO:",klcl(i),kbcon(i),ktop(i)
          dby (i,:)=0. 
          if(ierr(i) /= 0) cycle      
          do k=kts,klcl(i)
            dby (i,k)=hc (i,k)-he_cup (i,k)
!~ write(8,*)k,hc (i,k),he_cup (i,k),dby (i,k),z_cup(i,k)
          enddo
          DO  k=klcl(i)+1,ktop(i)+1
            dby (i,k)=hc (i,k)-hes_cup (i,k)
!~ write(8,*)k,hc (i,k),hes_cup (i,k),dby (i,k),z_cup(i,k)
          ENDDO
        ENDDO           
...
!------------------------------------------------------------------------------------
  ...
  !  local variables in this routine
     integer                             ::  i,k,k1
     real                                ::  dz,BU,dw2,dw1,kx,dz1m,Tv,Tve
...
!RETURN !<<<<<<<<<<<<
  ...
  !  initialize arrays to zero. 
      ...
      do i=its,itf
        ...
        !print*,"WS0=",kbcon(i)-1,vvel2d(i,kbcon(i)-1),kbcon(i),ktop(i)
...
!---- version 2
 ...
 BU = g*( (Tv-Tve)/Tve - 0.5*(qrco(i,k+1)+qrco(i,k) )) 
...
!2nd term
 ...
 if( vvel2d(i,k+1)< 0.) then 
   vvel2d(i,k+1) = 0.25*vvel2d(i,k)
 endif

!---- version 3
! 2nd term
! kx  = 2.*(1.+beta*C_d)*max(entr_rate_2d(i,k),cd(i,k))*dz
! dw2 =  (vvel2d(i,k))
! dw1 = 2./(f*(1.+gam)) * BU * dz 
! vvel2d(i,k+1)=max(0.,(dw1+dw2))/(1.+kx)
! vvel2d(i,k+1)=(dw1+dw2)/(1.+kx)

!----
                   
          !print*,"ws1=",k,vvel2d(i,k),dw1,dw2,kx
          !print*,"ws2=",k,sqrt(abs(vvel2d(i,k))),entr_rate_2d(i,k),BU,dby(i,k)!cd(i,k)
        enddo loop0
      enddo
...
!        where(vvel2d(i,:) >= 0.) vvel2d(i,:)= sqrt( vvel2d(i,:))
!        where(vvel2d(i,:) <  0.) vvel2d(i,:)=-sqrt(-vvel2d(i,:))
...
!print*,"P3:",xf_dicycle(i),xmb(i);call flush(6)          
          if(xmb(i) .le. 0.)then
               ierr(i)=13; xmb (i)=0.
          endif
      enddo
      !-aplly the scale-dependence Arakawa's approach
      do i=its,itf
          if(ierr(i) /= 0) cycle
          !- scale dependence 
          xmb(i)=sig(i)*xmb(i)
...
!
      DO i=its,itf
        ...
        ENDDO
!print*,"P4:",xmb(i),pre(i);call flush(6)        
