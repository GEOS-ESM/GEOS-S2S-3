! $Id: cloudnew.F90,v 1.44.18.1.2.1.20.1.2.1 2018/06/11 15:23:09 dbarahon Exp $
...
module cloudnew
   ...
   real, allocatable, dimension(:,:), device :: SC_RMFDTR_dev
   real, allocatable, dimension(:,:), device :: SC_QLWDTR_dev
   real, allocatable, dimension(:,:), device :: SC_QIWDTR_dev
   ...
   real, allocatable, dimension(:,:), device :: QRN_SC_dev
   real, allocatable, dimension(:,:), device :: QSN_SC_dev
   ...
   real, allocatable, dimension(:,:), device :: SC_UPDFRC_dev
   ...
   real, allocatable, dimension(:  ), device :: PRESC_dev
   ...
   real, allocatable, dimension(:  ), device :: SCARF_dev
   ...
   real, allocatable, dimension(:  ), device :: SNRSC_dev
   ...
   real, allocatable, dimension(:,:), device :: PFL_SC_dev
   real, allocatable, dimension(:,:), device :: PFI_SC_dev
   ...
   real, allocatable, dimension(:,:), device :: REV_SC_dev
   ...
   real, allocatable, dimension(:,:), device :: RSU_SC_dev
   real, allocatable, dimension(:,:), device :: ACLL_CN_dev
   real, allocatable, dimension(:,:), device :: ACIL_CN_dev
   real, allocatable, dimension(:,:), device :: ACLL_SC_dev
   real, allocatable, dimension(:,:), device :: ACIL_SC_dev
   ...
   real, allocatable, dimension(:,:), device :: VFALLSN_SC_dev
   ...
   real, allocatable, dimension(:,:), device :: VFALLRN_SC_dev
...
#ifdef _CUDA
   attributes(global) subroutine progno_cloud(IRUN,LM,DT,SCLMFDFR,DOSHLW,DOING_GF)
#else
...
!!$         CURAINMOVE_dev, CUSNOWMOVE_dev,   &
      ...
      integer, intent(in   ), value :: DOSHLW
      logical, intent(in   ), value :: DOING_GF
#else
      ...
      real, intent(in   ), dimension(IRUN,  LM) :: SC_RMFDTR_dev  ! MFD_SC
      real, intent(in   ), dimension(IRUN,  LM) :: SC_QLWDTR_dev  ! QLDET_SC
      real, intent(in   ), dimension(IRUN,  LM) :: SC_QIWDTR_dev  ! QIDET_SC
      real, intent(inout), dimension(IRUN,  LM) :: QRN_SC_dev     ! SHLW_PRC3
      real, intent(inout), dimension(IRUN,  LM) :: QSN_SC_dev     ! SHLW_SNO3
      real, intent(inout), dimension(IRUN,  LM) :: SC_UPDFRC_dev  ! UFRC_SC
...
! These are in constant memory in CUDA and are set in the GridComp
      ...
      RUN_LOOP: DO I = 1, IRUN
         ...
         K_LOOP: DO K = 1, LM         
            ...
            IF(.not. DOING_GF) QRN_CU_1D = QRN_CU_dev(I,K)

            MASS =  ( PPE_dev(I,K) - PPE_dev(I,K-1) )*100./MAPL_GRAV  ! layer-mass (kg/m**2)

            iMASS = 1.0 / MASS

            QRN_SC_1D = QRN_SC_dev(I,K)*iMASS
            QSN_SC_1D = QSN_SC_dev(I,K)*iMASS
         ...
         ! RAIN re-evaporation factors
         IF(.NOT. DOING_GF) then
        ...
        ENDIF
            ...
            ELSE
               call RADCOUPLE ( TEMP, PP_dev(I,K), PPE_dev(I,K)-PPE_dev(I,K-1), KH_dev(I,K-1), DTS_dev(I), CLDFRC_dev(I,K), ANVFRC_dev(I,K), &
                     Q_dev(I,K), QLW_LS_dev(I,K), QIW_LS_dev(I,K), QLW_AN_dev(I,K), QIW_AN_dev(I,K), QRN_ALL, QSN_ALL, & 
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
   ...
   subroutine cnvsrc( & 
      ...
      IF(.not. DOING_GF) TE   = TE +   (MAPL_ALHS-MAPL_ALHL) * fQi * TEND*DT/MAPL_CP
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
   ...
   attributes(device) &
#endif
   function LDRADIUS4(PL,TE,QC,NN,ITYPE) RESULT(RADIUS)
       ...
       real, intent(in) :: TE,PL,NN,QC
       INTEGER, intent(in) :: ITYPE
       INTEGER, PARAMETER :: CLOUD=1, ICE=2
       real :: RADIUS
       real :: NNX,RHO,LWL,IWL,BB
       ...
       !- liquid cloud effective radius ----- 
          ...
          lwl = RHO*QC* 1.e+3  !g/m3
      
          !- u[lwl] = g/m3 , u[NNX]= #/cm^3
          ...
          RADIUS= bx *  ( lwl /NNX)**r13bbeta*abeta*6.92 !6.92=(1.e-6)**bbeta	      
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
   ...
   subroutine RADCOUPLE(  &
         ...
         RAD_RL,          & 
         RAD_RI,          & 
         TEMPOR, FRLAND, CNV_FRACTION, SCLMFDFR, &
         DOING_GF   )
      ...
      real, intent(in )  :: tempor, FRLAND, CNV_FRACTION, SCLMFDFR
      logical, INTENT(IN) :: DOING_GF
   ...
   !  AFx  =  AF * SS 
     ...
     ! Number Concentration Assumptions
      NN_LAND  =  30.0e6*CNV_FRACTION + 150.0e6*(1.0-CNV_FRACTION)
      NN_OCEAN =  30.0e6
      ! Number Concentration Assumptions
     ...
     ! LIQUID RADII
      !-BRAMS formulation     
      RAD_RL = LDRADIUS4(PL,TE,RAD_QL,NN,1)
     ! apply limits
    ...
    ! ICE RADII
     !-BRAMS formulation  
      RAD_RI = LDRADIUS4(PL,TE,RAD_QI,NN,2)
    ! apply limits
