...
!
! $Id: wv_saturation.F90,v 1.1.12.2.10.1.8.1.110.1 2017/11/01 13:22:27 narnold1 Exp $
!
...
!!DONIFF Changed to Goff Gatch since that's what we use in GEOS-5(1946?) (05/20/17)
...
if (.false.) then !Murphy and Koop (2005)
!ice 
      if (typ.eq.1) then
...
! Input arguments
!
   real(r8), intent(in) :: t  ,tmelt        ! Temperature, Freezing point
!
! Output arguments
!
...
!!DONIF USe Murphy and Koop (2005) (Written by Andrew Gettelman)
 ...
 function MurphyKoop_svp_water(tx) result(es)
  real(r8), intent(in) :: tx  ! Temperature in Kelvin
  real(r8) :: es             ! SVP in Pa
  real(r8)::  t

  t=min(tx, 332.0_r8)
  t=max(123.0_r8, tx)  !prevent weird behavior
 ...
 function MurphyKoop_svp_ice(tx) result(es)
  ...
  real(r8) :: es             ! SVP in Pa
   ...
   t=max(100.0_r8, tx) !prevent weird behavior
   t=min(274.0_r8, tx)
