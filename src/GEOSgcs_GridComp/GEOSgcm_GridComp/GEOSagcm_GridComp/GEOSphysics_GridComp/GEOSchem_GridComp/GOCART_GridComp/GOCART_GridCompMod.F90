...
!!           short_name .eq. 'NO3AN3'   .or. &
    ...
    if (n_modes > 0 .and. n_aerosols > 0) then
        ...
        call ESMF_ConfigGetAttribute(CF, maxclean, default=5.0e-9, label='MAXCLEAN:', __RC__)
        call ESMF_AttributeSet(aero_aci, name='max_q_clean', value=maxclean, __RC__)
        
        call ESMF_ConfigGetAttribute(CF, CCNtuning, default=2.0, label='CCNTUNING:', __RC__)        
...
!  --------------------------------------------
   call Aero_GridCompRun1 ( gcChem, w_c, impChem, expChem, &
...
!  ---------------------
   ...
   call Aero_GridCompRun2 ( gcChem, w_c, impChem, expChem, &
...
! --------------------------
  mode_ = trim(mode)
  call ESMF_StringLowerCase(mode_, __RC__)
