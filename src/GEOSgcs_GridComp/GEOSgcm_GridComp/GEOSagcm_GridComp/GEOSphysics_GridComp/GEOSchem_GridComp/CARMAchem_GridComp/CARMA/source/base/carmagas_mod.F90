...
contains
  ...
  subroutine CARMAGAS_Create(carma, igas, name, wtmol, ivaprtn, icomposition, rc, shortname, dgc_threshold, ds_threshold)
    ...
    if (carma%f_gas(igas)%f_icomposition == I_GCOMP_H2O)   carma%f_igash2o = igas
    if (carma%f_gas(igas)%f_icomposition == I_GCOMP_H2SO4) carma%f_igash2so4 = igas
    if (carma%f_gas(igas)%f_icomposition == I_GCOMP_SO2)   carma%f_igasso2 = igas
