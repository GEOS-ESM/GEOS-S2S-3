!=============================================================================
!
! $Id: emiss_llnl.F90,v 1.1.1.1.22.1 2009/09/16 17:14:54 enielsen Exp $
!
! CODE DEVELOPER
!   John Tannahill, LLNL
!   jrt@llnl.gov
...
!     ----------------------
      ...
      INTEGER :: inPBL(i1:i2, ju1:j2, k1:k2)
      REAL :: PBLweight(i1:i2, ju1:j2, k1:k2)
      INTEGER :: kDeep, kPBLTop(i1:i2, ju1:j2)
...
!     -------------------------------------------
      ...
      DO ij = ju1,j2
       DO il = i1,i2
        kPBLTop(il,ij) = SUM(inPBL(il,ij,k1:k2))
       END DO
      END DO
...
!     --------------------------------------------------------------------------------
      ...
      mass_pbl(:,:) = mass(:,:,k1)
      DO ik = k1+1, k2
       WHERE(inPBL(:,:,ik) == 1) mass_pbl(:,:) = mass_pbl(:,:)+mass(:,:,ik)
      END DO
...
!     ---------------------------------
      ...
      PBLweight(:,:,:) = 0.00
      DO ik = k1,k2
       WHERE(inPBL(:,:,ik) == 1) PBLweight(:,:,ik) = mass(:,:,ik)/mass_pbl(:,:)
      END DO
...
!         --------------------------------------------------------------------------
	   ...
	   DO ik = k1,kDeep
            emass(:,:,ik) = emissionArray(inum)%pArray3D(:,:,k1) * tdt * PBLweight(:,:,ik)
           END DO
	  ELSE
...
!         -------------------------------------------------------------------
	  ...
	  IF(pr_emiss_3d .AND. .NOT. do_semiss_inchem) THEN
           ...
           IF(emissionSpeciesLayers(icx) == 1) THEN
	    DO ik = k1,kDeep
             emiss_3d_out(:,:,ik,ic) = emiss_3d_out(:,:,ik,ic) + PBLweight(:,:,ik) * &
                             (emissionArray(inum)%pArray3D(:,:,k1)*tdt/mcor(:,:) )
            END DO
           ELSE
	    DO ik = k1,k2
             emiss_3d_out(:,:,ik,ic) = emiss_3d_out(:,:,ik,ic) +  &
                             (emissionArray(inum)%pArray3D(:,:,ik)*tdt/mcor(:,:) )
	    END DO
	   END IF
