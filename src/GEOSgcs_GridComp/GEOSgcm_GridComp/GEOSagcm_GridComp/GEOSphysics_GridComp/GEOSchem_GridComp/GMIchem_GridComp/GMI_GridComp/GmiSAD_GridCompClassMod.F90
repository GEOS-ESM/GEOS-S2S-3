...
! --------------------------------------
   INTEGER, POINTER :: mapSpecies(:)
    ...
    real(KIND=DBL)          :: chem_cycle
    real(KIND=DBL)          :: chemintv
    real(KIND=DBL)          :: rateintv
...
!     -------------------------------------------------------------
      ...
      call ESMF_ConfigGetAttribute(gmiConfigFile, self%chem_cycle, &
     &                label   = "chem_cycle:", &
...
! ---------------------------
    ...
    CALL updateSurfaceAreaDensities (self%rateintv, tropopausePress, press3c, &
