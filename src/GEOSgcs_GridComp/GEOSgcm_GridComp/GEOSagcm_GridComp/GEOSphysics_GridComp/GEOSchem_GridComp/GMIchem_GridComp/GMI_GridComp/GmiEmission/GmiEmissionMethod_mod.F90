...
! !INTERFACE:
!
  subroutine InitializeEmission (self, SpeciesConcentration, gmiGrid, gmiClock, config, &
...
! Emission control routines
     ...
     &           mw, tdt, ndt, self%emiss_timpyr, self%num_emiss,              &
     ...
     &           self%emissionSpeciesLayers, self%exp_fac)
...
! -------------------------------------------------------------------------
     	concentration(ino_num)%pArray3D(:,:,:) = &
     	       concentration(ino_num)%pArray3D(:,:,:) + productionNO(:,:,:) * tdt
