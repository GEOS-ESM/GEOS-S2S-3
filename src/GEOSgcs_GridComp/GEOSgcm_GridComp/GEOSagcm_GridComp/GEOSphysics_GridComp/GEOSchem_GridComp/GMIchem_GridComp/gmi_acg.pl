...
$deposParamFile = "GMI_GridComp/GmiShared/GmiInclude/setkin_depos.h" unless ( $deposParamFile = $opt_d );
$mechParamFile = "GMI_GridComp/GmiShared/GmiInclude/setkin_par.h" unless ( $mechParamFile = $opt_p );
...
# List of specie names begins after "Species indicies:"
# -----------------------------------------------------
...
# Save the specie formula in uppercase
# ------------------------------------
  $m++;
  $l = length( $tokens[2] ) - 4;
...
# Create blank-fill string for each specie.  This 
...
# 0. Determine those species upon which deposition is being performed
# -------------------------------------------------------------------
find_depos();
if ( $opt_R ) { 
 register_exports();
...
# Scan the list of desired exports and continue only if specie(m) found in list
# -----------------------------------------------------------------------------
  $match = 0;
  for $d ( 0..$numDepos-1 ) {
   $deposName = $species[$deposSpecieNumber[$d]];
   if ( $deposName eq $species[$m] ) { $match = 1; }
  }

# Concatenate specie name to process abbreviation, right justified
# ----------------------------------------------------------------
  if ( $match == 1 ) {
...
# Store the specie number is a non-zero value is found
# ----------------------------------------------------
   if ( $value != $zeroString ) { 
    $numDepos++;
    $deposSpecieNumber[$numDepos-1] = $sn;
   }
...
DESCRIPTION
     ...
     Deposition parameters  setkin_depos.h        GmiShared/GmiInclude
     Mechanism parameters   setkin_par.h          GmiShared/GmiInclude
