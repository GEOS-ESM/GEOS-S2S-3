...
!
      ...
      IF ( LUN > 0 ) THEN
         ...
         DO 
            ...
            ! Get next line
            CALL DiagnFileGetNext( am_I_Root, HcoConfig, LUN, &
               cName, SpcName, ExtNr, Cat, Hier, SpaceDim,    &
               OutUnit, EOF, RC, lName=lName, UnitName=UnitName ) 
            ...
            ! Replace any underscores in unit string by spaces
            ...
            CALL MAPL_AddExportSpec(GC,                &
               SHORT_NAME         = ADJUSTL(cName),    &
               LONG_NAME          = ADJUSTL(lName),    &
               UNITS              = ADJUSTL(UnitName), &
               DIMS               = DIMS,              &
               VLOCATION          = VLOC,              &
               RC                 = STATUS              )
            IF ( STATUS /= ESMF_SUCCESS ) THEN
