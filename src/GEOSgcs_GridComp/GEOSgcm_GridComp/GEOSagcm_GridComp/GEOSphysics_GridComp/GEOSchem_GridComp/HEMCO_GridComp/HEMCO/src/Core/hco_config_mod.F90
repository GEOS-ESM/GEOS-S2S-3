...
!
    ...
    DO
       ...
       IF ( DctType == HCO_DCTTYPE_BASE ) THEN       
          ...
          CALL SpecName_Register ( HcoConfig, SpcName, RC )
          IF ( RC /= HCO_SUCCESS ) RETURN
...
!
    ...
    DO WHILE ( FLAG == HCO_SUCCESS ) 
       ...
       ELSE IF ( Lct%Dct%DctType   == HCO_DCTTYPE_MASK .AND. &
          ...
          ! Bug fix: on multi-thread simulations the mask coverage
          ! can cause problems. To get around this simply never allow
          ! mask to have full coverage
