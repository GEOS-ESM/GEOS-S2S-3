...
! Locals
   ...
   type (ESMF_GridComp), pointer :: GCS(:)
    ...
    CHARACTER(LEN=ESMF_MAXSTR) :: specieName(numRATs) = (/ "OX    ", "O3    ", "O3PPMV", "CH4   ", &
...
! ---------------------------------
  DO i = 1, numRATs
   CALL MAPL_AddExportSpec( GC, SHORT_NAME = TRIM(specieName(i)), &
  ...
  IF(MAPL_AM_I_ROOT()) THEN
   ...
   PRINT *, TRIM(Iam)//": RATs Provider List" 
    DO i = 1, numRATs
     PRINT *,"  "//TRIM(specieName(i))//": "//TRIM(RATsProviderName(i))
    END DO
  END IF
...
! -----------------------------------------------------------
  ...
  call ESMF_StringLowerCase(str, __RC__)
...
! -------------------------------
  IF(myState%enable_GOCART) then
     CALL MAPL_AddConnectivity ( GC, &
          SHORT_NAME  = (/'AIRDENS    ','AIRDENS_DRY', 'DELP       ', 'TPREC      ', 'CN_PRCP    ', 'NCN_PRCP   '/), &
...
!                                 'BRCphobic', 'BRCphilic', &
  ...
  IF(myState%enable_TR) then
     CALL MAPL_AddConnectivity ( GC, &
          SHORT_NAME  = (/'AIRDENS    ', 'AIRDENS_DRY', 'DELP       '/), &
  ...
  IF(myState%enable_TR .AND. myState%enable_GMICHEM) then
     ...
     CALL MAPL_AddConnectivity ( GC, &
          SHORT_NAME  = (/'OX    ', 'QQK007', 'QQK027', 'QQK028', 'DD_OX ', 'QQK005', &
                          'QQK204', 'QQK222', 'QQK223', 'QQK224', 'QQK253'/), &
...
!=============================================================================
    ...
    INTEGER, PARAMETER :: numRATs = 8
    CHARACTER(LEN=ESMF_MAXSTR) :: specieName(numRats) = (/"OX    ", "O3    ", "CH4   ", "N2O   ", &
...
!   ----------------------
    ...
    do i = 1, numRATS
       rat => null()
       call MAPL_GetPointer ( EXPORT, rat, trim(specieName(i)), __RC__)
       if ( .not. associated(rat) ) then
          print *, '******** RAT with null pointer: ', trim(specieName(i))
       else
          im = ubound(rat,1)
          jm = ubound(rat,2)
          km = ubound(rat,3)
          ijm = im*jm
          call pmaxmin(specieName(i), rat(:,:,:), qmin, qmax, ijm, km, 1. )
       end if
    end do
#endif
