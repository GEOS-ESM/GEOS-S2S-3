...
! -------------
   ...
   CHARACTER(LEN=ESMF_MAXSTR) :: emisFileName
...
! -------------------
   ...
   CALL I90_label ( 'ODS_emission_filename:', status )
   VERIFY_(status)
   CALL I90_Gtoken( gcSC%emisFileName, status )
   VERIFY_(status)
...
! -----
   ...
   REAL :: dt, qmax, qmin, requiredDt, szan, tropp, tx
...
! -------------
   ...
   REAL(KIND=DBL) ::  hcfc22, hcfc141b, hcfc142b,    co2b,     sf6
...
! ---------------------
  ...
  VERIFY_(status)
  CALL MAPL_GetPointer(impChem,  rhoadry, 'AIRDENS_DRY', RC=status)
  VERIFY_(status)
...
! ---------------------------------------------------
    CALL sza(gcSC%dayOfYear,gcSC%lonRad(i,j),gcSC%latRad(i,j),szan)
...
! If use flux-based boundary conditions, then turn on surface losses for MCF and CCl4
    ...
    IF(gcSC%doFlux) THEN
     ...
     mcfColumn=0.0
     ctcColumn=0.0
     mcfprofile(1:km) = w_c%qa(gcSC%iCH3CCl3)%data3d(i,j,km:1:-1)*numDens(1:km)
     ctcprofile(1:km) = w_c%qa(gcSC%iCCl4)%data3d(i,j,km:1:-1)*numDens(1:km)
     DO k=1,km
...
! -------------------------------------------------------------------
     IF(k == 1) THEN
      CALL SSGSfcFlx(cellDepth(1),cellVolume(1),numDens(1),numDensDry(1),n2o,ch4, &
                     ...
                     chbrcl2,chbr2cl,co2b,sf6,hfc23,hfc32,hfc125,hfc134a,hfc143a, &
...
! ------------------------
     CALL jcalc4(k,szan,O3Column(k),midPress(k),kel(k),aj,gcSC)
     CALL meso_phot(k,km,0,NOColumn,O3Column,O2Column,szan,aj,numDens,gcSC)
...
!-----------------------------------------------------------------------
 ...
 SUBROUTINE SSGSfcFlx(dZ,dV,m,mdry,n2o,ch4,f11,f12,f113,f114,f115,ccl4,ch3ccl3, &
                  ch3cl,ch3br,h1301,h1211,h1202,h2402,hcfc22,hcfc141b,hcfc142b, &
                                 chbr3,ch2br2,ch2brcl,chbrcl2,chbr2cl,co2b,sf6, &
...
! ----------------------------------------------------------------------
  ...
  REAL(KIND=DBL), INTENT(INOUT) :: hcfc22,hcfc141b,hcfc142b,co2b,sf6
...
! -------------------------
  ...
  CALL solverest(dt,requiredDt,examineDt,k,k1Strat,km,daytime,tropo,numDens,aj,ak,br, &
   ...
   qqj001(i,j,kRev) = qqj( 1)
   qqj002(i,j,kRev) = qqj( 2)
   qqj003(i,j,kRev) = qqj( 3)
   qqj004(i,j,kRev) = qqj( 4)
   qqj005(i,j,kRev) = qqj( 5)
   qqj006(i,j,kRev) = qqj( 6)
   qqj007(i,j,kRev) = qqj( 7)
   qqj008(i,j,kRev) = qqj( 8)
   qqj009(i,j,kRev) = qqj( 9)
   qqj010(i,j,kRev) = qqj(10)
   qqj011(i,j,kRev) = qqj(11)
   qqj012(i,j,kRev) = qqj(12)
   qqj013(i,j,kRev) = qqj(13)
   qqj014(i,j,kRev) = qqj(14)
   qqj015(i,j,kRev) = qqj(15)
   qqj016(i,j,kRev) = qqj(16)
   qqj017(i,j,kRev) = qqj(17)
   qqj018(i,j,kRev) = qqj(18)
   qqj019(i,j,kRev) = qqj(19)
   qqj020(i,j,kRev) = qqj(20)

   qqk001(i,j,kRev) = qqk( 1)
   qqk002(i,j,kRev) = qqk( 2)
   qqk003(i,j,kRev) = qqk( 3)
   qqk004(i,j,kRev) = qqk( 4)
   qqk005(i,j,kRev) = qqk( 5)
   qqk006(i,j,kRev) = qqk( 6)
   qqk007(i,j,kRev) = qqk( 7)
   qqk008(i,j,kRev) = qqk( 8)
   qqk009(i,j,kRev) = qqk( 9)
   qqk010(i,j,kRev) = qqk(10)
   qqk011(i,j,kRev) = qqk(11)
   qqk012(i,j,kRev) = qqk(12)
   qqk013(i,j,kRev) = qqk(13)
   qqk014(i,j,kRev) = qqk(14)
   qqk015(i,j,kRev) = qqk(15)
   qqk016(i,j,kRev) = qqk(16)
   qqk017(i,j,kRev) = qqk(17)
   qqk018(i,j,kRev) = qqk(18)
   qqk019(i,j,kRev) = qqk(19)
   qqk020(i,j,kRev) = qqk(20)
   qqk021(i,j,kRev) = qqk(21)
   qqk022(i,j,kRev) = qqk(22)
   qqk023(i,j,kRev) = qqk(23)
   qqk024(i,j,kRev) = qqk(24)
   qqk025(i,j,kRev) = qqk(25)
   qqk026(i,j,kRev) = qqk(26)
   qqk027(i,j,kRev) = qqk(27)
   qqk028(i,j,kRev) = qqk(28)
   qqk029(i,j,kRev) = qqk(29)
   qqk030(i,j,kRev) = qqk(30)
   qqk031(i,j,kRev) = qqk(31)
   qqk032(i,j,kRev) = qqk(32)
   qqk033(i,j,kRev) = qqk(33)
   qqk034(i,j,kRev) = qqk(34)
   qqk035(i,j,kRev) = qqk(35)
