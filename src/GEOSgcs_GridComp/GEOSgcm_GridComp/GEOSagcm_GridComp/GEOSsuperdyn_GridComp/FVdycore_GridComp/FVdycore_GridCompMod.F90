!  $Id: FVdycore_GridCompMod.F90,v 1.268 2015/10/09 17:15:54 ltakacs Exp $
...
!-----------------------------------------------------------------------
  ...
  Module FVdycore_GridCompMod

!BOP
!
! !MODULE: FVdycore_GridCompMod --- FVCAM Dynamical Core Grid Component

    !DESCRIPTION: 
!   This module implements the FVCAM Dynamical Core as
...
! !IROUTINE: SetServices --- Set services for FVCAM Dynamical Core
  ...
  !INTERFACE:
   Subroutine SetServices ( gc, rc )
     ...
     !DESCRIPTION: 
!     Set services (register) for the FVCAM Dynamical Core
!     Grid Component.
!         
!EOP         
!----------------------------------------------------------------------
...
! ----------------------
    ...
    if ( jfirstxy == 1  ) then
         j=1
         tmp2d(:,j) = fvgrid%acap*(  fvgrid%dl*RADIUS * fvgrid%dp*RADIUS)/im
    endif
    if ( jlastxy  == jm ) then
         j=jm
         tmp2d(:,j) = fvgrid%acap*(  fvgrid%dl*RADIUS * fvgrid%dp*RADIUS)/im
...
! we need to check if the grid is OK
  ...
  call ESMF_GRID_INTERIOR(GRID%GRIDXY, ifirstxy, ilastxy, &
                                       jfirstxy, jlastxy )

  call ESMF_GRID_INTERIOR(GRID%GRIDYZ, jfirst, jlast, &
                                       kfirst, klast )
...
! !LOCAL VARIABLES:
    ...
    ns  = max ( 1.0d0, ns )   ! for cases in which dt or dim is too small
    
    init_nsplit = ns
    
    call WRITE_PARALLEL ( ns ,format='("Dynamics    NSPLIT: ",(f7.2),/)' )
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  subroutine Run1(gc, import, export, clock, rc)
   use dynamics_vars, only : a2d3d, d2a3d, c2a3d
   use BlendingMod,   only : blend_wind_height
...
! !Local Variables:
    ...
    real(r8), allocatable :: qsum2 (:,:)   ! Vertically Integrated Variable
    ...
    real(r8), allocatable :: blend_u_f(:,:,:)
    real(r8), allocatable :: blend_v_f(:,:,:)
    real(r8), allocatable :: blend_pt_f(:,:,:)
    real(r8), allocatable :: blend_pe_f(:,:,:)

    real(r8)              :: blend_p_above        ! the highest of the two blending pressure levels
    real(r8)              :: blend_p_below        ! the lowest  of the two blending pressure levels
    ...
    logical LCONSV, LFILL, LBUD
    integer  CONSV,  FILL, NBUD, NBUDS

    parameter                           (nbuds = 19)
    character(len=ESMF_MAXSTR)  budnames(nbuds)
    logical                     budflags(nbuds)

    data budnames   / 'KECDCOR'    , &
                      'PECDCOR'    , &
                      'TECDCOR'    , &
                      'KEREMAP'    , &
                      'PEREMAP'    , &
...
!--------------------------------------------------
      ...
      do k=1,nbuds
      call CheckBud ( EXPORT,trim(budnames(k)),budflags(k),NBUD )
               LBUD = budflags(k) .and. LBUD
      enddo
      ...
      if( LBUD ) then
          ...
          if( NBUD.ne.0 ) then
              if(MAPL_AM_I_ROOT()) then
                 print *
                 print *, "Exact Energy Budgets Require the Following EXPORTS from DYN:"
                 print *, "------------------------------------------------------------"
                 do k=1,nbuds
                    if( budflags(k) ) then 
                        print *, "EXPORT Currently Turned   ON:  ",trim(budnames(k))
                    else
                        print *, "EXPORT Currently Turned  OFF:  ",trim(budnames(k))
                    endif
                 enddo
                 print *
                 print *, "You must ADD    the necessary EXPORTS"
                 print *, "      or DELETE the partial list from your HISTORY"
                 print *
              endif
...
!           ---------------------------------------------------
            ...
            if (rc_blend /= 0) then
                ! copy the forecast variables before they are overwritten by the analysis
                allocate(blend_u_f (ifirstxy:ilastxy,jfirstxy:jlastxy,km))
                allocate(blend_v_f (ifirstxy:ilastxy,jfirstxy:jlastxy,km))
                allocate(blend_pt_f(ifirstxy:ilastxy,jfirstxy:jlastxy,km))
                allocate(blend_pe_f(ifirstxy:ilastxy,jfirstxy:jlastxy,km+1))
...
!-------
            if( trim(uname).ne.'NULL' ) then
                call ESMFL_BundleGetPointertoData(DNS_Bundle,trim(uname),TEMP3D, RC=STATUS)
                if(STATUS==ESMF_SUCCESS) then
                   if(grid%iam==0) print *, 'Replaying ',trim(uname)
                   vars%u = TEMP3D
                endif
            endif
...
!-------
            if( trim(vname).ne.'NULL' ) then
                call ESMFL_BundleGetPointertoData(DNS_Bundle,trim(vname),TEMP3D, RC=STATUS)
                if(STATUS==ESMF_SUCCESS) then
                   if(grid%iam==0) print *, 'Replaying ',trim(vname)
                   vars%v = TEMP3D 
                endif
            endif

! Put A-Grid Wind Fields on the D-grid
! ------------------------------------
            if( trim(rgrid).eq.'A-GRID' .and. trim(uname).ne.'NULL' .and. trim(vname).ne.'NULL' ) then
                if(grid%iam==0) print *, 'Convert A-Grid Winds to D-Grid'
                call a2d3d( state%grid, vars%u, vars%v )
            endif
 
! Pressure
!---------
            if( trim(dpname).ne.'NULL' ) then
                call ESMFL_BundleGetPointertoData(DNS_Bundle,trim(dpname),TEMP3D, RC=STATUS)
                DNS_PRESSURE: if(STATUS==ESMF_SUCCESS) then
                   if(grid%iam==0) print *, 'Replaying ',trim(dpname)
                   vars%pe(:,:,1) = grid%ak(1)
                   do k=2,km+1
...
!-----------------------------------
            if( trim(o3name).ne.'NULL' ) then
                call ESMFL_BundleGetPointertoData(DNS_Bundle,trim(o3name),TEMP3D, RC=STATUS)
...
!---------
            if( trim(qname).ne.'NULL' ) then
                call ESMFL_BundleGetPointertoData(DNS_Bundle,trim(qname),TEMP3D, RC=STATUS)
                DSN_HUMIDITY: if(STATUS==ESMF_SUCCESS) then
                   if(grid%iam==0) print *, 'Replaying ',trim(qname)
                   if( qqq%is_r4 ) then 
...
!------------
            if( trim(tname).ne.'NULL' ) then
                call ESMFL_BundleGetPointertoData(DNS_Bundle,trim(tname),TEMP3D, RC=STATUS)
                DSN_THETAV: if(STATUS==ESMF_SUCCESS) then
                   if(grid%iam==0) print *, 'Replaying ',trim(tname)
                   if( trim(tvar).eq.'THETAV' ) DNS_thv = TEMP3D 
                   if( trim(tvar).eq.'TV'     ) DNS_thv = TEMP3D/vars%pkz
                   if( trim(tvar).eq.'THETA' .or. &
                       trim(tvar).eq.'T'      ) then 
                       if( qqq%is_r4 ) then 
                           if( trim(tvar).eq.'THETA' ) DNS_thv = TEMP3D*(1.0+eps*qqq%content_r4)
                           if( trim(tvar).eq.'T'     ) DNS_thv = TEMP3D*(1.0+eps*qqq%content_r4)/vars%pkz
                       else
                           if( trim(tvar).eq.'THETA' ) DNS_thv = TEMP3D*(1.0+eps*qqq%content   )
                           if( trim(tvar).eq.'T'     ) DNS_thv = TEMP3D*(1.0+eps*qqq%content   )/vars%pkz
                       endif
                   endif
                else
                   if( qqq%is_r4 ) then 
...
! Blend wind and temperature
! --------------------------
            if (rc_blend /= 0) then

                if (grid%iam == 0) then
                    write(*, '(A)') 'INFO::REPLAY::BLENDING: Blending...'
                endif

                ! make sure that p_above <= p_below
                blend_p_above = min(rc_blend_p_above, rc_blend_p_below)
                blend_p_below = max(rc_blend_p_above, rc_blend_p_below)

                call blend_wind_height(blend_u_f, blend_v_f, blend_pt_f, blend_pe_f, &
                                       vars%u, vars%v, vars%pt, vars%pe,             &
                                       blend_p_below, blend_p_above,                 &
                                       ifirstxy, ilastxy, jfirstxy, jlastxy, km)

                deallocate(blend_u_f)
                deallocate(blend_v_f)
                deallocate(blend_pt_f)
                deallocate(blend_pe_f)

                if (grid%iam == 0) then
                    write(*, '(A)') 'INFO::REPLAY::BLENDING: Done.'
                    print *
                endif
...
!           Apply mass fixer tracers after state update
!           -------------------------------------------
...
! ----------------------
    ...
    if(associated(temp2d)) then
         ...
         if ( jfirstxy == 1  ) then
            j=1
            tmp2d(:,j) = grid%acap*(  grid%dl*RADIUS * grid%dp*RADIUS)/im
         endif
         if ( jlastxy  == jm ) then
            j=jm
            tmp2d(:,j) = grid%acap*(  grid%dl*RADIUS * grid%dp*RADIUS)/im
...
! -----------------------------------------------------
    ...
    call Energetics (state,vars%u,vars%v,vars%pt,vars%pe,delp,vars%pkz,phisxy, kenrg0,penrg0,tenrg0,ke=ke,cpt=cpt,gze=gze)
...
! -------------------------
      ...
      call FVdycore_wrapper( phisxy, tempxy, qqq, STATE,    &
                             ...
                             keadv,  kepg, kedp, kehot,     &
                             dthdtremap, dthdtconsv, dtmp,  &
                             P00, PI, CP, KAPPA, OMEGA,     &
                             RADIUS, GRAV, RGAS, EPS,       &
                             NKE, NPHI, LCONSV, LFILL       )
      call MAPL_TimerOff(MAPL,"-WRAPPER")

! Filter EPV Near Poles
! ---------------------
      call MAPL_TimerOn (MAPL,"-DIAG_Polar_FFT")
      call Polar_FFT ( epvxyz,grid%lattice )
      call MAPL_TimerOff(MAPL,"-DIAG_Polar_FFT")
...
! --------------------------
     ...
     do k=1,size(names)
        if( state%vars%tracer(k)%is_r4 ) then 
                     tempxy = state%vars%tracer(k)%content_r4
        else
                     tempxy = state%vars%tracer(k)%content
        endif
        call FILL_Q ( tempxy,delp,names(k),ilastxy-ifirstxy+1,jlastxy-jfirstxy+1,km )
        if( state%vars%tracer(k)%is_r4 ) then 
                     state%vars%tracer(k)%content_r4 = tempxy
        else
                     state%vars%tracer(k)%content    = tempxy
        endif
     enddo
...
!-----------------------------------------------------------------------
    ...
    subroutine CheckBud ( EXPORT,NAME,LBUD,NBUD )
      type (ESMF_State), intent(inout) :: EXPORT
      character(len=*),  intent(in)    :: NAME
      integer,           intent(inout) :: NBUD
      logical,           intent(inout) :: LBUD
      integer       status, rc
      real(kind=4), pointer :: temp2d(:,:)
...
!-----------------------------------------------------------------------
  ...
  subroutine Energetics (state,ud,vd,thv,ple,delp,pk,phiS,keint,peint,teint,ke,cpt,gze)
...
!-----------------------------------------------------------------------
  ...
  subroutine Run2(gc, import, export, clock, rc)
    use dynamics_vars, only : a2d3d, d2a3d
...
! ------------------------------------------
    ...
    call Energetics (state,vars%u,vars%v,thv,vars%pe,dp,vars%pkz,phisxy,kenrg0,penrg0,tenrg0)
...
! ---------------------------------------------------------------------------------------------------
     ...
     allocate( va_yz(im,jfirst:jlast,kfirst:klast) )
     allocate( vd_yz(im,jfirst:jlast,kfirst:klast) )
...
! -------------------------------------
    call a2d3d( state%grid, tend_u, tend_v )
...
! ---------------
    ...
    if ( jn == jm ) then
    do k=KL,KU
         call par_xsum ( state%grid,  DUM(i1:in,jm,k), 1, sumout )
         sumout = sumout/im
         do i=i1,in
         DUM(i,jm,k) = sumout
         enddo
   ...
   ASSERT_(ANY(AK /= 0.0) .or. ANY(BK /= 0.0))
    ...
    subroutine FILL_Q ( Q,DP,NAME,IM,JM,LM )
      ...
      integer num,L,STATUS
          ...
          num = count( q < 0.0_8 )
      if( num.ne.0 ) then
!     write(6,1000) trim(name),num,100.0*float(num)/float(IM*JM*LM)
!1000 format(1x,'Negative values for : ',a,3x,i,2x,'( ',g,' )')
...
!++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    ...
    subroutine Polar_FFT (q,lattice)
    implicit none

    type ( dynamics_lattice_type ) lattice
    real(r8) q( lattice%im(lattice%pei), lattice%jm(lattice%pej), lattice%lm )
    integer i,j,im,jm,img,jmg,lm
    real(r8) PHI,PHIC,DPHI,CC,X
    ...
    allocate ( s(img+2,jm) )
       S = 1.0D0
    DPHI = MAPL_PI_R8/(JMG-1)
    PHIC = MAPL_PI_R8*80.0D0/180.0D0
      DO j=1,JM
         phi = -MAPL_PI_R8*0.5D0 + (lattice%jglobal(j)-1)*DPHI
         cc  = dcos(phi)/dcos(phic)
         DO I=3,IMG+2
            x=(I-1)/2
          S(I,J)= min( cc/dsin(x*mapl_pi_r8/img),1.0D0 )**3
         ENDDO
      ENDDO
    call G3_AVRX (q,im,jm,lm,s,lattice)
    deallocate (s)

    return
    end subroutine Polar_FFT

end module FVdycore_GridCompMod
