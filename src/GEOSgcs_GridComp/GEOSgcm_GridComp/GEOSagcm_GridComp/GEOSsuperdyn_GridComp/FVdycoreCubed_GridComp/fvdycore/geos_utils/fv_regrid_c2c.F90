...
! Range check the MOIST tracers
      ...
      enddo
      !do iq=1,size(extra_rst)
         !if (is_master()) write(*,*)'Results for file: ',trim(extra_rst(iq)%file_name)         
         !do j=1,size(extra_rst(iq)%vars)
            !if (extra_rst(iq)%vars(j)%nLev/=1) then
               !call prt_maxmin('QP_model', extra_rst(iq)%vars(j)%ptr3d(is:ie,js:je,1:npz), is, ie, js, je, 0, extra_rst(iq)%vars(j)%nLev, 1._FVPRC)
            !else
               !call prt_maxmin('QP_model', extra_rst(iq)%vars(j)%ptr2d(is:ie,js:je), is, ie, js, je, 0, 1, 1._FVPRC)
            !end if
         !enddo
      !enddo
