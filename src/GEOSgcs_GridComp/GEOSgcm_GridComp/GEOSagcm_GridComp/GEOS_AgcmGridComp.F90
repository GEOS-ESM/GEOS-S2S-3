! $Id: GEOS_AgcmGridComp.F90,v 1.85.16.3.2.3.4.1.4.1.4.9.2.5.2.1.28.1.4.1 2018/03/28 19:31:29 mathomp4 Exp $
...
! -------------------------------------
  ...
  type CONNECT_ANAnBKG
     private
     type (MAPL_HorzTransform)  :: ANA2BKG
     type (MAPL_HorzTransform)  :: BKG2ANA
...
!-srf-gf-scheme
    call MAPL_AddConnectivity ( GC,                                          &
          SRC_NAME  = (/'DYNF_Q       ','DYNF_T       ','DYNF_PLE     ',     &
                        'DYNF_UA      ','DYNF_VA      ','Q_N          '      &
                      /),                                                    &
          DST_NAME  = (/'DYNF_Q       ','DYNF_T       ','DYNF_PLE     ',     &
                        'DYNF_UA      ','DYNF_VA      ','Q_N          '      &
                      /),                                                    &
          SRC_ID = SDYN,                                                     &
          DST_ID = PHYS,                                                     &
    ...
    call MAPL_AddConnectivity ( GC,              &
         SRC_NAME  = (/'DTDTDYN ','DQVDTDYN' /), &
          DST_NAME = (/'DTDTDYN ','DQVDTDYN' /), &
          SRC_ID = SDYN,                         &
          DST_ID = PHYS,                         &
          RC=STATUS  )
     VERIFY_(STATUS)
...
!-----------------------------------------------------
     ...
     call MAPL_TerminateImport    ( GC,                                                     &
          SHORT_NAME = (/'DUDT  ','DVDT  ','DTDT  ','DPEDT ','DQVANA','DQLANA','DQIANA','DOXANA','PHIS  '/),  &
          CHILD      = SDYN,                                                                &
          RC=STATUS  )
     VERIFY_(STATUS)
             ...
             if(NAMES(K)=='Q') then ! Q
                ...
                if(TYPE  == CORRECTOR) then
                    ...
                    ! ---------------------
                       ...
                       do L=1,lm
                          qdp_bkg(:,:,L) = ( q(:,:,L)+qlls(:,:,L)+qlcn(:,:,L)+qils(:,:,L)+qicn(:,:,L) )*dp(:,:,L)
                       enddo
                    ENDIF
#if debug
                ...
                if(TYPE  == CORRECTOR) then
                    ...
                    ! ---------------------
                       ...
                       do L=1,lm
                          qdp_ana(:,:,L) = ( q(:,:,L)+qlls(:,:,L)+qlcn(:,:,L)+qils(:,:,L)+qicn(:,:,L) )*dp_ana(:,:,L)
                       enddo
...
!-srf-gf-scheme
! - save initial state (before phase 1 of dynamics)
   call DynForcing(1,dt)
!-srf-gf-scheme
...
!-srf-gf-scheme
! - get dynamics forcing for physics (after phase 1 of dynamics)
    call DynForcing(2,dt)
!-srf-gf-scheme
...
!
    ...
    type (MAPL_HorzTransform),pointer :: ANA2BKG => NULL()
    type (MAPL_HorzTransform),pointer :: BKG2ANA => NULL()
...
!   -------------
    ...
    if (.not.myANA%initialized) then
       ...
       if(JMana_World==6*IMana_World) then
         ...
         myANA%GRIDana = AppGridCreateF(IMana_World,JMana_World,LM,NX,NY,STATUS)
         call WRITE_PARALLEL("Created cube myAna%GRIDana...")
       else
         myANA%gridAnaName='PC'//trim(adjustl(imstr))//'x'//trim(adjustl(jmstr))//'-DC'
         myANA%GRIDana = MAPL_LatLonGridCreate (Name=trim(myAna%gridAnaName), &
                                                NX = NX,                &
                                                NY = NY,                &
...
!      -------------------------
      ...
      if (myANA%do_transforms) then
         ...
         call MAPL_HorzTransformCreate( myAna%ANA2BKG, myAna%GRIDana, GRID   , RC=STATUS )
         VERIFY_(STATUS)
         call MAPL_HorzTransformCreate( myAna%BKG2ANA, GRID   , myAna%GRIDana, RC=STATUS )
         VERIFY_(STATUS)
...
!        --------------------------------------------------------------------
         ...
         if (myANA%do_transforms) then
             ...
             qdum2(:,:,1)=phis_bkg
             call MAPL_HorzTransformRun (myANA%BKG2ANA, qdum2, qdum1, RC=STATUS );VERIFY_(STATUS)
    ...
    if (do_transforms) then
        ANA2BKG => myANA%ANA2BKG
        BKG2ANA => myANA%BKG2ANA
    endif
...
!   ------------------------------
    ...
    do ni = 1, nq
       ...
       if (rank==2) then
           ...
           if (fromANA2BKG) then
               qdum2(:,:,1)=aptr2d
               call MAPL_HorzTransformRun (ANA2BKG, qdum2, qdum1, RC=STATUS )
               VERIFY_(STATUS)
               gptr2d=qdum1(:,:,1)
           else
               gptr2d=aptr2d
           ...
           if (fromANA2BKG) then
               call MAPL_HorzTransformRun (ANA2BKG, aptr3d, gptr3d, RC=STATUS )
               VERIFY_(STATUS)
           else
               gptr3d=aptr3d
           endif
...
!   -------
    ...
    if (fromANA2BKG) then
       if( L_CUBE ) then
           call MAPL_HorzTransformRun (ANA2BKG, uptr, vptr, du_aux, dv_aux, RC=STATUS )
           VERIFY_(STATUS)
       else
           call MAPL_HorzTransformRun (ANA2BKG, uptr, du_aux, RC=STATUS )
           VERIFY_(STATUS)
           call MAPL_HorzTransformRun (ANA2BKG, vptr, dv_aux, RC=STATUS )
           VERIFY_(STATUS)
...
! Winds:
          if( L_CUBE ) then
              call MAPL_HorzTransformRun (BKG2ANA, u_bkg, v_bkg, du_aux, dv_aux, RC=STATUS )
              VERIFY_(STATUS)
          else
              call MAPL_HorzTransformRun (BKG2ANA, u_bkg, du_aux, RC=STATUS )
              VERIFY_(STATUS)
              call MAPL_HorzTransformRun (BKG2ANA, v_bkg, dv_aux, RC=STATUS )
              VERIFY_(STATUS)
!             call POLEFIX ( du_aux,dv_aux,VM,myANA%GRIDana )
          endif

! Specific humidity:
          call MAPL_HorzTransformRun (BKG2ANA, q_bkg, dq_aux, RC=STATUS )
          VERIFY_(STATUS)

! Pressure edges:
          call MAPL_HorzTransformRun (BKG2ANA, ple_bkg, dple_aux, RC=STATUS );VERIFY_(STATUS)
...
!              Surface pressure:
               ...
               qdum1(:,:,1)=ps_bkg
               call MAPL_HorzTransformRun (BKG2ANA, qdum1, qdum2, RC=STATUS )
               dps_aux = qdum2(:,:,1)
               deallocate(qdum1)
               deallocate(qdum2)
!              Analyzed pressure edges:
...
! Virtutal Temperature:
          call MAPL_HorzTransformRun (BKG2ANA, tv_bkg, dt_aux, RC=STATUS )
          VERIFY_(STATUS)
! Ozone:
          call MAPL_HorzTransformRun (BKG2ANA, o3_bkg, do3_aux, RC=STATUS )
          VERIFY_(STATUS)
...
!         Bring wind increments from analysis grid to model grid
          if( L_CUBE ) then
              call MAPL_HorzTransformRun (ANA2BKG, du_aux, dv_aux, du_inc, dv_inc, RC=STATUS )
              VERIFY_(STATUS)
          else
              call MAPL_HorzTransformRun (ANA2BKG, du_aux, du_inc, RC=STATUS )
              VERIFY_(STATUS)
              call MAPL_HorzTransformRun (ANA2BKG, dv_aux, dv_inc, RC=STATUS )
              VERIFY_(STATUS)
...
!         Bring specific humidity increment from analysis grid to model grid
          call MAPL_HorzTransformRun (ANA2BKG, dq_aux, dq_inc, RC=STATUS )
...
!            Bring pressure increment from analysis grid to model grid
          ...
          call MAPL_HorzTransformRun (ANA2BKG, dple_aux, dple_inc, RC=STATUS );VERIFY_(STATUS)
...
!         Calculate virtual temperature increment
          ...
          call MAPL_HorzTransformRun (ANA2BKG, dt_aux, dt_inc, RC=STATUS )
!         Convert virtual temperature increment into dry temperature increment
...
!         Bring specific humidity increment from analysis grid to model grid
          call MAPL_HorzTransformRun (ANA2BKG, do3_aux, do3_inc, RC=STATUS )
...
!-srf-gf-scheme
    subroutine DynForcing(task,dt)
      implicit none
      !~ include "mpif.h" !srf, tmp
...
!     real, pointer, dimension(:,:,:)     :: PLE_N    !   => null()
!     real, pointer, dimension(:,:,:)     :: U_N      !   => null()
!     real, pointer, dimension(:,:,:)     :: V_N      !   => null()
!     real, pointer, dimension(:,:,:)     :: T_N      !   => null()
      real, pointer, dimension(:,:,:)     :: Q_N      !   => null()

      real, pointer, dimension(:,:,:)     :: DYNF_Q   ! => null()
      real, pointer, dimension(:,:,:)     :: DYNF_T   ! => null()
      real, pointer, dimension(:,:,:)     :: DYNF_PLE ! => null()
      real, pointer, dimension(:,:,:)     :: DYNF_UA  ! => null()
      real, pointer, dimension(:,:,:)     :: DYNF_VA  ! => null()
    
      !-local
      REAL :: dti
      !
      !-srf-tmp
      integer :: whoami,WANTRANK=1!srf
      !~ call mpi_comm_rank(MPI_COMM_WORLD,whoami,status)!srf
      !~ VERIFY_(STATUS)!srf
      !~ !srf-tmp
    
      dti=1.0/dt
    
      call MAPL_GetPointer( GEX(PHYS), Q,   'Q',   rc=STATUS ); VERIFY_(STATUS)
      call MAPL_GetPointer( GEX(SDYN), T,   'T',   rc=STATUS ); VERIFY_(STATUS)
      call MAPL_GetPointer( GEX(SDYN), PLE, 'PLE', rc=STATUS ); VERIFY_(STATUS)
      call MAPL_GetPointer( GEX(SDYN), U,   'U',   rc=STATUS ); VERIFY_(STATUS)
      call MAPL_GetPointer( GEX(SDYN), V,   'V',   rc=STATUS ); VERIFY_(STATUS)

      call MAPL_GetPointer( GEX(SDYN), Q_N,   'Q_N', ALLOC=.true., rc=STATUS ); VERIFY_(STATUS)
!     call MAPL_GetPointer( GEX(SDYN), T_N,   'T_N',   rc=STATUS ); VERIFY_(STATUS)
!     call MAPL_GetPointer( GEX(SDYN), PLE_N, 'PLE_N', rc=STATUS ); VERIFY_(STATUS)
!     call MAPL_GetPointer( GEX(SDYN), U_N,   'U_N',   rc=STATUS ); VERIFY_(STATUS)
!     call MAPL_GetPointer( GEX(SDYN), V_N,   'V_N',   rc=STATUS ); VERIFY_(STATUS)

      call MAPL_GetPointer( GEX(SDYN), DYNF_Q,   'DYNF_Q', ALLOC=.true., rc=STATUS ); VERIFY_(STATUS)
      call MAPL_GetPointer( GEX(SDYN), DYNF_T,   'DYNF_T', ALLOC=.true., rc=STATUS ); VERIFY_(STATUS)
      call MAPL_GetPointer( GEX(SDYN), DYNF_PLE, 'DYNF_PLE',ALLOC=.true., rc=STATUS ); VERIFY_(STATUS)
      call MAPL_GetPointer( GEX(SDYN), DYNF_UA,  'DYNF_UA', ALLOC=.true., rc=STATUS ); VERIFY_(STATUS)
      call MAPL_GetPointer( GEX(SDYN), DYNF_VA,  'DYNF_VA', ALLOC=.true., rc=STATUS ); VERIFY_(STATUS)

      if(task ==1 )then
       !- saves 'Q' at begin of the time step (before dynamics)
       Q_N      = Q
!      T_N      = T
!      PLE_N    = PLE
!      U_N      = U
!      V_N      = V
       !- 1st part of procedure to save dynamics tendencies
       DYNF_Q   = Q
       DYNF_T   = T
       DYNF_PLE = PLE
       DYNF_UA  = U
       DYNF_VA  = V
!~ if(whoami==WANTRANK) print*,"forcing1",DYNF_Q(1,1,70),Q_N(1,1,70)!,T_N(1,1,70)
!~ if(whoami==WANTRANK) print*,"forcing1",DYNF_T(1,1,70),T(1,1,70),PLE(1,1,70)!;call flush(6)!srf
      else
      !- get tendencies from dynamics only
       DYNF_Q   = dti*( Q   - DYNF_Q)
       DYNF_T   = dti*( T   - DYNF_T)
       DYNF_PLE = dti*( PLE - DYNF_PLE)
       DYNF_UA  = dti*( U   - DYNF_UA)
       DYNF_VA  = dti*( V   - DYNF_VA)
!~ if(whoami==WANTRANK) print*,"forcing2",DYNF_Q(1,1,70),Q_N(1,1,70),Q(1,1,70)!;call flush(6)!srf
!~ if(whoami==WANTRANK) print*,"forcing2",DYNF_T(1,1,70),T(1,1,70),PLE(1,1,70)!;call flush(6)!srf
      endif
!~ ! if( whoami==WANTRANK)     print*,"forcing3-for-scm-only- DUNFQ=",DYNF_Q(1,1,70);call flush(6)!srf
!~ call flush(6)!srf     

  end subroutine DynForcing
!-srf-gf-scheme
