!  $Id: GEOS_mkiauGridComp.F90,v 1.56 2017/07/18 18:58:11 ltakacs Exp $
...
! Local derived type aliases
  ...
  type (MAPL_HorzTransform), pointer  :: ANA2BKG
  type (MAPL_HorzTransform), pointer  :: BKG2ANA
...
! --------------------------------------------------------------------------------------------------------
    ...
    if (refresh_internal_state) then
       if (.not. first) then
          ...
          call WRITE_PARALLEL("Destroying ANA2BKG...")
          call MAPL_HorzTransformDestroy(mkiau_internal_state%ANA2BKG, RC=STATUS )
          VERIFY_(STATUS)
          call WRITE_PARALLEL("Destroying BKG2ANA...")
          call MAPL_HorzTransformDestroy(mkiau_internal_state%BKG2ANA, RC=STATUS )
          VERIFY_(STATUS)
       end if
       ...
       if(JMana_World==6*IMana_World) then
         ...
         GRIDana = AppGridCreateF(IMana_World,JMana_World,LM,NX,NY,STATUS)
          ...
          GRIDana = MAPL_LatLonGridCreate (Name=trim(gridAnaName), &
                                           NX = NX,                & 
                                           NY = NY,                &
...
!      Validate grid
!      -------------
       call ESMF_GridValidate(GRIDana,RC=STATUS)
       VERIFY_(STATUS)
       ...
       call MAPL_HorzTransformCreate( mkiau_internal_state%ANA2BKG, GRIDana, GRIDbkg, Conservative=ana2bkgConsrv, RC=STATUS )
       VERIFY_(STATUS)
       call MAPL_HorzTransformCreate( mkiau_internal_state%BKG2ANA, GRIDbkg, GRIDana, Conservative=bkg2anaConsrv, RC=STATUS )
       VERIFY_(STATUS)
    ...
    ANA2BKG => mkiau_internal_state%ANA2BKG
    BKG2ANA => mkiau_internal_state%BKG2ANA
...
!   -------
    ...
    if (do_transforms) then
       if( L_CUBE ) then
           call MAPL_HorzTransformRun (ANA2BKG, uptr, vptr, du, dv, rotate=.false., RC=STATUS )
           VERIFY_(STATUS)
       else
           call MAPL_HorzTransformRun (ANA2BKG, uptr, du, RC=STATUS )
           VERIFY_(STATUS)
           call MAPL_HorzTransformRun (ANA2BKG, vptr, dv, RC=STATUS )
           VERIFY_(STATUS)
...
! **********************************************************************
    ...
    if ( trim(GRIDINC)=="ANA" .and. do_transforms) then
      if (L_CUBE) then
         call MAPL_HorzTransformRun (BKG2ANA, uptr3d, vptr3d, u_bkg, v_bkg, rotate=.false., RC=STATUS )
         VERIFY_(STATUS)
      else
          call MAPL_HorzTransformRun (BKG2ANA, uptr3d, u_bkg, RC=STATUS )
          VERIFY_(STATUS)
          call MAPL_HorzTransformRun (BKG2ANA, vptr3d, v_bkg, RC=STATUS )
          VERIFY_(STATUS)
    ...
    if (trim(GRIDINC)=="ANA" .and. do_transforms) then
       call MAPL_HorzTransformRun (BKG2ANA, ptr3d, tv_bkg, RC=STATUS )
       VERIFY_(STATUS)
    else
       tv_bkg=ptr3d
    endif
    ...
    if( associated(temp3d) ) temp3d = ptr3d
    if (trim(GRIDINC)=="ANA" .and. do_transforms) then
       call MAPL_HorzTransformRun (BKG2ANA, ptr3d, dp_bkg, RC=STATUS )
       VERIFY_(STATUS)
    else
       dp_bkg=ptr3d
    endif
    ...
    if( associated(temp3d) ) temp3d = ptr3d
    if (trim(GRIDINC)=="ANA" .and. do_transforms) then
       call MAPL_HorzTransformRun (BKG2ANA, ptr3d, o3_bkg, RC=STATUS )
       VERIFY_(STATUS)
    else
        o3_bkg=ptr3d
    endif
    ...
    if (trim(GRIDINC)=="ANA" .and. do_transforms) then
       call MAPL_HorzTransformRun (BKG2ANA, ptr3d, q_bkg, RC=STATUS )
       VERIFY_(STATUS)
    else
       q_bkg=ptr3d
    endif
...
! **********************************************************************
      ...
      if( DAMPBEG.ne.DAMPEND ) then
          ...
          call blend ( ple_ana,u_ana,v_ana,t_ana,q_ana,o3_ana, &
                       ple_bkg,u_bkg,v_bkg,t_bkg,q_bkg,o3_bkg, im,jm,lm, DAMPBEG,DAMPEND )
      else
          if(first .and. MAPL_AM_I_ROOT()) then
...
! **********************************************************************
    ...
    if (trim(GRIDINC)=="ANA" .and. do_transforms) then
       if( L_CUBE ) then
           call MAPL_HorzTransformRun (ANA2BKG, uptr3d, vptr3d, duwindfix, dvwindfix, rotate=.false., RC=STATUS )
           VERIFY_(STATUS)
       else
           call MAPL_HorzTransformRun (ANA2BKG, uptr3d, duwindfix, RC=STATUS )
           VERIFY_(STATUS)
           call MAPL_HorzTransformRun (ANA2BKG, vptr3d, dvwindfix, RC=STATUS )
           VERIFY_(STATUS)
    ...
    if (trim(GRIDINC)=="ANA" .and. do_transforms) then
       if( L_CUBE ) then
           call MAPL_HorzTransformRun (ANA2BKG, uptr3d, vptr3d, du, dv, rotate=.false., RC=STATUS )
           VERIFY_(STATUS)
       else
           call MAPL_HorzTransformRun (ANA2BKG, uptr3d, du, RC=STATUS )
           VERIFY_(STATUS)
           call MAPL_HorzTransformRun (ANA2BKG, vptr3d, dv, RC=STATUS )
           VERIFY_(STATUS)
           call POLEFIX ( du,dv,VM,GRIDbkg )
       endif
    else
...
! -----------
    ...
    if (trim(GRIDINC)=="ANA" .and. do_transforms) then
       call MAPL_HorzTransformRun (ANA2BKG, ptr3d,  dt, RC=STATUS )
       VERIFY_(STATUS)
    else
       dt=ptr3d
    endif
...
! --------
    ...
    if (trim(GRIDINC)=="ANA" .and. do_transforms) then
       call MAPL_HorzTransformRun (ANA2BKG, ptr3d,  dq, RC=STATUS )
       VERIFY_(STATUS)
    else
       dq=ptr3d
    endif
...
! -----
    ...
    if (trim(GRIDINC)=="ANA" .and. do_transforms) then
       call MAPL_HorzTransformRun (ANA2BKG, ptr3d,  do3, RC=STATUS )
       VERIFY_(STATUS)
    else
       do3=ptr3d
    endif
...
! ---
    ...
    if (trim(GRIDINC)=="ANA" .and. do_transforms) then
       call MAPL_HorzTransformRun (ANA2BKG, ptr3d, dple, RC=STATUS )
       VERIFY_(STATUS)
    else
       dple=ptr3d
    endif
...
! get all export names; for each of them get pointer and zero it out
  ...
  subroutine blend ( plea,ua,va,ta,qa,oa, &
                     pleb,ub,vb,tb,qb,ob, im,jm,lm, pabove,pbelow )
      implicit none
      integer    im,jm,lm
