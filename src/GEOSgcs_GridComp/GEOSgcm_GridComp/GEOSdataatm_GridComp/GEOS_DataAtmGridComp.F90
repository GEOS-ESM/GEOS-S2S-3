...
!    VERIFY_(STATUS)
    call MAPL_GetPointer(SurfExport, RH, 'RH2M', RC=STATUS)
    VERIFY_(STATUS)
...
!    if(MAPL_AM_I_ROOT()) print*, 'min max Ed ', minval(Ed), maxval(Ed)
    ...
    do j = 1, JM
      do i = 1, IM
        if (ZTH(i,j) > 0.0)then
          ...
          wspd = SPEED(i,j)
          ozone = OZ(i,j)
          wvapor = WV(i,j)*0.1 !convert from kg m-2 to cm
          relhum = RH(i,j)  !*100.0 !*100 if RH from merra2
          do nl = 1,nlt
