!  $Id: GEOS_DataAtmGridComp.F90,v 1.15.6.11.4.18.2.1.2.1.2.5.2.4.4.5.8.4 2020/08/07 20:29:52 sakella Exp $
...
!--------------------------------------------------
    ...
    call MAPL_GetResource ( STATE, USTAR_MIN, Label="CICE_USTAR_MIN:", DEFAULT=0.005, RC=STATUS)
    VERIFY_(STATUS)
    ...
    VERIFY_(STATUS)
    call MAPL_GetResource ( STATE, DALB_MLT,  Label="CICE_DALB_MLT:",     DEFAULT=-0.025,            RC=STATUS)
    VERIFY_(STATUS)
...
!    allocate(sed_sum(NT),STAT=STATUS);  VERIFY_(STATUS)
    ...
    allocate(TS (NT,NUM_SUBTILES),STAT=STATUS); VERIFY_(STATUS)
    allocate(HH (NT,NUM_SUBTILES),STAT=STATUS); VERIFY_(STATUS)
    allocate(SS (NT,NUM_SUBTILES),STAT=STATUS); VERIFY_(STATUS)
...
! Get parameters
    ...
    Tf = -depressT * SS_FOUNDi  ! default option in CICE 
...
!#endif
    ...
    endwhere
!
!   AOIL behaves as if it is _OFF_
!
    HH(:,WATER) = 0.
    TWMTF       = 0.
    TS(:,WATER) = TS_FOUNDi
...
!#endif
    ...
    do k=1, NT
       ...
       FRZMLT(K) = (TF(K)-(TS(K, WATER)-MAPL_TICE))*SALTWATERCAP*HW_hack/DT  ! should use HH, but HH is set 0 when AOIL is off => FRZMLT = 0.
       FRZMLT(K) = min(max(FRZMLT(K),-FRZMLT_MAX),FRZMLT_MAX)
       if((TS(K, WATER)-MAPL_TICE) < TF(K)) then
          TS(K, WATER) = TF(K) + MAPL_TICE
       endif

       if(FRZMLT(K)<0.0) then ! heat the already existing ice from below
          FRZMLTDB  = FRZMLT(K)
          TSCDB     = TS(K, WATER) - MAPL_TICE 
          TFDB      = TF(K) 
          TAUXBOTDB = TAUXBOT(K)
          TAUYBOTDB = TAUYBOT(K)
          TBOTDB    = TBOT(K) 
...
! units of rain and snow are mm/s should be kg m-2 s-1 (as expected of the export)
   if(associated(AORAIN )) AORAIN  = (rain/1000.) * MAPL_RHO_SEAWATER 
   if(associated(AOSNOW )) AOSNOW  = (snow/1000.) * MAPL_RHO_SEAWATER

! Solar at base of skin layer ignores salt in computing skin layer depth
!  NIR is all absorbed.
!-----------------------------------------------------------------------

    VAR1 = (HH(:, WATER)/MAPL_RHOWTR)
...
! Specific hunidity of saturated air at SST,
    ...
    var1 = 0.98*640380.0*exp(-5107.4/TS_FOUNDi)
    qsat = var1/rhoa                     
...
! Calculate exchange coefs
    ...
    call ncar_ocean_fluxes(sqrt((u10-uw)**2+(v10-vw)**2),&
         & t10, TS_FOUNDi, q10, qsat, var1, var2==1.0,&
         & cd, ch, ce, ustar, var3)
...
! Absolute mass of salt and fresh water in kg / m-2
    ...
    SS(:,WATER)  = .001*SS(:,WATER)*HH(:,WATER)
    SI  = .001*SI *HI

    HH(:,WATER)  = HH(:,WATER) - SS(:,WATER)
    HI  = HI - SI
...
! Update values over water
    ...
    if(associated(AOQFLUX)) AOQFLUX = (1.-FRCICE)*var1

    HH(:,WATER)  = HH(:,WATER) + DT*(rain+rr+(1-FRCICE)*(snow+var1))
    VAR2 = max(min(HH(:,WATER),MAXWATERDEPTH),MINWATERDEPTH)-HH(:,WATER)
    HH(:,WATER)  = HH(:,WATER) + VAR2

! Add salt ( Relaxation to Levitus )

    SSS = .001*SSS
    SS(:,WATER)  = (SS(:,WATER) + (DT/TAUSSS)*SSS*HH(:,WATER)) / (1.0 + (DT/TAUSSS)*(1.-SSS))
...
! 
    ...
    var2 = lwrad - MAPL_STFBOL*(TS(:,WATER))**4 ! Net LW rad, positive down
    var1 = var1+var2
    if(associated(lwnx)) lwnx=(1-FRCICE)*var2
    if(associated(FSURFL)) FSURFL=FSURFL+(1-FRCICE)*var2
    if(associated(AOLWFLX)) AOLWFLX = (1.-FRCICE)*var2
    ...
    var2 = rhoa*MAPL_CP*ustar*ch*cd*(t10-TS(:,WATER)) ! Sensible heat flux, 
                                             ! positive down
    var1 = var1+var2           
    if(associated(shfx)) shfx=(1-FRCICE)*var2
    if(associated(FSURFL)) FSURFL=FSURFL+(1-FRCICE)*var2
    if(associated(AOSHFLX)) AOSHFLX = (1.-FRCICE)*var2

    VAR2 = MAPL_CAPWTR*HH(:,WATER) ! Assume heat capacity of salt = 0.0

    TS(:,WATER)  = TS(:,WATER) + (1-FRCICE)*DT*(var1/VAR2)
...
! Reset mass variables
    ...
    SS(:,WATER)  = 1000.*(SS(:,WATER)/HH(:,WATER)) ! BACK TO PSU
    SI  = 1000.*(SI/HI)
...
!#ifdef USE_R8              
             ...
             PEN(K) = exp(-(KUVR/MAPL_RHOWTR)*HH(K, WATER))
             PUR(K) = DRUVRTHRUDB(1)*PEN(K)
             PUF(K) = DFUVRTHRUDB(1)*PEN(K)
             PEN(K) = exp(-(KPAR(K)/MAPL_RHOWTR)*HH(K, WATER))
...
!#endif
    ...
    DTS =  DT*(FHOCNL+FSWABSUNDICE)/(SALTWATERCAP*HH(:,WATER))
    TS(:,WATER)  =  TS(:,WATER) + DTS 
    ...
    SS(:,WATER) = SS(:,WATER) + DT*FSALTL

    HH(:,WATER)  = HH(:,WATER) + SS(:,WATER)
    SS(:,WATER)  = 1000.*(SS(:,WATER)/HH(:,WATER)) ! BACK TO PSU

    where (SLMASK > 0.5 .and. FRCICE > 0.0)
        SS(:,WATER) = max(min(SS(:,WATER),MAXSALINITY),MINSALINITY)
    endwhere
    if(associated(SSKINW2)) SSKINW2 = SS(:,WATER)
    ...
    if(associated(AODRNIR)) AODRNIR = SWRAD * FRNIRDIR * FR(:,WATER)
    if(associated(AODFNIR)) AODFNIR = SWRAD * FRNIRDIF * FR(:,WATER)
