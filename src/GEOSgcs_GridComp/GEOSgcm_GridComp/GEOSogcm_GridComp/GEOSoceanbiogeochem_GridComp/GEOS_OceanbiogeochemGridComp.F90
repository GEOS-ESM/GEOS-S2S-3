!  $Id: GEOS_OceanbiogeochemGridComp.F90,v 1.5.2.12.8.9.2.8.6.14.2.2.8.5.8.2.10.1.2.1.4.2 2023/06/20 19:19:10 mmehari Exp $
...
!print*, 'CO2SC(im/2,jm/2) aco2', CO2SC(im/2,jm/2), aco2
...
!endif
       ...
       if ( IS_MIDNIGHT ) then
         if(mapl_am_i_root()) write(6,'(I4,A,I3,A,3I2)')           & 
                 YY, "/", DOY, ' ', hr, mn, sec
...
BIO = max(BIO,0.0)    !reduce MOM4 propensity for negative values
    ...
    if ( associated(ppDIATOM) ) &
      where ( MASK(:,:,1) == 0.0 ) ppDIATOM = MAPL_UNDEF
    if ( associated(ppCHLORO) ) &
      where ( MASK(:,:,1) == 0.0 ) ppCHLORO = MAPL_UNDEF
    if ( associated(ppCYANO) ) &
      where ( MASK(:,:,1) == 0.0 ) ppCYANO  = MAPL_UNDEF
    if ( associated(ppCOCCO) ) &
      where ( MASK(:,:,1) == 0.0 ) ppCOCCO  = MAPL_UNDEF
    if ( associated(ppDINO) ) & 
      where ( MASK(:,:,1) == 0.0 ) ppDINO   = MAPL_UNDEF
    if ( associated(ppPHAEO) ) & 
      where ( MASK(:,:,1) == 0.0 ) ppPHAEO  = MAPL_UNDEF
    if ( associated(PCO2) ) &
