...
!   interface to ocean/data_ocean components. 
!   
!EOP
...
! ---------------------
    ...
    if (dual_ocean) then
       OCEAN_NAME="DATASEA"
       OCNd = MAPL_AddChild(GC, NAME=OCEAN_NAME, SS=DataSeaSetServices, RC=STATUS)
       VERIFY_(STATUS)
...
!EOS
    ...
    if(DO_DATASEA==0) then
       call MAPL_TerminateImport    ( GC, SHORT_NAME=(/'TAUX  ','TAUY  ', &
...
!-----------------------------------------------
    call MAPL_GetResource(STATE,DT,  Label="OCEAN_DT:",  RC=STATUS) ! seconds
    VERIFY_(status)
...
!-----------------------------------------
       ...
       call MAPL_GetResource(STATE,DT,  Label="OCEAN_DT:"   ,               RC=STATUS)
       VERIFY_(status)
       call MAPL_GetResource(STATE, SSS_MIN, Label="SSS_MIN:", default=1.e-5, RC=STATUS)
       VERIFY_(status)
