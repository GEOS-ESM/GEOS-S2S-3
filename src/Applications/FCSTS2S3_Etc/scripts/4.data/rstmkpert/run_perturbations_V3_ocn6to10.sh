...
setvars_perturbations(){
    ...
    [[ "${arratm[@]}" =~ "$mmdd" ]] && doATMperts=0 || doATMperts=0 
...
usage="$(basename "$0") -- this is a program to:
        ...
        options:
            -a  run atm perturbation. 1 to run ( default = 1 )
            -o  run ocn perturbation. 1 to run ( default = 1 )
...
if (( $doOCNperts ));then
    ...
    #todo:  copy frand values from the previous stdout file, message_rstmkpert_fcst*. 
    fprevmsg=$dstdout/message_rstmkpert_fcst_$pertdate
    [[ ! -f $fprevmsg ]] && die "File does NOT exist : $fprevmsg" && exit
        
    grep -A11 "OcnPertsV3 - rand_10_pert" $fprevmsg 2>/dev/null | tail -10 > $frand10
    grep -A9  "Ocn sepfac" $fprevmsg 2>/dev/null | tail -n +2 | tr -s '[:space:]' | sed 's#^ ##g' > $focnseps

    wmessage "Note: values for $( basename $frand10 ) and $( basename $focnseps ) files are extracted from :"
    wmessage "   $fprevmsg"
    wmessage 
