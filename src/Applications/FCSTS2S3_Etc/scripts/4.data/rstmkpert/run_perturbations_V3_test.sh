...
setvars_atm(){
    ...
    if [[ "$strexpid" == "test" ]];then 
        fatmalpha10=$datm/alpha10_rho_test_9ddiffs_v3.txt 
    else
...
setvars_ocn() {
    ...
    #07/20/2022 This chunk of code was used for test since rstmkpert kept
    #           failing to create perturbation for a couple of dates.  
    #local _int=$( IndexOf $pertdate ${arrpertdate_exception[@]} )
    #if (( $_int > 0 ));then
    #    #note:  2022/07/06 ocn_alpha10*-m10p.txt did not work. A.Borovikov provided
    #    #                  new set of the files with "m20p
    #    #ocn_alpha10_DJF_9ddiffs_5levs_V3_eh020-m10p.txt
    #    #focnalpha10=$docn/ocn_alpha10_${season}_9ddiffs_5levs_V3_eh020-m10p.txt
    #    #focnalpha10=$docn/ocn_alpha10_${season}_9ddiffs_5levs_V3_eh020-m20p.txt
    #    focnalpha10=$docn/ocn_alpha10_${season}_9ddiffs_5levs_V3_eh020-m90p.txt
    ...
    if [[ "$strexpid" == "test" ]];then 
        focnalpha10=$docn/ocn_alpha10_test_9ddiffs_5levs_V3.txt
    else
        focnalpha10=$docn/ocn_alpha10_${season}_9ddiffs_5levs_V3.txt
    fi
...
mkdate() {
    ...
    if [[ "$strexpid" == "test" ]];then 
        if [[ "$_atmorocn" == "atm" ]];then
            local _ind2beg=($( shuf -i1-30 -n4 ))
            local _ind4seps=($( shuf -i1-9 -n4 ))
            local _seps=$fatmseps
            local _date1=$pertdate 

        elif [[ "$_atmorocn" == "ocn" ]];then
            local _ind2beg=($( shuf -i1-30 -n10 ))
        ...
        if [[ "$_atmorocn" == "atm" ]];then
            local _ind2beg=($( shuf -i1-60 -n4 ))
            local _ind4seps=($( shuf -i1-9 -n4 ))
            local _seps=$fatmseps
            local _date1=$pertdate

        elif [[ "$_atmorocn" == "ocn" ]];then
            local _ind2beg=($( shuf -i1-60 -n10 ))
            
            #abnote:    shuf does not allow repetitions, so need something different for ocn perts
            local _ind4seps=($( seq 10 | xargs -Iz  perl -e 'print int(rand() * 9 + 1) . "\n";' ))
            local _seps=$focnseps
            local _date1=$iyear$( printf '%02g' $imonth )$dumdd
        fi
    fi
    ...
    for i in ${!_ind2beg[@]};do
        ...
        if [[ "$strexpid" == "test" ]];then 
wmessage \@$LINENO $_date1 ${ind}            
            local pertbdates=$( date -d "${_date1} -${ind}days" +%Y%m%d )
        else
...
atm_mkpert(){
    ...
    for i in ${!arratmens[@]};do 
        ...
        local     pd=$( date -d "$rd $_op${difr}days" +%Y%m%d )
        ...
        (( $pertdate < $rd )) && die "rd, $rd, is after $pertdate"
        (( $pertdate < $pd )) && die "pd, $pd, is after $pertdate"
