...
#kntodo:    remove panic stop if exists
if ( -e $EXPDIR/       ) /bin/rm -f $EXPDIR/PANICSTOP
...
# ---------------------------------------------------
set caprealend = `cat $HOMDIR/cap_end | cut -c1-8`
...
>>>COUPLED<<<     @CPEXEC -rf RESTART $EXPDIR
    ...
    set fcap =  `basename $flinkpath`
    
    if ( $fcap == "CAP_0.rc" ) then
        set fcapnext = `basename $flinkpath | sed "s?0?1?" `
        unlink CAP.rc
        ln -s $fcapnext CAP.rc    
        #if ( -e $EXPDIR/CAP_1.rc ) set enddate1 =  `cat $EXPDIR/CAP_1.rc | grep END_DATE: | tr -s '[:space:]' | cut -d' ' -f2 | cut -c1-8`
        set enddate1 =  `cat $EXPDIR/CAP_1.rc | grep END_DATE: | tr -s '[:space:]' | cut -d' ' -f2 | cut -c1-8`
        #echo "knnote: @842 NEW Linked CAP file: $fcapnext"
    else
        set enddate1 = $enddate 
        #echo "knnote: @845 CURRENTLY Linked CAP file: $flinkpath"
    endif
...
if ( $rc == 0 ) then
    ...
    if( $GCMEMIP == TRUE ) then
        ...
        #knnote:    setup archiving
        set  ferr = $EXPDIR/archive/stderr_gcmarch
        set fcomp = $EXPDIR/archive/gcmarch_archcompleted
        set fcomp_del = $EXPDIR/archive/gcmarch_deloutcompleted

        if ( -e $fcomp ) rm -f $fcomp 
        if ( -e $fcomp_del ) rm -f $fcomp_del
        if ( ! -e $ferr ) touch $ferr

        #if( $SITE == NCCS ) then
        #    cd $HOMDIR/archive
        #    sbatch run_gcmarch.sh 
        #    cd  $HOMDIR
        #endif

        #if( $capdate < $enddate ) then 
        if( $capdate < $enddate1 ) then
            #echo "knnote: @876 capdate, enddate,enddate1 = $capdate,$enddate, $enddate1, and sbatch gcm_run.j"
            @BATCH_CMD $HOMDIR/gcm_run.j

        else if( $capdate == $enddate1 ) then
            #echo "knnote: @880 capdate,enddate,enddate1,caprealend = $capdate,$enddate,$enddate1,$caprealend"

            #todo:  edit a symbolic link of CAP.rc for the rest of forecast months (7 months).
            #if( $enddate != $caprealend ) then 
            if( $capdate == $enddate && $enddate == $enddate1 ) then
                #echo "knnote: I am here to change symlinks"
                set flinkpath = `readlink -f CAP.rc`
                set fcapnext = `basename $flinkpath | sed "s?1?2?" `
                unlink CAP.rc
                ln -s $fcapnext CAP.rc    
                
                set flinkpathhis = `readlink -f HISTORY.rc`
                set fhisnext = `basename $flinkpathhis | sed "s?1?2?" `
                unlink HISTORY.rc
                ln -s $fhisnext HISTORY.rc

                #todo:  resubmit even after 3month run
                #note:  RERUN will allow an exp to continue running after the first 3-month segment.
                if( $RERUN == TRUE && $enddate != $caprealend ) then
                    @BATCH_CMD $HOMDIR/gcm_run.j
                endif
            endif
        endif
