...
cp -f $center_fname3d $RECENTER_WRKDIR/center3d.nc
cp -f $center_fname2d $RECENTER_WRKDIR/center2d.nc
if ($logit_transform == 'True') then
    cp -f $center_fname_cice $RECENTER_WRKDIR/center_cice_raw.nc
    ${UMD_LETKFUTILS}/cice_hack.py center_cice_raw.nc center_cice.nc $logit_transform False > cicestuff.out    #logit of AICE
else
    cp -f $center_fname_cice $RECENTER_WRKDIR/center_cice.nc
endif
...
#   eric fix a bug (maybe) NO!
#${UMD_LETKFSCRIPTS}/ocean_recenter.py ${yyyy} ${mm} ${dd} $center_fname3d $center_fname2d $center_fname_cice ${ODAS_Ne} ${TEMP_UNITS}

mkdir recenter
mv -f recenter*.nc recenter
mv -f moments*${yyyy}${mm}${dd}.nc recenter
...
foreach ens_member (`ls -d ${ENSDIR}/recenter2d-${yyyy}${mm}${dd}.*.nc`)
    ...
    cp -f ${RECENTER_WRKDIR}/$tmpcice ./bkg/${ensmdir}/ocean_velocity.res.nc
end
...
foreach ens_member (`ls -d ${ENSDIR}/recenter3d-${yyyy}${mm}${dd}.*.nc`)
    ...
    cp -f ${RECENTER_WRKDIR}/$ens_member ./bkg/${ensmdir}/ocean_temp_salt.res.nc
...
foreach ens_member (`ls -d ${ENSDIR}/recenter2d-${yyyy}${mm}${dd}.*.nc`)
    ...
    #mkdir ./bkg/$ensmdir
    cp -f ${RECENTER_WRKDIR}/$ens_member ./bkg/${ensmdir}/ocean_barotropic.res.nc
end
echo $n members
