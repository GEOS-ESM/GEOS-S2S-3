...
#   fixed to may 2012 1440x721x50 bkg
#    pathname='/gpfsm/dnb42/projects/p17/ehackert/geos5/exp/eh022/ocean_das/BKG_regrid/BKGERR/anom-'+ODAS_Nx+'x'+ODAS_Ny+'x'+ODAS_Nz+'-'+ODAS_GRID_TYPE+'/'+MM+'/'
    flist=glob.glob(pathname+'*'+MM+'.nc')
    flist.sort()
    print('flist is',flist)

#  Bin's fix 3/26/18 to convert Ne environmental var to integer for numeric comparison
    Ne=int(Ne)
    ...
    if (len(flist)<Ne):
        ...
        print('SHOULD NOT BE HERE - NO BKG YET FOR ANYTHING BESIDES MAY')
        EXIT
...
def main():
    ...
    print('Eric Debug', yyyy,mm,dd,center_fname,center_fname2d,center_fname_cice,Ne,Units)
    os.system('pwd')
    ...
    command     = 'cp -f recenter_tmp.nml recenter.nml'
    os.system(command)
...
#       command = RUN_CMD +str(Ne)+' ./ocean_recenter.x '+yyyymmdd+' '+append_to_file+' '+center_fname_cice+' '+str(scaling)+' '+str(inflation)
#       command = 'mpiexec_mpt -np '+str(Ne)+' ./ocean_recenter.x '+yyyymmdd+' '+append_to_file+' '+center_fname_cice+' '+str(scaling)+' '+str(inflation)
        print(command)
        os.system(command)
...
#  Bin's recipe 4119 (/home/bzhao/V3_Recipes)
#       command = RUN_CMD +str(Ne)+' ./ocean_recenter.x '+yyyymmdd+' '+append_to_file+' '+center_fname2d+' '+str(scaling)+' '+str(inflation)
#       command = 'mpiexec_mpt -np '+str(Ne)+' ./ocean_recenter.x '+yyyymmdd+' '+append_to_file+' '+center_fname2d+' '+str(scaling)+' '+str(inflation)
        os.system(command)
        append_to_file='.true.'
...
#   Bin's recipe 4119 (/home/bzhao/V3_Recipes)
#       command = RUN_CMD +str(Ne)+' ./ocean_recenter.x '+yyyymmdd+' '+append_to_file+' '+center_fname+' '+str(scaling)+' '+str(inflation)
#       command = 'mpiexec_mpt -np '+str(Ne)+' ./ocean_recenter.x '+yyyymmdd+' '+append_to_file+' '+center_fname+' '+str(scaling)+' '+str(inflation)
        print(command)
        os.system(command)
        append_to_file='.true.'

#   with open('EnsembleSize.txt', 'wb') as fh:
    with open('EnsembleSize.txt', 'w') as fh:
        fh.write(str(Ne)+'\n')
