...
#   if observers fail in ocean_obs.py bring down everything
set OBSSTATUS = $status
echo 'in ocean_observer.csh after ocean_obs.py', $status
if ( $OBSSTATUS != 0 ) then
    echo 'in ocean_obs.py BAD OBS crash it'
#   rm -f Nobs  #  remove Nobs so  ocean_observer.csh crases
#   rm -f $UMD_OBSERVERDIR'/obs-'${yyyy}${mm}${dd}'_'${hh}'.nc'
#   rm -f $UMD_OBSERVERDIR'/gmao-'${yyyy}${mm}${dd}'_'${hh}'.nc'
    touch $SCRDIR'/OCN_OBS_'${yyyy}${mm}${dd}'_'${hh}   # Tell the scheduler that we're done!
    scancel ${SLURM_JOB_ID}  #  if bad observer, crash model 
    exit (1)
endif
set  Nobs = `cat Nobs`
if ( $Nobs == 0 ) then
    touch $SCRDIR'/OCN_OBS_'${yyyy}${mm}${dd}'_'${hh}   # Tell the scheduler that we're done!
    rm -rf $UMD_OBSERVERDIR
    exit
endif
...
$RUN_CMD ${ODAS_Ne} ./oceanobsop.x -obsin ${yyyy}${mm}${dd}.dat -gues bkg/ # Get omf's
...
# Clean up
#---------
rm -rf $UMD_OBSERVERDIR/bkg
rm -rf $UMD_OBSERVERDIR/ana
rm -rf $UMD_OBSERVERDIR/recenter
rm -rf $UMD_OBSERVERDIR/states
rm -rf $UMD_OBSERVERDIR/tmpcice.*.nc
