...
if(fname == 'center2d.nc'):
 file_input = Dataset(fname,'r+')
#  do add file_input2 to file_input for temperature
 incr_SLV = file_input2.variables['SLV'][:,:,:]
 SLV = file_input.variables['SLV'][:,:,:]

 print('in do_bias_correction.py for center2d.nc')
...
if(fname == 'center_cice.nc'):    # Assumes incr is for AICE currently not implemented
 print(' ICE INCREMENT NOT SUPPORTED')
 print(' INCR has 5 ice types, center_cice.nc has just 1 ')
 exit()
# file_input = Dataset(fname,'r+')
#  do add file_input2 to file_input for temperature
# incr_ice = file_input2.variables['ice'][:,:,:]
# AICE = file_input.variables['AICE'][:,:,:]
#
# print('in do_bias_correction.py for center_cice.nc')
#
##print (AICE.shape)
##print (incr_ice.shape)
#
# AICE2 = AICE[:,:,:]+incr_ice[:,:,:]
# file_input['AICE'][:,:,:] = AICE2[:,:,:,:]
#
##print (AICE.shape)
#print (AICE2)
