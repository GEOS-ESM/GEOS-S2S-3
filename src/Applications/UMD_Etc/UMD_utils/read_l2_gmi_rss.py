...
def gather_l2_gmi_rss(yy_, mm_, dd_, hh_, NDAYS = 0.125, plot = False):
    ...
    if  plot:
        ...
        ax = plt.axes(projection=ccrs.Miller(central_longitude=-80))
        ax.set_global()
        ax.coastlines()    
        ...
        gl.xlabel_style = {'color': 'None'}
        
        c = ax.scatter(lon_[qcf_==1], lat_[qcf_==1], s=4, c=sst_[qcf_==1.], 
                   marker='o', edgecolor='None', vmin=-1., vmax=34., cmap=plt.cm.jet)
        
        fig.colorbar(c)
        ax.set_title(r'GMI L2 SST RSS [%s-%s]'%(t_start.strftime('%Y%m%d_%HUTC'), t_end.strftime('%Y%m%d_%HUTC')))
