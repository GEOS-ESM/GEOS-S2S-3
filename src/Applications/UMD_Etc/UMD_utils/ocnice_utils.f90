...
contains
 ...
 !---------------------------------------------------------------------------------
  ...
  subroutine write_mom_sponge(fname, varname, ogrid, dt)
    ...
    if(varname.eq.'temp')then
     open(10, file='ocean_sst.txt')
     read(10, fmt="(A30)") date_units
     read(10, fmt="(A20)") date_origin
     read(10, fmt=*) Nt    

     allocate(VARtmp(ogrid%Nx, ogrid%Ny, ogrid%Nz, Nt), Time(Nt), X(ogrid%Nx), Y(ogrid%Nx), Z(ogrid%Nx), COEFF(ogrid%Nx, ogrid%Ny, ogrid%Nz), COEFF2(ogrid%Nx, ogrid%Ny, ogrid%Nz), VAR2d(ogrid%Nx, ogrid%Ny), VAR2d2(ogrid%Nx, ogrid%Ny) )
     ...
     if(varname.eq.'salt')then
      open(10, file='ocean_sss.txt')
      read(10, fmt="(A30)") date_units
      read(10, fmt="(A20)") date_origin
      read(10, fmt=*) Nt    
      ...
      data_varname='sss'
      do sss_time_index=0,Nt-1
         read(10, fmt=*)Time(sss_time_index+1)
         read(10, fmt=*)data_filename
         call readrst2d(data_filename, data_varname, VAR2d)
         VARtmp(:,:,1,sss_time_index+1)=VAR2d
      end do
      close(10)
     endif
    endif
 
!  now same for SIC (always get SIC so no relax under ice)
...
!   data_varname='sic'
    ...
    COEFF = 0.0
    COEFF2 = 0.0   ! always make SST COEFF2 = 0.0 so SST relax is avoided
...
!   end where
    ...
    if(varname.eq.'temp')call check( nf90_put_var(ncid, varid, COEFF2) )
    if(varname.eq.'salt')call check( nf90_put_var(ncid, varid, COEFF) )
