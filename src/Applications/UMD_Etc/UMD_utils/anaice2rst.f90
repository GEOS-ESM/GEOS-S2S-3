...
!!$ *
  ...
  character*1028 :: Salt_fname  = 'seaicethermo_internal_rst'
  character*1028 :: Water_fname  = 'saltwater_import_rst'
  character*1028 :: Ocean_fname  = 'ocean_temp_salt.res.nc'
  character*1028 :: Ana_fname  = 'ana.nc4'
  character*1028 :: hice_fname = 'hice.nc4'
  character*1028 :: Outfile  = 'seaicethermo_internal_rst_OUT.nc4'
  character*1028 :: WOutfile  = 'saltwater_import_rst_OUT.nc4'
  ...
  logical       :: DO_HICE_ITD = .false.
  logical       :: DO_HICE_MASK= .false.
  ...
  real*4, allocatable, dimension(:,  :)    :: sit_mask
  ...
  real*8            :: sic_bkg                 ! Background ice Fraction
  real*8            :: sit_bkg                 ! Background ice thickness
  ...
  real*4, parameter :: min_freeze_salinity = 5.0
  ...
  do arg_index = 1, Nargs, 2
     ...
     case('-hice_itd')
        call get_command_argument(arg_index+1, arg2)
        DO_HICE_ITD = str2bool(arg2)
     case('-hice_mask')
        call get_command_argument(arg_index+1, arg2)
        DO_HICE_MASK = str2bool(arg2)
     case('-do_hsno')
        call get_command_argument(arg_index+1, arg2)
  ...
  command = "cp "//trim(Water_fname)//" "//trim(WOutfile)
  call execute_command_line (command, exitstat=i)
...
!            salin(k)=saltmax ! for isosaline ice
  ...
  !=============
  if (DO_AICE_ANA) then
     print*, 'Starting from V3, ODAS will run in dual-ocean mode'
     print*, 'sea ice concentration is nudged to data in the model'
     print*, 'the only data injection that could happen here is ice thickness'
     print*, 'You may want to set ODAS_do_aice to False'
     !print*, '***aborting now****'
     !stop -1  
  ...
  if (DO_HICE_ANA) then     
     allocate(sit_mask(ogrid%Nx, ogrid%Ny))
     sit_mask(:,:) = 1.0
     print *,'Rebalancing state for sea-ice thickness'
     allocate(sit_ana(ogrid%Nx, ogrid%Ny))
     varname = 'HICE'
     status = nf90_open(trim(Ana_fname), nf90_NoWrite, ncid)
     if(status /= nf90_NoErr) then
         print*, 'could not open ', trim(Ana_fname)
         stop
     endif 
     status = nf90_inq_varid(ncid,varname,varidvi) 
     call check( nf90_close(ncid) )
     if(status == nf90_NoErr) then
         call readseaicerst(Ana_fname, varname, ogrid, ana_fmt, sit_ana)
     else
         call readseaicerst(hice_fname, varname, ogrid, ana_fmt, sit_ana)
         if(DO_HICE_MASK) then
              varname = 'MASK'
              call readseaicerst(hice_fname, varname, ogrid, ana_fmt, sit_mask) 
         endif
     endif
     where (sit_ana<0.0)
        sit_ana=0.0
  ...
  varname = 'TS_FOUND'
  call readrst2d(Water_fname, varname, Tw)

  allocate(Sw(oraster%Ntiles, 1))
  varname = 'SS_FOUND'
  call readrst2d(Water_fname, varname, Sw)
  ...
  do i=1,oraster%Ntiles
     if(i==targt) then
         ...
         print*, 'initial aicen: ', (aicen(i,n), n=1,ncat)
  ...
  do i=1,oraster%Ntiles
     ...
     if (DO_AICE_ANA) then
        ...
        !if(abs(sic_bkg-sic_ana(oraster%GIs(i), oraster%GJs(i))) > 0.01) then
        if (DO_ALL_1ST) then
           ...
           enddo 
           if(oob) cycle
           Tf = -depressT * max(Sw(i,1), min_freeze_salinity) ! Linear liquidus approximation (Bitz and Lipscomb, 1999)
           !if(sic_bkg>0.0) then
              if(sic_incr > 0.0) then  ! generate ice
                if(aicen(i,1) > puny) then
                   ...
                   call  restore_ice_state(aicen(i,:),vicen(i,:),vsnon(i,:),&
                                          eicen(i,:,:),esnon(i,:,:), &
                                          nilyr, nslyr, 1, 1, sic_incr, &
                                          hi, hs, qin_save, qsn_save, himin=hinew)  
                else
                   aicen(i,1) = aicen(i,1) + sic_incr
                   vicen(i,1) = aicen(i,1) * hinew
                   ! new ice enthalpy
                   do k=1,nilyr
...
!           if(beta <= beta_max) then 
        ...
        elseif (sic_ana(oraster%GIs(i), oraster%GJs(i)) > puny) then
           ...
           Tf = -depressT * max(Sw(i,1), min_freeze_salinity) ! Linear liquidus approximation (Bitz and Lipscomb, 1999)
           aicen(i,1) = aicen(i,1) + sic_ana(oraster%GIs(i), oraster%GJs(i))
           vicen(i,1) = aicen(i,1) * hinew
           ! new ice enthalpy
           do k=1,nilyr
              qi0 = -rhoi * (cp_ice*(Tmlt(k)-Tf)   &
        ...
        Tf = -depressT * max(Sw(i,1), min_freeze_salinity) + 273.15! Linear liquidus approximation (Bitz and Lipscomb, 1999)

        if( DO_ADHOC_TW ) then
           if ( sum( aicen(i,:) ) > 0.01 .and. sic_bkg < 0.01 ) then
              Tw(i,1) = Tf 
        ...
        if ( (sum( aicen(i,:) ) > 0.01) .and. (sic_bkg>0.01) .and. (sum( aicen(i,:))<0.99) .and. (sic_bkg<0.99) ) then
           Tliquid= ( Tw(i,1) - sic_bkg*Tf )/(1.0-sic_bkg)            ! Keep liquid water temp unchanged
           if(i==targt) then
              print*, Tf, Tliquid, Tw(i,1), sic_bkg
           endif
           Tincr = sum( aicen(i,:) ) * Tf + ( 1.0 - sum( aicen(i,:) ) ) * Tliquid - Tw(i,1) 
           ...
           if(i==targt) then
              print*,  sum(aicen(i,:)), (1.0 - sum( aicen(i,:))), Tw(i,1), Tincr
           endif
           madjice(i) = oraster%wghts(i)
           madjiceij(oraster%GIs(i), oraster%GJs(i)) = &
                        madjiceij(oraster%GIs(i), oraster%GJs(i)) + madjice(i)
  ...
  do i=1,oraster%Ntiles
     ...
     if (DO_HICE_ANA) then   
        if(i==targt) then
            print*, 'lon, lat: ', oraster%lons(i), oraster%lats(i)
            print*, 'before ana_hice aicen: ', (aicen(i,n), n=1,ncat)
            print*, 'before ana_hice vicen: ', (vicen(i,n), n=1,ncat)
            print*, 'before ana_hice MASK: ', sit_mask(oraster%GIs(i), oraster%GJs(i))
        endif
        if(DO_HICE_ITD) then ! distribute thickness difference based on ITDs
           sic_bkg = sum( aicen(i,:) )
           sit_bkg = sum( vicen(i,:) )
           if(sit_mask(oraster%GIs(i), oraster%GJs(i)) > 0.5 .and. sic_bkg > puny) then
              sit_inc = sit_ana(oraster%GIs(i), oraster%GJs(i)) - sit_bkg 
              do n=1, ncat 
                if(aicen(i,n) > puny) then
                   call  save_old_state(aicen(i,:),vicen(i,:),vsnon(i,:),&
                                        eicen(i,:,:),esnon(i,:,:), &
                ...
                vice_new = vicen(i,n) + sit_inc*aicen(i,n)/sic_bkg
                if(aicen(i,n) > puny) then
                    hice_new = vice_new / aicen(i,n)
                    if(hice_new < hin_max(n-1)) then
                       hice_new = hin_max(n-1)
                       if(n==1) then
                          aicen(i,n)   = c0
                          eicen(i,:,n) = c0
                          vsnon(i,n)   = c0
                    ...
                    if(hice_new > hin_max(n)  ) hice_new = hin_max(n)
                    vice_new = hice_new * aicen(i,n)
                endif
                vicen(i,n) = vice_new
                if(aicen(i,n) > puny) then
                     do k=1,nilyr
                        eicen(i,k,n) = qin_save(k) * vicen(i,n)   &
                                       / real(nilyr,kind=dbl_kind)
                     enddo
                endif 
              enddo 
            ...
            if(sit_mask(oraster%GIs(i), oraster%GJs(i)) > 0.5) then
  ...
  !      mass_incr = mass_incr + (beta-1.0)*oraster%areas(i)*rho_ice*sum( vicen(i,:) )   ! Update sea-ice mass increment
        ...
        if(i==targt) then
           print*, 'lon, lat: ', oraster%lons(i), oraster%lats(i)
           print*, 'after ana_hice aicen: ', (aicen(i,n), n=1,ncat)
  ...
  !enddo
  !do i=1,oraster%Ntiles
  !  if(abs(oraster%lons(i)-(-90.0)) < 0.5 .and.  abs(oraster%lats(i)-(60.0)) < 0.5) then
 ...
 !    print *,'Sea-ice mass increment = ',mass_incr,' kg'
  ...
  if (DO_AICE_ANA .or. DO_HICE_ANA) then
  ...
  varname = 'TS_FOUND'
  call check(nf90_open(WOutFile, NF90_WRITE, fid_in) )
  call check(nf90_inq_varid(fid_in, varname, varid))
  call check(nf90_put_var(fid_in, varid, Tw))
  call check(nf90_close(fid_in))  
  ...
  if(allocated(sit_mask)) deallocate(sit_mask)
