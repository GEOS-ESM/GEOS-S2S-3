...
!!$ *        -DO_SPONGE dt_restore_sst dt_restore_sss  : nudge to sst/sss with restoring time scale hh hours
!!$ *
!!$ * INPUT FILES:
...
!!$ * Stuff to do: Save the date of the increment in the file, pass the date as argument ...
  ...
  real*4        :: dt_restore_sst, dt_restore_sss
  ...
  do arg_index = 1, Nargs
     ...
     case('-DO_SPONGE')
        ...
        call get_command_argument(arg_index+2, arg3)
        read(arg3,*)dt_restore_sss
     end select
  enddo
  print *,DO_ANA_INCR
  print *, dt_restore_sst, dt_restore_sss
  ...
  if (DO_SPONGE) then
     print *,'Saving temp sponge'
     outfile='temp_sponge.nc'
     varname='temp'
     call write_mom_sponge(outfile, varname, ogrid, dt_restore_sst)  

     print *,'Saving salt sponge'
     outfile='salt_sponge.nc'
     varname='salt'
     call write_mom_sponge(outfile, varname, ogrid, dt_restore_sss)  
  end if
