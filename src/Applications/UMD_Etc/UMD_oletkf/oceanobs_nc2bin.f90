PROGRAM oceanobs_nc2bin
  ...
  TYPE OceanObs
     ...
     real(r_size), allocatable, dimension(:)  :: instid
  END TYPE OceanObs
  ...
  allocate(obs_data%typ(nobs), obs_data%lon(nobs), obs_data%lat(nobs), obs_data%depth(nobs), obs_data%value(nobs), obs_data%oerr(nobs), obs_data%instid(nobs))    
...
! print *,'r_sngl=',r_sngl
  ...
  call check(nf90_inq_varid(ncid,'instid',varid))
  call check(nf90_get_var(ncid,varid,obs_data%instid))
