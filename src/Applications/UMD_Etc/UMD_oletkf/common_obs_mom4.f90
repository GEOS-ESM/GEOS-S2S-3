...
CONTAINS
  ...
  SUBROUTINE Trans_XtoY(elm,ri,rj,rk,v3d,v2d,yobs)        !(OCEAN)
    ...
    SELECT CASE (intelm)
    CASE(id_u_obs)  ! U
       CALL itpl_3d(v3d(:,:,:,iv3d_u),ri,rj,rk,yobs)  ! <--- No properly wired anymore DO NOT USE (Guillaume)
    CASE(id_aice_obs)  ! AICE
       CALL itpl_2d(v3d(:,:,1,iv3d_u),ri,rj,yobs)      
    CASE(id_hice_obs)  ! HICE
       CALL itpl_2d(v3d(:,:,2,iv3d_u),ri,rj,yobs)
    CASE(id_v_obs)  ! V
...
!
  ...
  ELSEIF(NINT(elem) == id_sst_obs) THEN ! surface observation !(OCEAN)
    rk = 0.0d0
  ELSEIF(NINT(elem) == id_aice_obs) THEN   ! surface observation, u == AICE, HICE, ... !(OCEAN)
    rk = 0.0d0
  ELSEIF(NINT(elem) == id_hice_obs) THEN   ! surface observation, u == AICE, HICE, ... !(OCEAN)
    rk = 0.0d0
...
SUBROUTINE phys2ijk_tri(elem,rlon,rlat,rlev,ri,rj,rk)     !(OCEAN)
!===============================================================================
! Coordinate conversion ... attempt a nearest nei interpolation when in tripolar grid
!===============================================================================
  IMPLICIT NONE
  REAL(r_size),INTENT(IN) :: elem
  REAL(r_size),INTENT(IN) :: rlon
...
! REAL(r_size) :: lnps(nlon,nlat)
  REAL(r_size) :: plev(nlev)
  REAL(r_size) :: test_2dloc(nlon,nlat)  
  integer :: loc2d(2)
  INTEGER :: i,j,k
...
!
  ...
  test_2dloc = 0.0  
  test_2dloc=(lon2d-rlon)**2+(lat2d-rlat)**2
  loc2d=minloc(test_2dloc)
  ri = loc2d(1)
  rj = loc2d(2)

  !print *,'TRYING INTERP IN TRIPOLAR REGION'
  !print *,'ri=',ri,'rj=',rj,'lon='
  !print *,'lon,lat=',lon2d(ri,rj),lat2d(ri,rj)
  !print *,'obs_lon,obs_lat=',rlon,rlat
...
SUBROUTINE read_obs(cfile,nn,elem,rlon,rlat,rlev,odat,oerr,instid,obhr)
!===============================================================================
...
!===============================================================================
  ...
  REAL(r_size),INTENT(OUT) :: instid(nn)
...
SUBROUTINE read_obs2(cfile,nn,elem,rlon,rlat,rlev,odat,oerr,instid,ohx,oqc,obhr)
!===============================================================================
...
!===============================================================================
  ...
  do n=1,nn
    ...
    oerr(n) = REAL(wk(6),r_size)
    instid(n) = REAL(wk(7),r_size)
    ohx(n)  = REAL(wk(8),r_size)
    oqc(n)  = NINT(wk(9))
    if (obs2nrec>9) then
      obhr(n) = REAL(wk(10),r_size)
    endif
  enddo
...
SUBROUTINE write_obs(cfile,nn,elem,rlon,rlat,rlev,odat,oerr,instid,obhr)
!===============================================================================
...
!===============================================================================
  ...
  REAL(r_size),INTENT(IN) :: instid(nn)
...
SUBROUTINE write_obs2(cfile,nn,elem,rlon,rlat,rlev,odat,oerr,instid,ohx,oqc,obhr)
!===============================================================================
...
!===============================================================================
  ...
  do n=1,nn
    ...
    wk(7) = REAL(instid(n),r_sngl)! Instrument ID
    wk(8) = REAL(ohx(n),r_sngl)   ! Model forecast transformed to observation space: H(xb)
    wk(9) = REAL(oqc(n),r_sngl)   ! Quality control ID (1==keep, 0==discard) for use in assimilation
    if (obs2nrec>9) then
      wk(10) = REAL(obhr(n),r_sngl)   ! Quality control ID (1==keep, 0==discard) for use in assimilation
      if (dodebug) PRINT '(I6,2F7.2,F10.2,6F12.2)',NINT(wk(1)),wk(2),wk(3),wk(4),wk(5),wk(6),wk(7),wk(8),wk(9),wk(10)
    else
...
SUBROUTINE write_ods(cfile,nn,elem,rlon,rlat,rlev,odat,oerr,instid,ohx,oqc,obhr)
!===============================================================================
! Write out observations with appended H(xb) for each ob
!===============================================================================
  use netcdf
  ...
  INTEGER :: n,iunit
  LOGICAL, PARAMETER :: dodebug=.false.
  CHARACTER(255) :: ods_fname, varname
  integer ::  Nobs, ncid, varid, obs_dimid, dimids(1)
  integer :: varid_obsid, varid_lon, varid_lat, varid_lev,  varid_obs, varid_sigo, varid_omf, varid_qc, varid_instid

  Nobs = nn

  ods_fname='./'//trim(cfile)//'.nc'
  print *,'ods_fname=',ods_fname
  call check( nf90_create(ods_fname, NF90_CLOBBER, ncid) )
  call check( nf90_def_dim(ncid, "nobs", Nobs, obs_dimid) )
  dimids = (/ obs_dimid /) 

  call check( nf90_def_var(ncid, 'OBSID', NF90_REAL, dimids, varid_obsid) )
  call check( nf90_def_var(ncid, 'lon', NF90_REAL, dimids, varid_lon) )
  call check( nf90_def_var(ncid, 'lat', NF90_REAL, dimids, varid_lat) )
  call check( nf90_def_var(ncid, 'lev', NF90_REAL, dimids, varid_lev) )
  call check( nf90_def_var(ncid, 'obs', NF90_REAL, dimids, varid_obs) )
  call check( nf90_def_var(ncid, 'sigo', NF90_REAL, dimids, varid_sigo) )
  call check( nf90_def_var(ncid, 'instid', NF90_REAL, dimids, varid_instid) )
  call check( nf90_def_var(ncid, 'omf', NF90_REAL, dimids, varid_omf) )
  call check( nf90_def_var(ncid, 'qc', NF90_REAL, dimids, varid_qc) )
  call check( nf90_enddef(ncid) )


  call check( nf90_put_var(ncid, varid_obsid, REAL(elem,r_sngl)) )
  call check( nf90_put_var(ncid, varid_lon, REAL(rlon,r_sngl)) )
  call check( nf90_put_var(ncid, varid_lat, REAL(rlat,r_sngl)) )
  call check( nf90_put_var(ncid, varid_lev, REAL(rlev,r_sngl)) )
  call check( nf90_put_var(ncid, varid_obs, REAL(odat,r_sngl)) )
  call check( nf90_put_var(ncid, varid_sigo, REAL(oerr,r_sngl)) )
  call check( nf90_put_var(ncid, varid_instid, REAL(instid,r_sngl)) )
  call check( nf90_put_var(ncid, varid_omf, REAL(odat-ohx,r_sngl)) )
  call check( nf90_put_var(ncid, varid_qc, REAL(oqc,r_sngl)) )

  call check( nf90_close(ncid) )

END SUBROUTINE write_ods
