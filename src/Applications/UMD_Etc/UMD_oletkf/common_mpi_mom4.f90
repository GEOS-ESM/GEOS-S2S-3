...
!===============================================================================
  ...
  !REAL,INTENT(IN) :: v3dg(nlon,nlat,nlev,nv3d)
  !REAL,INTENT(IN) :: v2dg(nlon,nlat,nv2d)
  !REAL,INTENT(OUT) :: v3d(nij1,nlev,nv3d)
  !REAL,INTENT(OUT) :: v2d(nij1,nv2d)
  !REAL, ALLOCATABLE :: bufs(:,:,:) !(nij1max,nlevall,nprocs)
  !REAL, ALLOCATABLE :: bufr(:,:) !(nij1max,nlevall)
  ...
  DO n=1,nv3d
    DO k=1,nlev
      j = j+1
      !write(nrank+150,*)nrank,j,bufr(1:nij1,j)
  ...
  DO n=1,nv2d
     !print *,'n=',n,' of ',nv2d
    j = j+1
    !print *,nrank,j,bufr(1:nij1,j)
    !write(nrank+100,*)nrank,j,bufr(1:nij1,j)
    !write(nrank+101,*)nrank,j,v2d(:,n)
    !v2d(1:nij1,n) = 0.0    
    !if (n.ne.4) then
    v2d(1:nij1,n) = real(bufr(1:nij1,j),r_size) !real(bufr(1:nij1,j))!0.0 !REAL(bufr(1:nij1,j),r_size)
    !else
    !   v2d(1:nij1,n) = 0.0
    !end if
  END DO
