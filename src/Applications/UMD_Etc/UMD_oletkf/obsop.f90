PROGRAM obsop
  ...
  REAL(r_size), ALLOCATABLE :: rinstid(:)
...
! LOGICAL :: dodebug = .true.
  ...
  !-----------------------------------------------------------------------------
  CALL get_nobs(obsinfile,7,nobs)
  ALLOCATE( elem(nobs) )
  ALLOCATE( rlon(nobs) )
  ALLOCATE( rlat(nobs) )
  ALLOCATE( rlev(nobs) )
  ALLOCATE( odat(nobs) )
  ALLOCATE( oerr(nobs) )
  ALLOCATE( rinstid(nobs) )
  ALLOCATE( ohx(nobs) )
  ALLOCATE( oqc(nobs) )
  ALLOCATE( obhr(nobs) )
  if (obs2nrec==9) then
     CALL read_obs(trim(obsinfile),nobs,elem,rlon,rlat,rlev,odat,oerr,rinstid)
  elseif (obs2nrec==10) then
     CALL read_obs(trim(obsinfile),nobs,elem,rlon,rlat,rlev,odat,oerr,rinstid,obhr)
  else
  ...
  DO n=1,nobs
     ...
     oqc(n) = 1
!  ADDED BY ERIC TO ELIMINATE BIG OMFs (randomly picked 100 for too big omf)
     if(abs(odat(n)-ohx(n)).gt.100.)oqc(n) = 0
  enddo !1:nobs
  ...
  CALL write_obs2(obsoutfile,nobs,elem,rlon,rlat,rlev,odat,oerr,rinstid,ohx,oqc,obhr)
  
  CALL write_ods(obsoutfile,nobs,elem,rlon,rlat,rlev,odat,oerr,rinstid,ohx,oqc,obhr)

  if (ALLOCATED(o3d)) DEALLOCATE(o3d)
  if (ALLOCATED(o2d)) DEALLOCATE(o2d)
  DEALLOCATE( elem,rlon,rlat,rlev,odat,oerr,rinstid,ohx,oqc,obhr,v3d,v2d )
  CALL finalize_mpi
CONTAINS
