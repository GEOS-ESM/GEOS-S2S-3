...
CONTAINS
  ...
  SUBROUTINE obs_local(ij,ilev,var_local,hdxf,rdiag,rloc,dep,nobsl,nobstotal)
    ...
    do n=1,nn
       ...
       if(NINT(oelm) == id_eta_obs) then
          dlev = 0.0d0 ! allow all levels to be influenced by eta
       ...
       horizontal_localization : if (localization_method .eq. 1) then
          ...
          if (.false.) then !(modulo(n,10000) .eq. 0) then
             ...
             print *, "dist = ", dist
             print *, "olon,olat,xlat,xlon,dist",olon,olat,xlat,xlon,dist
             print *, "oelm",oelm
             print *, "=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=" 
          endif
       ...
       !!!!!!!!!!!!!!!!!!!!!!!!!!!!! DANGER !!!!!!!!!!!!!!!!!!!!!!!!!!
       !!!!!!!!!!!!!!!!!!!!!!!!!!!!! DANGER !!!!!!!!!!!!!!!!!!!!!!!!!!
       ! Guillaume: There's a bug in the routines comented out below, discontinuity appear along the meridian in a small box bounded
       !            by ~ Ghana/Nigeria/Cameroon
...
!!$            ) blocked_by_land = .true.
       ...
       observation_localization : if (localization_method .eq. 1 ) then
          ...
          elseif(NINT(oelm) == id_sst_obs) then    ! GUILLAUME: if sst obs ...
             rloc(nobsl) =EXP(-0.5d0 * ((dist/sigma_a)**2 + (dlev/sigma_obsv_sst)**2)) &
                  & * var_local(iobs)
          elseif(NINT(oelm) == id_sss_obs) then    ! ERIC: if sss obs ...
             rloc(nobsl) =EXP(-0.5d0 * ((dist/sigma_a)**2 + (dlev/sigma_obsv_sss)**2)) &
                  & * var_local(iobs)
          elseif(NINT(oelm) == id_eta_obs) then    ! GUILLAUME: if adt obs 1/4 loc scales of prof ... 
             rloc(nobsl) =0.25d0*EXP(-0.5d0 * ((dist/sigma_a)**2 + (dlev/sigma_obsv_sst)**2)) &
                  & * var_local(iobs)
          elseif(NINT(oelm) == id_aice_obs) then    ! GUILLAUME: if aice obs ...
             !rloc(nobsl) =EXP(-0.5d0 * ((dist/(tmperr*sigma_a))**2 ))
             rloc(nobsl) =EXP(-0.5d0 * ((dist/(sigma_a))**2 ))
          else
