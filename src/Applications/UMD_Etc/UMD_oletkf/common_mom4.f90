...
! WRITE(6,*) "phi0(nlon,nlat) = ", phi0(nlon,nlat)
  ...
  ! dx and dy
  !
  !Guillaume: read curvilinear structured grid
...
!===============================================================================
  ...
  DO j=1,nlat
    DO i=1,nlon
      ...
      SSHclm_m(i,j) = 0.0!REAL(buf4(i,j),r_size)
    END DO
  END DO
...
SUBROUTINE read_grd4(infile,v3d,v2d,precision)
  !===============================================================================
  ...
  print *,'tsfile:',tsfile,' iv3d_t=',iv3d_t
...
!!! v
  ...
  v2d(:,:,iv2d_sst) = v3d(:,:,ilev_sfc,iv3d_t) !G's sausaging ....
...
!!$
  altimetry : if(DO_ALTIMETRY) then
     ...
     !print *,'reading eta_t, iv2d_eta=',iv2d_eta,' shape(v2d)=',shape(v2d),' nv2d=',nv2d
!!! SSH
     call check( NF90_INQ_VARID(ncid,'eta_t',varid) )
     !v2d(:,:,iv2d_eta) = 0.0
     call check( NF90_GET_VAR(ncid,varid,v2d(:,:,iv2d_eta)) )

     v2d(:,:,iv2d_ssh) = v2d(:,:,iv2d_eta)
...
!  INCLUDE 'netcdf.inc'
  ...
  REAL(r_size),INTENT(INOUT) :: v3d(nlon,nlat,nlev,nv3d)
  REAL(r_size),INTENT(INOUT) :: v2d(nlon,nlat,nv2d)
...
!!$  call check( NF90_CLOSE(ncid) )
  ...
  if (DO_ALTIMETRY) then
    ...
    DO j=1,nlat
      DO i=1,nlon
        if (kmt(i,j) .ge. 1) buf4(i,j,ilev_sfc) = REAL(v2d(i,j,iv2d_eta),r_sngl) !REAL(v2d(i,j,iv2d_eta)+SSHclm_m(i,j),r_sngl)
      END DO
    END DO
    call check( NF90_PUT_VAR(ncid,varid,buf4(:,:,ilev_sfc)) )
    call check( NF90_CLOSE(ncid) )
...
!!$
!!$
!!$        if (v3d(i,j,k,iv3d_t) < -4.0d0) then
!!$          !WRITE(6,*) "WARNING: Bad temp value in analysis output:"
!!$          !WRITE(6,*) "v3d(",i,",",j,",",k,") = ", v3d(i,j,k,iv3d_t)
!!$          v3d(i,j,k,iv3d_t) = -4.0d0
!!$        endif
!!$
!!$        if (k .eq. 1 .and. v2d(i,j,iv2d_sst) < -4.0d0) v2d(i,j,iv2d_sst) = -4.0d0
!!$
!!$        if (v3d(i,j,k,iv3d_t) > 40.0d0) then
!!$          !WRITE(6,*) "WARNING: Bad temp value in analysis output:"
!!$          !WRITE(6,*) "v3d(",i,",",j,",",k,") = ", v3d(i,j,k,iv3d_t)
!!$          v3d(i,j,k,iv3d_t) = 40.0d0
!!$        endif
!!$
!!$        if (k .eq. 1 .and. v2d(i,j,iv2d_sst) > 40.0d0) v2d(i,j,iv2d_sst) = 40.0d0
!!$
!!$        if (v3d(i,j,k,iv3d_s) < 0.0d0 ) then
!!$          !WRITE(6,*) "WARNING: Bad salt value in analysis output:"
!!$          !WRITE(6,*) "v3d(",i,",",j,",",k,") = ", v3d(i,j,k,iv3d_s)
!!$          v3d(i,j,k,iv3d_s) = 0.0d0
!!$        endif
!!$
!!$        if (k .eq. 1 .and. v2d(i,j,iv2d_sss) < 0.0d0) v2d(i,j,iv2d_sss) = 0.0d0
!!$
!!$        if (v3d(i,j,k,iv3d_s) > 50.0d0) then
!!$          !WRITE(6,*) "WARNING: Bad salt value in analysis output:"
!!$          !WRITE(6,*) "v3d(",i,",",j,",",k,") = ", v3d(i,j,k,iv3d_s)
!!$          v3d(i,j,k,iv3d_s) = 50.0d0
!!$        endif
!!$
!!$        if (k .eq. 1 .and. v2d(i,j,iv2d_sss) > 50.0d0) v2d(i,j,iv2d_sss) = 50.0d0
!!$
!!$      enddo
!!$    enddo
!!$  enddo
     ...
     do k=1,nlev
        do j=1,nlat
           do i=1,nlon
              ...
              if ( isnan( v3d(i,j,k,m) ) ) then
                 if ( wet(i,j).eq.0.0 ) then
                    v3d(i,j,k,m) = 0.0
                    v2d(i,j,:) = 0.0                    
                 else
...
!===============================================================================
  ...
  do k=nlev,1,-1
    do j=1,nlat
      do i=1,nlon
        if ( kmt(i,j) < 1 .and. v3d(i,j,k,3) > 0.0d0 ) then !STEVE: wanted MAX(v3d(i,j,k,:)), but salinity is 35 on land due to unit conversion
          kmt(i,j) = k
        endif
      enddo
    enddo
