MODULE LEVITUS_CLIM_MODULE
  ...
  !
    ...
    fname_lev = '/gpfsm/dnb04/projects/p71/aogcm/g5odas/obs/raw/LEVITUS/levitus.grid.ieee'  
    open  (UNIT_LEV, file=fname_lev(1:len_trim(fname_lev)), status='old', &
                     form='unformatted', convert='big_endian') 
    read  (UNIT_LEV) LevLon, LevLat, LevDepth 
    print *, LevLon(1),LevLon(idim),LevLat(1),LevLat(jdim),LevDepth(1),LevDepth(kdim)
    close (UNIT_LEV)
...
!      fname_lev = '/gpfsm/dnb04/projects/p71/aogcm/g5odas/obs/raw/LEVITUS/2009/Data/ptemp/ptemp.' // mmm
      fname_lev = '/gpfsm/dnb05/projects/p23/lren1/LEVITUS/2018/Data/ptemp/ptemp.' // mmm
      print *, fname_lev
    elseif (svar=='SALT') then
!      fname_lev = '/gpfsm/dnb04/projects/p71/aogcm/g5odas/obs/raw/LEVITUS/2009/Data/salt/salt.' // mmm
      fname_lev = '/gpfsm/dnb05/projects/p23/lren1/LEVITUS/2018/Data/salt/salt.' // mmm
     print *, fname_lev
    else
    ...
    open(UNIT_LEV, file=fname_lev(1:len_trim(fname_lev)),form='unformatted', status='old', convert='little_endian')      
    
    !print *, kdim, shape(Ldata)
    do k=1,kdim
       
       read (UNIT_LEV) LData 
       print *, LData(1,1), LData(200,90)     
  ...
  ! Report on new Data, maxdiff from Profile, and depth of maxdiff
    ...
    i = 1
    !do i=1,Adim
    !print *, 'lev mod ', Adim, shape(Adata), maxL
    do while (Adepth(i) <= maxL .and. i<=Adim)
       ...
       if (abs(Ldata(minj)-Ldata(minj+inc)) >0) then
