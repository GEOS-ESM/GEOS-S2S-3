!DIR$ ID "%Z%	$RCSfile: zeus_clocks.f90,v $ %Y% $Date: 2023/12/05 20:21:25 $ $Revision: 1.1 $"
...
#ifdef _PARALLEL
   use COMMUNICATION_UTILITIES, only : WRITE_PARALLEL
#endif
#define TIME_KIND 8
   ...
   TYPE T_time
     PRIVATE
     real (kind=TIME_KIND) :: value !intern. type for handling time
   END TYPE
   ...
   real (kind=TIME_KIND), parameter :: JULIAN_OFFSET = 1721423.5
   ...
   contains
         ...
         real (kind=TIME_KIND) :: rv 
#include "zeus_clocks.julian_compute.code"
...
#include "zeus_clocks.julian_compute.code"
        ...
        subroutine SECONDS_TO_DATETIME( rv,seconds ) 
         ...
         real (kind=TIME_KIND),intent(in) :: seconds
         
         integer   :: day
         integer   :: month
         integer   :: year
         
         real (kind=TIME_KIND), PARAMETER :: RGREG = 2299161.0 ! Beginning of Gregorian calendar
         integer(kind=8) :: jalpha,ja,jb,jc,jd,je
         real (kind=TIME_KIND) :: julian_day, t
         real (kind=TIME_KIND) :: intt
         real (kind=TIME_KIND) :: julian_second
          ...
          intt = MOD(t, REAL(86400,KIND=TIME_KIND) )
          rv%hour   = intt / 3600.
          rv%minute = MOD(intt/60., REAL(60.,KIND=TIME_KIND))
          rv%second = MOD(intt, REAL(60.,KIND=TIME_KIND))
   ...
   integer,intent(out),optional :: status
     ...
     if ( .not. associated(alarm%clock)  ) then
       if (verbose) then
        call WRITE_PARALLEL("Clock: Alarm has a bad clock pointer ")                
       endif
       ...
       if ( ( alarm%i .gt. size(alarm%clock%alarms)) .or.     &
            ( alarm%i .le. 0)  ) then
         if (verbose) then
          call WRITE_PARALLEL("Clock: Invalid alarm reference  ")
         endif
   ...
   character*132 :: message
     ...
     if ( seconds <= 0. ) then
       if ( present(status) ) then
        ...
        call WRITE_PARALLEL("WARNING: Z_SET_ALARM_INTERVAL")
        call WRITE_PARALLEL("  seconds <= 0. ")
         write(message, &
               FMT='("  seconds = ",E14.7,"  Alarm: ", A60)') &
               seconds,alarm%clock%alarms(alarm%i)%name
        call WRITE_PARALLEL( message )
#else
...
#ifdef _PARALLEL
     call write_Parallel("WARNING: Clock interval should be > 0.")
#else
   ...
   real (kind=TIME_KIND) function Z_JULIAN_DATE_CLOCK(clock, status)
   !*---------------------------------------
   ...
   real (kind=TIME_KIND) function Z_JULIAN_DATE_alarm(alarm, status)
   !*---------------------------------------
   !*-- return the real value of the julian date
   type (T_alarm),intent(in):: alarm
   integer,intent(out),optional :: status
   ...
   real(kind=TIME_KIND)  function Z_SEC_OF_DAY_CLOCK(clock, status)
   !*---------------------------------------------
   ...
   real(kind=TIME_KIND) function Z_SEC_OF_DAY_ALARM(alarm, status)
   !*---------------------------------------------
   !*-- return the time in seconds in start of day
   type (T_alarm),intent(in):: alarm
   integer,intent(out),optional :: status
   ...
   real(kind=TIME_KIND) function Z_SEC_OF_DAY_DATETIME(time, status)
   !*---------------------------------------------
   !*-- return the time in seconds in start of day
   type (T_Datetime),intent(in):: time
   integer,intent(out),optional :: status
...
#ifdef _PARALLEL
    call WRITE_PARALLEL(" ")
    write(UNIT=OUT_LINE, FMT='("Current state of the clock: ", A22)') &
      cz_timeline(clock)
    call WRITE_PARALLEL(OUT_LINE)
    write(UNIT=OUT_LINE, FMT='("  Clock interval = ",F14.1," seconds")') &
...
#ifdef _PARALLEL
       call WRITE_PARALLEL("Interval    Ringing   Origin                    Name")
#else
...
#ifdef _PARALLEL
          write(UNIT=OUT_LINE, FMT='(I8,4X,L7,3X,A22, 5x, A30)') &
   ...
   type (T_Time)     :: tim
   integer :: iy,im
   real(kind=TIME_KIND) :: fracy,fracy1,fracy2
   real(kind=TIME_KIND) :: fracm,fracm1,fracm2
   real(kind=TIME_KIND) :: s2
   ...
   real(kind=4), intent(in) :: b
   type (T_timeInterval) :: rv
   real(kind=TIME_KIND) :: dt
   
    rv%value = MOD(a%value,REAL(b,KIND=TIME_KIND)) 
   ...
   integer, intent(in) :: month
    ...
    !  add the JULIAN_OFSET
         ...
         real (kind = TIME_KIND) function Z_JULIAN_SECOND_TIME (time,status)
         !*---------------------------------------
         ...
         real (kind = TIME_KIND) function Z_JULIAN_SECOND_DATETIME  &
                                                                      (time,status)
         !*---------------------------------------
         !*-- return the real value of the time
         type (T_Datetime),intent(in):: time
         ...
         real (kind = TIME_KIND) function Z_JULIAN_SECOND_CLOCK     &
                                                       (clock, status)
         !*---------------------------------------
         !*-- return the real value of the time
         type (T_Clock),intent(in):: clock
         ...
         real (kind = TIME_KIND) function Z_JULIAN_SECOND_alarm     &
                                                       (alarm, status)
         !*---------------------------------------
         !*-- return the real value of the time
         type (T_alarm),intent(in):: alarm
         ...
         real (kind=TIME_KIND) function Z_DAYAD_TIME (time,status )
         !*---------------------------------------------
         ...
         real (kind=TIME_KIND) function Z_DAYAD_DATETIME ( time, status )
         !*---------------------------------------------
         !*-- return the number of days since 0z 1/1/1
         type (T_Datetime),intent(in) :: time
         integer, intent(inout), optional :: status
         ...
         real (kind=TIME_KIND) function Z_DAYAD_3INT (year,month,day,status)
         !*---------------------------------------------
         !*-- return the number of days since 0z 1/1/1
         ...
         real (kind=TIME_KIND) function Z_DAYAD_6INT ( year, month, day, &
         ...
         real (kind=TIME_KIND) function Z_DAYAD_CLOCK ( clock , status )
         !*---------------------------------------------
         !*-- return the number of days since 0z 1/1/1
         type (T_Clock), intent(in) :: clock
         integer, intent(inout), optional :: status
         ...
         real (kind=TIME_KIND) function Z_DAYAD_ALARM ( alarm , status )
         !*---------------------------------------------
         !*-- return the number of days since 0z 1/1/1
         type (T_Alarm), intent(in) :: alarm
         integer, intent(inout), optional :: status
         ...
         real (kind=TIME_KIND) function Z_SECONDAD_TIME ( time , status )
         !*---------------------------------------------
         ...
         real (kind=TIME_KIND) function Z_SECONDAD_DATETIME (time, status )
         !*---------------------------------------------
         !*-- return the number of seconds since 0z 1/1/1
         type (T_Datetime),intent(in) :: time
         integer, intent(inout), optional :: status
         ...
         real (kind=TIME_KIND) function Z_SECONDAD_3INT            &
         ...
         real (kind=TIME_KIND) function Z_SECONDAD_6INT (year, month, day, &
                                                   hour, minute, second, status ) 
         !*---------------------------------------------
         !*-- return the number of seconds since 0z 1/1/1
         integer, intent(in) :: year, month, day, hour, minute, second
         ...
         real (kind=TIME_KIND) function Z_SECONDAD_CLOCK ( clock , status )
         !*---------------------------------------------
         !*-- return the number of seconds since 0z 1/1/1
         type (T_Clock), intent(in) :: clock
         integer, intent(inout), optional :: status
         ...
         real (kind=TIME_KIND) function Z_SECONDAD_alarm ( alarm , status )
         !*---------------------------------------------
         !*-- return the number of seconds since 0z 1/1/1
         type (T_alarm), intent(in) :: alarm
         integer, intent(inout), optional :: status
         ...
         real (kind=TIME_KIND) function Z_JULIAN_DAY_TIME( time, status )
         !*---------------------------------------------
         ...
         real (kind=TIME_KIND) function Z_JULIAN_DAY_DATETIME(time, status)
         !*---------------------------------------------
         !*-- return the number of julian days since origin
         type (T_Datetime),intent(in) :: time
         integer,intent(out),optional :: status
         ...
         real (kind=TIME_KIND) function Z_JULIAN_DAY_6INT( year,month,day, &
             hour, minute, second, status )
         !*---------------------------------------------
         !*-- return the number of julian days since origin
         integer,intent(in) :: year, month, day, hour, minute, second
         ...
         real (kind=TIME_KIND) function Z_JULIAN_DAY_5INT( year,month,day, &
             hour, minute, second, status )
         !*---------------------------------------------
         !*-- return the number of julian days since origin
         integer,intent(in) :: year, month, day, hour, minute
         ...
         real (kind=TIME_KIND) function Z_JULIAN_DAY_6REAL(year,month,day, &
             hour, minute, second, status )
         !*---------------------------------------------
         !*-- return the number of julian days since origin
         real,intent(in) :: year, month, day, hour, minute
         ...
         real (kind=TIME_KIND) function Z_JULIAN_DAY_CLOCK( clock, status)
         !*---------------------------------------------
         !*-- return the number of julian days since origin
         type (T_Clock), intent(in) :: clock 
         integer,intent(out),optional :: status
         ...
         real (kind=TIME_KIND) function Z_JULIAN_DAY_ALARM( alarm, status)
         !*---------------------------------------------
         !*-- return the number of julian days since origin
         type (T_alarm), intent(in) :: alarm 
         integer,intent(out),optional :: status
