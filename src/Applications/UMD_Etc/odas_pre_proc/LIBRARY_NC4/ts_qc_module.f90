        ...
        if ( abs(dt(k-1)) > ttoli(k-1) ) then
          ...
          elseif ( t(k-1)<= 0.5*(ttoli(k-2)+ttoli(k))/2 ) then
            ...
            if (k /= npts) then ! not last point in profile
              ...
              npts = npts-1
              t(0:k-3)            = t(0:k-3)
              t(k-2:npts-1)       = t(k-1:npts-1)
              s(0:k-3)            = s(0:k-3)
              s(k-2:npts-1)       = s(k-1:npts-1)
              z(0:k-3)            = z(0:k-3)
              z(k-2:npts-1)       = z(k-1:npts-1)
              tqc_lev(0:k-3)      = tqc_lev(0:k-3)
              tqc_lev(k-2:npts-1) = tqc_lev(k-1:npts-1)
              sqc_lev(0:k-3)      = sqc_lev(0:k-3)
              sqc_lev(k-2:npts-1) = sqc_lev(k-1:npts-1)
            endif
          endif
        else  ! No flag set
          !print *, 'No Flag', k
