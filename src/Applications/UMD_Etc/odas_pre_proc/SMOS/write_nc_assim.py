...
def param(inst,var):
	...
	if (inst=='SMAP'):
		...
		param.UNITS      = 'psu'
		param.LONG_NAME  = 'Sea Surface Salinity'
#		param.SOURCE     = 'SMAP V4.0: podaac-ftp.jpl.nasa.gov/allData/smap/L3/JPL/V4/8day_running'
		param.SOURCE     = 'SMAP V2.0: podaac-ftp.jpl.nasa.gov/allData/smap/L3/RSS/V2/8day_running/SCI/'
		param.TITLE      = 'SMAP Sea Surface Salinity'

	if (inst=='SMOS'):
		param.VAR        = 'SSS'
		param.VAR_ID     = 102
		param.INST_ID    = 555 #Track
		param.INST_ERROR = 0.1
		param.UNITS      = 'psu'
		param.LONG_NAME  = 'Sea Surface Salinity'
		param.SOURCE     = 'SMOS L32Q RE06: ftp://ftp.ifremer.fr/Ocean_products/GRIDDED/L32Q/'
		param.TITLE      = 'SMOS Sea Surface Salinity'

	return param


########################################################################
def write_nc_2D(fname,param,N_PROF,DATA_ID,DATE_TIME,LON,LAT,NPTS,QC_FLAG,INST_ID,QC_PRF,DATA,OBS_ERROR,DEPTH,QC_LEV,OBS_BIAS,OBS_XSWATH,TRK_ID):
     ...
     t    = ncfile.createVariable('TRK_ID',np.dtype('i4').char,('N_PROF'))
     t[:] = TRK_ID    
     t.long_name = '0 ascending, 1 descending'
    ...
    ##########################################################################
     ...
     t    = ncfile.createVariable('OBS_BIAS',np.dtype('float32').char,('N_PROF','N_LEVS'))
     t[:] = OBS_BIAS
     t.long_name     = 'Bias'
     t.missing_value = param.MISSING

     t    = ncfile.createVariable('OBS_XSWATH',np.dtype('float32').char,('N_PROF','N_LEVS'))
     t[:] = OBS_XSWATH
     t.long_name     = 'X_Swath'
     t.missing_value = param.MISSING
     t.units         = 'kilometers'
...
########################################################################
def append_nc_2D(fname,param,N_PROF,DATA_ID,DATE_TIME,LON,LAT,NPTS,QC_FLAG,INST_ID,QC_PRF,DATA,OBS_ERROR,DEPTH,QC_LEV,OBS_BIAS,OBS_XSWATH,TRK_ID):
     ...
     trk_id0   = ncfile.variables['TRK_ID']
     ...
     obs_bias0  = ncfile.variables['OBS_BIAS']
     obs_xswath0= ncfile.variables['OBS_XSWATH']
     ...
     obs_bias0[tm0:tm+tm0] = OBS_BIAS[:]
     obs_xswath0[tm0:tm+tm0] = OBS_XSWATH[:]
