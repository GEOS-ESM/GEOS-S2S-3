...
#       print 'max min long is', max, min
    ...
    leva=leva[I]
    instida=instida[I]
    print('length of instid is',len(instid), len(instida),'after sensor trimming')
#       print instid
...
#               print 'lon after',lon[k]
    for k in range(len(lona)):
        if(lona[k] < 0.):
#               print 'lon before', lona[k]
            lona[k]=lona[k]+360.
#               print 'lon after',lona[k]
    ...
    I=np.where((lona>=int(lonw)) & (lona<=int(lone)))
    lona=lona[I]
    lata=lata[I]
    obsida=obsida[I]
    obsa=obsa[I]
    ...
    I=np.where((lata>=int(lats)) & (lata<=int(latn)))
    lona=lona[I]
    lata=lata[I]
    obsida=obsida[I]
    obsa=obsa[I]
...
#
    if len(obsid) == 0:
        ...
        exit()
    if len(obsida) == 0:
        print('NO DATA FOUND DEPTHS for oma pass',levtop,levbot)
        exit()
...
#       flip depth to negative for plotting
    ...
    obsaouts=np.empty(shape=(50),dtype='float')
    ...
    obsaoutn=np.empty(shape=(50),dtype='float')
    ...
    obsaout2=np.empty(shape=(50),dtype='float')
    ...
    obsaout2=[0. for col in range(50)]
...
#               omfmomaoutn[k]=omfmomaoutn[k]+1.
    for i in range(len(oma)):             #  do i=1,nobs
        for k in range(len(depthmod)):    #  do k=0,49
            if (leva[i] >= depthtop[k] and leva[i] < depthbot[k]):
                obsaouts[k] = obsaouts[k] + obsa[i]
                obsaoutn[k]=obsaoutn[k]+1.
...
#       do the averaging here
    for k in range(len(depthmod)):    #  do k=0,49
        ...
        if(omaoutn[k] > 0):
            ...
            omaout2[k] = np.nan
        if(obsaoutn[k] > 0):
            obsaout2[k] = obsaouts[k]/obsaoutn[k]
        else:
            obsaout2[k] = np.nan
    for k in range(len(depthmod)):    #  do k=0,49
        if(omfoutn[k] > 0 and omaoutn[k] > 0):
            omfmomaout2[k] = abs(omfout2[k])-abs(omaout2[k])
        else:
