...
class Ctl(dset.NCDset):
    def __init__(self,exp,collection,freq='MONTHLY'):
        ...
        if freq=='MONTHLY':
            ...
            fmt=fmt.get('MONTHLY',
                        '{data_path}/{collection}/{expid}.{collection}.monthly.{year}{month}.nc4')
            flist=sp.array([fmt.format(data_path=exp.data_path,collection=collection,expid=exp.expid,
                                       year=str(dd.year).zfill(4),month=str(dd.month).zfill(2)) for dd in r[:]])
        elif freq=='DAILY':
            r=rrule.rrule(rrule.DAILY,dtstart=datetime.date(exp.start_year,1,1),until=datetime.date(exp.end_year,12,31))
            fmt=fmt.get('DAILY',
                        '{data_path}/{collection}/{expid}.{collection}.monthly.{year}{month}{day}_1500z.nc4')
            flist=sp.array([fmt.format(data_path=exp.data_path,collection=collection,expid=exp.expid,
                                       year=str(dd.year).zfill(4),month=str(dd.month).zfill(2),
                                       day=str(dd.day).zfill(2)) for dd in r[:]])
    ...
    def __init__(self,exp,collection,freq='MONTHLY'):
        ...
        for fname in self.flist:
            try:
                f=nc.Dataset(fname)
                vlist=f.variables
                if vlist.has_key('LON'):
                    lat=vlist['LAT'][:]
                    lon=vlist['LON'][:]
                elif vlist.has_key('lons'):
                    lat=vlist['lats'][:]
                    lon=vlist['lons'][:]
                f.close()
                break
...
class CtlMOM(dset.NCDset):
    def __init__(self,exp):
        ...
        r=rrule.rrule(rrule.MONTHLY,dtstart=datetime.date(exp.start_year,1,15),until=datetime.date(exp.end_year,12,15))
        fmt=getattr(exp,'fmt',{}).get(collection,
                                    '{data_path}/{collection}/ocean_month.e{year}{month}01_00z.nc')
        flist=sp.array([fmt.format(data_path=exp.data_path,collection=collection,
                                   year=str(dd.year).zfill(4),month=str(dd.month).zfill(2)) for dd in r[:]])
