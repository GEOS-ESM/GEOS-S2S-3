...
# global variables
#-----------------
my ($bcsHEAD, $bcsHEAD_ops, $bcsTagIN, $bcsTagOUT, $bkgFLG, $capture);
my ($debug, $dbHash, $c2cX, $drymassFLG, $ESMABIN, $ESMATAG, $expid);
my ($gcmFLG, $g5modules, $getinput, $getsponsorX);
...
my (%IN, %iceIN, %OUT, %CS, %CSo, %atmLevs, %input_restarts);
...
sub init_tag_arrays_and_hashes {
    ...
    foreach (@INL)   { $landIceVER{$_} = 2; $bcsTAG{$_} = "Icarus-NL_Reynolds" }
    ...
    %rank = ( "Icarus_Ostia"         => 20,
              ...
              "Icarus_Reynolds"      => 18,
              "Icarus-NL_Ostia"      => 17,
              "Icarus-NL_MERRA-2"    => 16,
              "Icarus-NL_Reynolds"   => 15,
    ...
    #------------------------------------------
    $rank_1_catchcn = $rank{"Icarus-NL_Reynolds"};
    $rank_1_route = $rank{"Icarus-NL_Reynolds"};
    $rank_saltwater_split = $rank{"Icarus_Ostia"};
}
...
#=======================================================================
sub set_IN_OUT {
    my ($HH, $bcsTAG, $agrid, $ogrid, $bcsdir, $tile, $topo, $val);
    ...
    foreach $HH (\%IN, \%iceIN, \%OUT) {
        ...
        elsif ($rank{$bcsTAG} >= $rank{"Icarus-NL_Reynolds"}) {
            $bcsdir = "$bcsHEAD/Icarus-NL/$bcsTAG/$gridID";
        }
...
sub regrid_upperair_rsts_CS {
    ...
    my ($DYN, $MOIST, $ACHEM, $AGCM, $AGCMout, $GMICHEM, $GOCART);
...
;
    ...
    if ($slurmjob) {
        print_("\nThe CS regridding is MPI based; found SLURM_JOBID; "
               ."run in interactive batch envirnoment\n");
...
#=======================================================================
sub split_saltwater {
    my ($expid, $template, $tile, $type, $saltwater, $cmd);
    ...
    if ($rst_tarfile) {
        foreach (qw(bkg ana_satb)) {
            ...
            foreach $ana (@ana_tar) {
                ...
                $dbase =~ s/$expid/$newid/ if $expid ne $newid;
                $dest = "$outdir/$dbase";
                copyinput($ana, $dest) unless -e $dest;
            }
        }
    }
}
...
GRID IDENTIFIERS
   ...
   cubed-sphere grids
   ------------------
   Cn, where n = {24, 48, 90, 180, 360, 500, 720, 1000, 1440, 2000, 2880, 5760}
