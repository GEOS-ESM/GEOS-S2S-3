!  $Id: MAPL_VarSpecMod.F90,v 1.32.94.1 2019/01/16 19:22:20 atrayano Exp $
...
!ALT: check ungridded dims
      if (.not.associated(S1%SPECPtr%UNGRIDDED_DIMS)) then
         if (associated(S2%SPECPtr%UNGRIDDED_DIMS)) RETURN
      else
         if (.not.associated(S2%SPECPtr%UNGRIDDED_DIMS)) RETURN
         SS1 = size(S1%SPECPtr%UNGRIDDED_DIMS)
         SS2 = size(S2%SPECPtr%UNGRIDDED_DIMS)
         if (SS1 /= SS2) RETURN
         DO I = 1,SS1
            if (S1%SPECPtr%UNGRIDDED_DIMS(I) /= S2%SPECPtr%UNGRIDDED_DIMS(I)) RETURN
         END DO
      end if
      MAPL_VarSpecEQ = .TRUE.
      RETURN
