!  $Id: MAPL_CFIO.F90,v 1.141.2.6.2.2.2.9.2.10.70.1 2021/03/31 01:14:03 atrayano Exp $
...
!                     ---------------------------------
  ...
  interface MAPL_CFIORead
     ...
     module procedure MAPL_CFIOReadBundle_NoTime
...
!                     ---------------------------------
  ...
  type StoredGlobalCoords
     real(KIND=8), pointer :: LONS2D(:) => NULL()
     real(KIND=8), pointer :: LATS2D(:) => NULL()
...
!-------
    ...
    integer        :: I, J, II, LL, LT, K, LEN, IMO, JMO, LMG, IML, JML
...
!          call Write_Parallel("DEBUG:Storing LAT/LONs")
          ...
          if (.not.foundEmpty) then
             print *,'ERROR: Need bigger table with storedCoords'
             ASSERT_(.false.)
          end if
       end if
    endif
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  subroutine MAPL_CFIOReadBundle_NoTime ( FILE, BUNDLE, RC)
    use netcdf
    implicit none
!
! !ARGUMENTS:
!
    ...
    ! this procedure will open the file, then will extract the time of the
    ! first record, convert to ESMF_Time, and call the "regular"
    ! readBundle routine

    ! Status related vars
    character(len=*), parameter  :: Iam="MAPL_CFIOReadBundle_NoTime"
    integer                      :: STATUS
    ...
    timeId = -1
    status = NF90_INQ_VARID (fid, 'time', timeId)
    VERIFY_(status)

! Paranoic check: just in case status was 0 but timeId stayed negative
    if ( timeId .lt. 0 ) then
       status = -43
!         print *, "could not find time coord"
       RETURN_(status)
    end if

    !     Try assuming this file has been written with CFIO
    !     -------------------------------------------------
    status = NF90_GET_ATT(fid,timeId,'begin_date',begDate)
    if ( status .eq. 0 ) then
       status = NF90_GET_ATT(fid,timeId,'begin_time',begTime)
    end if
    VERIFY_(status)
    ...
    ! parse date
    yy = begDate / 10000
    mm = mod (begDate, 10000) / 100
    dd = mod (begDate, 100)

    ! parse time
    h = begTime / 10000
    m = mod (begTime, 10000) / 100
    s = mod (begTime, 100)

    call ESMF_TimeSet (currTime, YY=yy, MM=mm, DD=dd,          &
         H=h, M=m, S=s, RC=status )
    VERIFY_(status)

    call MAPL_CFIOReadBundle(file, currTime, bundle, RC=status)
    VERIFY_(status)

    RETURN_(ESMF_SUCCESS)
  end subroutine MAPL_CFIOReadBundle_NoTime
...
! Locals
   ...
   !----------------------------------------------
       ...
       call MAPL_CFIORegistryGet(filename,cfio,rc=status)
       VERIFY_(STATUS) 
...
!!!--------------------------------------------------------------------
  ...
  subroutine MAPL_CFIOReadParallel(bundlelist,filelist,time,blocksize,RegridMethod,gsiMode,rc)
     ...
     character(len=*), optional, intent(in) :: regridMethod
     logical, optional, intent(in) :: gsiMode
     integer, optional, intent(out) :: rc
     ...
     integer :: blocksize_
     logical :: gsiMode_
     character(len=ESMF_MAXSTR) :: regridMethod_
     integer, allocatable :: slices(:), psize(:), root(:)
     logical, allocatable :: reading(:)
     integer :: n,nn,n1,n2, nnodes, nfiles, hw
     ...
     allocate(cfio(nfiles),stat=status)
     VERIFY_(STATUS)
     hw=0
     if (present(gsiMode)) then 
     ...
     do while(n1 <=nfiles)
        ...
        do n=n1,n2
           nn=nn+1
           call MAPL_CFIOCreateFromFile(cfio(n),filelist(n),bundlelist(n),regridmethod=regridmethod_,hw=hw,rc=status)
        ...
        do n=n1,n2
           ...
           VERIFY_(status)
           call MAPL_CFIOReadBundleRead(cfio(n),time,hw=hw,rc=status)
           VERIFY_(status)
        enddo
     ...
     IAM = "MAPL_CFIOReadParallel"

     RETURN_(ESMF_SUCCESS)

  end subroutine MAPL_CFIOReadParallel

  subroutine MAPL_CFIOCreateFromFile(MCFIO,filename,bundlein,RegridMethod,cfio_external,hw,only_vars,rc)
    ...
    type(ESMF_CFIO),       optional, intent(INOUT) :: cfio_external
    character(len=*),      optional, intent(IN   ) :: only_vars
    integer,               optional, intent(  OUT) :: RC
    ...
    character(len=ESMF_MAXSTR) :: units,cfiovarname,long_name,gridname, gridnamef,tilefile,regridmethod_,bundlevarname
    integer, allocatable    :: gridToFieldMap(:)
    ...
    type(ESMF_Field) :: field
    integer          :: status
    logical          :: geosgridnames,found
    real, pointer    :: lonsfile(:) => null()
    real, pointer    :: latsfile(:) => null()
    Character(len=ESMF_MAXSTR), parameter :: Iam = "MAPL_CFIOReadBundleRead"

    call ESMF_VMGetCurrent(vm,rc=status)
    VERIFY_(STATUS)
    call ESMF_VMGet(vm,localPet=mcfio%myPE,rc=status)
    VERIFY_(STATUS)
    mcfio%fname=trim(filename)

    if (present(cfio_external)) then
       call ESMF_CFIOSet(mcfio%cfio, fName=trim(fileName), RC=STATUS)
       VERIFY_(STATUS)
       call ESMF_CFIOFileOpen  (mcfio%cfio, FMODE=1, cyclic=.false., RC=STATUS)
       VERIFY_(STATUS)
    else
       mcfio%cfio = ESMF_CFIOCreate (cfioObjName=trim(filename), RC=STATUS)
       VERIFY_(STATUS)
       call ESMF_CFIOSet(mcfio%cfio, fName=trim(fileName), RC=STATUS)
       VERIFY_(STATUS)
       call ESMF_CFIOFileOpen  (mcfio%cfio, FMODE=1, cyclic=.false., RC=STATUS)
    ...
    call ESMF_CFIOGet       (mcfio%CFIO,     grid=CFIOGRID,                     RC=STATUS)
    VERIFY_(STATUS)
    call ESMF_CFIOGridGet   (CFIOGRID, IM=IM, JM=JM, KM=LM,               RC=STATUS)
    VERIFY_(STATUS)
    call ESMF_CFIOGridGet    (CFIOGRID, LON=LONSFILE, LAT=LATSFILE, RC=STATUS)
    ...
    call ESMF_CFIOGet (mcfio%CFIO,varObjs=VARS, nVars=fVars, RC=STATUS)
    VERIFY_(STATUS)
    ...
    if (bvars>0) then
       ...
       nvars=0
       do i=1,fVars
          call ESMF_CFIOVarInfoGet(vars(i),vname=cfiovarname,rc=status)
          VERIFY_(status)
          if ( present(ONLY_VARS) ) then
             if ( index(','//trim(ONLY_VARS)  //',', &
                        ','//trim(CFIOVARNAME)//',') < 1 ) cycle
   ...
   !ALT: for now we add only HorzVert (no tiles)
    ...
    if (trim(regridmethod_)=="VOTING" .or. trim(regridmethod_)=="CONSERVATIVE") then
       call ESMF_GridGet(esmfgrid, name=gridname, rc=rc)
       VERIFY_(STATUS)
       call MAPL_GenGridName(im, jm, LONSfile, LATSfile, gridname=gridnamef, geos_style=geosGridNames)
       if (.not. geosGridNames) call MAPL_GeosNameNew(gridname)
       tileFile=trim(adjustl(gridnamef)) // '_' //trim(adjustl(gridname))  // '.bin'
       call MAPL_HorzTransformCreate(mcfio%trans,tileFile, gridnamef, gridname, RootOnly=.false., vm=vm,rc=status)
       VERIFY_(status)
    else if (trim(regridmethod_)=="BILINEAR") then
       call MAPL_HorzTransformCreate(mcfio%trans,im,jm,img,jmg,rc=status)
    end if
  ...
  subroutine MAPL_CFIOReadBundleRead(MCFIO,time,hw,rc)
    ...
    integer   :: nn,l,k,lm,nlevs
    integer   :: im0,jm0,img,jmg,nymd,nhms,lt,hw_
    integer   :: counts(3)
    logical   :: myGlobal
    real, pointer :: farray2d(:,:)
    real, pointer :: farray3d(:,:,:)
    real, pointer :: ptr2(:,:)
    real, pointer :: ptr3(:,:,:)
    character(len=ESMF_MAXSTR) :: date
    character(len=ESMF_MAXSTR) :: name
    ...
    ! This needs cleanup 
    allocate(farray2d(mcfio%im,mcfio%jm),stat=status)
    VERIFY_(STATUS)
    allocate(farray3d(mcfio%im,mcfio%jm,1),stat=status)
    VERIFY_(STATUS)

    lt = size(mcfio%reqs)
    allocate(mcfio%buffer(lt))
    nn = 0
    VARS: do L=1,size(MCFIO%VarDims)
       ...
       RANK: if (MCFIO%VarDims(L)==2) then
          ...
          if (myGlobal) then
             call ESMF_CFIOVarRead(mcfio%cfio,trim(MCFIO%VARNAME(L)),farray2d,timeString=date,rc=status)
             VERIFY_(STATUS)
             allocate(mcfio%buffer(nn)%ptr(img,jmg),stat=status)
             VERIFY_(STATUS)
             call TransAndSave(farray2d,mcfio%buffer(nn)%ptr,mcfio%reqs(nn))
          end if
          ...
          do k = 1,MCFIO%lm
             ...
             call MAPL_CreateRequest(MCFIO%GRID, MCFIO%Krank(nn), MCFIO%reqs(nn), &
                  tag=nn, RequestType=MAPL_IsScatter, DstArray = ptr3(:,:,k), prepost=.true.,hw=hw_,RC=STATUS)

             if (myGlobal) then

                call ESMF_CFIOVarRead (mcfio%cfio, trim(MCFIO%VARNAME(L)), farray3d, &
                     kbeg=k, kount=1, timeString=DATE, RC=STATUS)
                allocate(mcfio%buffer(nn)%ptr(img,jmg),stat=status)
                VERIFY_(STATUS)
                call TransAndSave(farray3d(:,:,1),mcfio%buffer(nn)%ptr,mcfio%reqs(nn))
             end if
          end do
    ...
    subroutine TransAndSave(ptrin,ptrout,req)
       real  :: ptrin(:,:)
       real  :: ptrout(:,:)
       type(MAPL_CommRequest) :: req

       if (MAPL_HorzTransformIsCreated(mcfio%trans)) then
          call MAPL_HorzTransformRun(mCFIO%Trans, ptrin, ptrout, MAPL_undef, rc=STATUS)
          VERIFY_(STATUS)
       else
          ptrout=ptrin
       endif
       if (mcfio%gsiMode) call shift180Lon2D_(ptrout,img,jmg)
       call MAPL_ArrayIScatter(ptrout,req,hw=hw_,rc=status)
       VERIFY_(STATUS)
  ...
  subroutine MAPL_CFIOReadBundleWait(MCFIO,rc)
    ...
    integer          :: status
    Character(len=ESMF_MAXSTR), parameter :: Iam = "MAPL_CFIOReadBundleWait"
  ...
  subroutine MAPL_CFIORegistryGet(filename,cfio,time_is_cyclic,rc) 
     character(len=*), intent(in) :: filename
     logical, optional, intent(inout) :: time_is_cyclic
     integer, optional, intent(out) :: rc
     type(ESMF_CFIO), pointer, intent(inout) :: cfio
     integer :: status
     character(len=ESMF_MAXSTR) :: Iam="MAPL_CFIORegistryGet"

     character(len=ESMF_MAXPATHLEN) :: cfiofname
     ...
     if (CFIOPreExist) then
       ...
       CFIORegister(iCFIO) =  ESMF_CFIOCreate (cfioObjName=trim("TEMP_name"), RC=STATUS)
       VERIFY_(STATUS)
       CFIO => CFIORegister(iCFIO)
  ...
  end subroutine MAPL_CFIORegistryGet
