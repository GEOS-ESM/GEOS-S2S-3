...
!
! !REVISION HISTORY:
!  SVN:$Id: ice_step_mod.F90,v 1.21.6.3.2.2.14.4.2.8 2018/09/04 16:30:11 bzhao Exp $
!
...
#ifdef GEOS
#if 0
     ! asum_init = global_sum(vice(:,:,:),     distrb_info,       &
     !                        field_loc_center, tarea)
     ! if (my_task == master_task) then
     !    write (nu_diag,*) ' init ice volume = ', asum_init
     ! endif
      do iblk = 1, nblocks
         this_block = get_block(blocks_ice(iblk), iblk)
         ilo = this_block%ilo
         ...
         do j = jlo, jhi
         do i = ilo, ihi
            if (this_block%i_glob(i) == 234 .and. &
                this_block%j_glob(j) == 199) then
                print*, 'before transport'
                do n=1,ncat
                   print*, n, aicen(i,j,n,iblk), vicen(i,j,n,iblk)  
                   print*, n, vsnon(i,j,n,iblk), esnon(i,j,n,iblk)  
                enddo
                print*, sum(aicen(i,j,:,iblk)), sum(vicen(i,j,:,iblk))
            endif
         enddo               ! i
         enddo               ! j
      enddo               ! j
...
#ifdef GEOS
#if 0
      vice = sum(vicen, dim=3)
      asum_final = global_sum(vice(:,:,:),     distrb_info,       &
                             field_loc_center, tarea)
      if (my_task == master_task) then
         write (nu_diag,*) 'remap ice volume = ', asum_final
         write (nu_diag,*) 'remap ice volume dif = ', asum_final - asum_init
      endif
      asum_init = asum_final
      !call scale_ice_state_frocean(0)
      do iblk = 1, nblocks
         this_block = get_block(blocks_ice(iblk), iblk)
         ilo = this_block%ilo
         ihi = this_block%ihi
         ...
         do j = jlo, jhi
         do i = ilo, ihi
            if (this_block%i_glob(i) == 257 .and. &
                this_block%j_glob(j) == 197) then
...
!!                         Delt    (:,:,  iblk), divu      (:,:,  iblk), &
      ...
      asum_final = global_sum(vice(:,:,:),     distrb_info,       &
                             field_loc_center, tarea)
      if (my_task == master_task) then
         write (nu_diag,*) 'ridge ice volume = ', asum_final
         write (nu_diag,*) 'ridge ice volume dif = ', asum_final - asum_init
      endif
      asum_init = asum_final
      ...
      do iblk = 1, nblocks
         ...
         do i = ilo, ihi
            if (this_block%i_glob(i) == 257 .and. &
                ...
                enddo
                print*, sum(aicen(i,j,:,iblk)), sum(vicen(i,j,:,iblk))
                print*, dardg1dt(i,j,iblk), dardg2dt(i,j,iblk)
            endif
         enddo               ! i
         enddo               ! j
      enddo               ! j
...
!#endif
      ...
      asum_final = global_sum(vice(:,:,:),     distrb_info,       &
                             field_loc_center, tarea)
      if (my_task == master_task) then
         write (nu_diag,*) 'clean ice volume = ', asum_final
         write (nu_diag,*) 'clean ice volume diff = ', asum_final - asum_init
         write (nu_diag,*) 'clean ice volume diff(%) = ', &
                 100*abs(asum_final - asum_init)/asum_init
      endif
#endif
      ...
      real (kind=dbl_kind), dimension(nx_block,ny_block,max_blocks) :: &
       ...
       asum_init = global_sum(vice(:,:,:),     distrb_info,       &
                             field_loc_center, tarea)
       if (my_task == master_task) then
          write (nu_diag,*) 'clean0 ice volume = ', asum_init
       endif
#endif
        do iblk = 1, nblocks  
           this_block = get_block(blocks_ice(iblk),iblk)
       ...
       asum_final = global_sum(vice(:,:,:),     distrb_info,       &
                             field_loc_center, tarea)
       if (my_task == master_task) then
          write (nu_diag,*) 'clean0 ice volume = ', asum_final
          write (nu_diag,*) 'clean0 ice volume diff = ', asum_final - asum_init
       endif
#endif
      ...
      subroutine scale_ice_state_frocean(direction)
      ...
      real (kind=dbl_kind), dimension (nx_block,ny_block) :: &
        ...
        do iblk = 1, nblocks  
           ...
           if(direction /= 0) then
               scale_factor(:,:) = toceanfr(:,:,iblk) 
           else
               where(toceanfr(:,:,iblk) > c0)
                  scale_factor(:,:) = c1/toceanfr(:,:,iblk) 
               elsewhere
                  scale_factor(:,:) = c1
               endwhere
           endif
           ...
           do n=1,ncat
             aicen(:,:,n,iblk) = aicen(:,:,n,iblk) * scale_factor(:,:)
             vicen(:,:,n,iblk) = vicen(:,:,n,iblk) * scale_factor(:,:)
             vsnon(:,:,n,iblk) = vsnon(:,:,n,iblk) * scale_factor(:,:)
           enddo 
           do k=1,ntilyr  
             eicen(:,:,k,iblk) = eicen(:,:,k,iblk) * scale_factor(:,:) 
           enddo
           do k=1,ntslyr  
             esnon(:,:,k,iblk) = esnon(:,:,k,iblk) * scale_factor(:,:) 
           enddo
           aice0(:,:,iblk) = aice0(:,:,iblk) * scale_factor(:,:)
       enddo

       ! note: tracers are not scaled
       ! in particular, not sure if volume pond should be scaled or not.
       ! for now, it is ok as long as not using melt pond
      ...
      end subroutine scale_ice_state_frocean
#endif
