...
!
! !REVISION HISTORY:
!  SVN:$Id: ice_mechred.F90,v 1.12 2018/02/05 19:58:44 bzhao Exp $
!
...
!-----------------------------------------------------------------------
      ...
      !*** in namelist now for tuning purpose
      real (kind=dbl_kind) :: & 
         Pstar,               & ! constant in Hibler strength formula 
                                ! (kstrength = 0) 
...
!BOP
...
#ifdef GEOS
      subroutine init_ice_mechred(strength    , & 
                                  rdg_partic  , & 
                                  rdg_redist  , &
                                  murdg       , &
                                  ps)     
      ...
      integer (kind=int_kind), intent(in) :: & ! defined in namelist 
         strength    , & ! 0 for simple Hibler (1979) formulation 
                         ! 1 for Rothrock (1975) pressure formulation 
         rdg_partic  , & ! 0 for Thorndike et al. (1975) formulation 
                         ! 1 for exponential participation function 
         rdg_redist      ! 0 for Hibler (1980) formulation 
                         ! 1 for exponential redistribution function 
      real (kind=real_kind), intent(in) :: & ! defined in namelist 
         murdg, ps

         kstrength    = strength
         krdg_partic  = rdg_partic
         krdg_redist  = rdg_redist
         mu_rdg       = real(murdg, kind=dbl_kind)
         Pstar        = real(ps,    kind=dbl_kind)

      end subroutine init_ice_mechred  
#endif
!
...
!
      ...
      do ij = 1, icells
         i = indxi(ij)
         j = indxj(ij)
         if(isnan(aice0(i,j))) then
            print*, 'asum_ridge: i: ',i,' j: ',j, 'NAN aice0'  
            call abort_ice('asum_ridge: bad aice0')
         endif
         asum(ij) = aice0(i,j)
      enddo
...
!
      ...
      !-----------------------------------------------------------------
         ...
         if(isnan(closing_net(ij))) then
            write (nu_diag,*) ' '
            write (nu_diag,*) 'Ridging error: NAN closing_net'
            write (nu_diag,*) 'i, j, closing_net:', i, j, closing_net(ij)
            write (nu_diag,*) 'rdg_shear: ', rdg_shear(i,j)
            write (nu_diag,*) 'rdg_conv: ', rdg_conv(i,j)
            write (nu_diag,*) 'divu_adv: ', divu_adv(ij)
            write (nu_diag,*) 'asum: ', asum(ij)
            call abort_ice('ridge_prep: bad closing_net')
         endif
...
!ocl novrec      !Fujitsu
      do ij = 1, icells
         ...
         if(isnan(aice0(i,j))) then
            ...
            write (nu_diag,*) ' '
            write (nu_diag,*) 'Ridging error: NAN aice0'
            write (nu_diag,*) 'i, j, aice0:', i, j, aice0(i,j)
            write (nu_diag,*) 'apartic: ', apartic(ij,0)
            write (nu_diag,*) 'closing_gross: ', closing_gross(ij)
            write (nu_diag,*) 'closing_net: ', closing_net(ij)
            write (nu_diag,*) 'opning: ', opning(ij)
            write (nu_diag,*) 'aksum: ', aksum(ij)
            return
         endif
         if (aice0(i,j) < -puny) then
            l_stop = .true.
...
!
      ...
      real (kind=dbl_kind), dimension (nx_block,ny_block,ncat), &
         intent(in) :: &
         aicen, vicen   ! concentration and volume of ice in each category
...
!
      ...
      do ij = 1, icells
         if (abs(asum(ij) - c1) < puny) then
            ...
            write(nu_diag,*)  0, aice0(i,j)
            do n = 1, ncat
               write(nu_diag,*) n, aicen(i,j,n), vicen(i,j,n)
            enddo
         endif
      enddo
