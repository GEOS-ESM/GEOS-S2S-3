...
! !OUTPUT PARAMETERS
  INTEGER, POINTER, INTENT(OUT), DIMENSION(:,:)   :: ireg       ! number of land types in each grid square
  INTEGER, POINTER, INTENT(OUT), DIMENSION(:,:,:) :: iuse       ! fraction of grid box area occupied by land type
  INTEGER, POINTER, INTENT(OUT), DIMENSION(:,:,:) :: iland      ! land type id in grid square for ireg land types
...
!      landNum(ic) = m
     ...
     IF(ic > NTYPE) THEN
      ...
      CALL MAPL_Sort(milFrac,landNum)
...
! !OUTPUT PARAMETERS
  REAL*8 , POINTER, INTENT(OUT), DIMENSION(:,:,:) :: xlai       ! leaf area index of land type
